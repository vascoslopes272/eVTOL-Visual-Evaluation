...
function sideHTML(r,d,other,legacy){const f0=r.figs.find(f=>f.main)||r.figs[0];const rest=r.figs.filter(f=>f!==f0);
 ...
 <div class="ptitle2" style="font-size:16px;font-weight:600">${esc(r.title)}</div>
 ...
 <p class="help">1 not visible in the figures · 2 visible in the figures too · 3 the text states another type · 4 the text does not settle it · ←/→ navigate · click a figure to zoom</p>`}
function bind(r,P){
...
document.addEventListener('keydown',e=>{if(e.target.tagName==='INPUT'||e.target.tagName==='SELECT'){if(e.key==='Enter'){e.target.blur()}else return}
 ...
 if(e.key==='1'&&AG(r))decide(r,'confirm');else if(e.key==='1'&&!AG(r)&&r.text!=='NS')decide(r,'text','no');else if(e.key==='2'&&!AG(r)&&r.text!=='NS')decide(r,'text','yes');else if(e.key==='3')decide(r,'other');else if(e.key==='4')decide(r,'unsure');
