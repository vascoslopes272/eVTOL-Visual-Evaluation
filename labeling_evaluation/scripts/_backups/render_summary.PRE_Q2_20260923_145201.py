...
def load_tables(out: Path) -> dict:
    ...
    return {c.stem: pd.read_csv(c, keep_default_na=False) for c in sorted(tdir.glob("*.csv"))}
...
def main(argv) -> int:
    ...
    md = report.write_markdown(None, tables, figs, out, filename="LABELLING_ANALYSIS_BRIEF.md",
