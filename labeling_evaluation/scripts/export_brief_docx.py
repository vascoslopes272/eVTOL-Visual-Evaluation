...
USABLE_W_IN = 6.50
USABLE_H_IN = 7.70
#: the tallest a single figure may print. The print stylesheet keeps figures well under the
#: text height; pandoc's default let a tall panel fill a page on its own, which is most of why
#: the Word file ran 67 % longer than the PDF of the same markdown (measured 2026-09-25).
MAX_FIG_H_IN = 5.60
...
def _fit(pct: float, px: tuple[int, int] | None) -> str:
    ...
    iw, ih = px
    h = w * ih / iw
    cap = min(USABLE_H_IN, MAX_FIG_H_IN)
    if h > cap:
        w *= cap / h
        h = cap
...
def _compact_styles(doc) -> None:
    """Bring the Word file to the density of the printed PDF.

    ``build_styled_md_pdf`` renders this document in ``--compact`` mode; pandoc has no notion of
    that and writes Word's defaults (11 pt body, 1.08 line spacing, a 10 pt gap after every
    paragraph, 11 pt table text). On the same A4 page and the same markdown that ran 67 pages
    against the PDF's 40, measured 2026-09-25. This sets the body, heading and table styles to
    the print sizes; the panels keep the sizes ``_restyle_box_paragraph`` already gives them.
    """
    from docx.shared import Pt
    from docx.enum.text import WD_LINE_SPACING

    def _tune(name, size=None, before=None, after=None, spacing=1.0):
        try:
            st = doc.styles[name]
        except KeyError:
            return
        if size is not None and st.font is not None:
    ...
    _tune("Normal", size=9.5, before=0, after=4, spacing=1.05)
    _tune("Body Text", size=9.5, before=0, after=4, spacing=1.05)
    _tune("First Paragraph", size=9.5, before=0, after=4, spacing=1.05)
    _tune("Compact", size=9.5, before=0, after=2, spacing=1.05)
    _tune("Title", size=17, before=0, after=6)
    _tune("Subtitle", size=10.5, before=0, after=8)
    _tune("Heading 1", size=14, before=12, after=4)
    _tune("Heading 2", size=11.5, before=9, after=3)
    _tune("Heading 3", size=10, before=7, after=3)
    _tune("Heading 4", size=9.5, before=6, after=2)
    _tune("Heading 5", size=9, before=5, after=2)
    _tune("Caption", size=8, before=1, after=6)
    _tune("Image Caption", size=8, before=1, after=6)
    for lst in ("List Paragraph", "Compact List", "First Paragraph"):
        _tune(lst, size=9.5, before=0, after=2, spacing=1.02)

    # tables: the document's tables are wide and many-rowed, and 11 pt forced them to wrap
    for t in doc.tables:
        for row in t.rows:
            for cell in row.cells:
                for para in cell.paragraphs:
                    ...
                    for run in para.runs:
                        # the panels are one-cell tables and own their sizes already
