...
TABLE_CAPTIONS: Dict[str, str] = {
    "a2_d1_rejection_reasons": "Why a patent is not representative: the wizard's reason first, then the tag that removed it",
    "a2_d1_similars": "The \"but similar\" tags: near misses of the domain, how each is caught, and what it removes",
    "a2_d9_publication_lag_region": "Priority-to-publication lag, years, three main regions",
    "a2_d1_filing_status": "Filing status at the three patent levels, and what each status means",
    "a2_d7_duplicates": "From aircraft observations to unique aircraft: originals, observations (O1, O2) and similars "
                        "(S3); the last column counts the same types one step earlier — every aircraft observation on "
                        "all the patents approved at labelling, before the domain gate removed any",
    ...
    "a2_d2_label_set": "The label set: total slots, the export columns they fill, and the slots answered per aircraft",
    "a2_d2_questions_by_card": "Slots per card and kind (Figure 3.3b), and the export columns they fill",
    "a2_d4_missingness": "Blanks where the parent field says the part exists; values an override hides are left out",
    "a2_d3_selected_fields": "Most common answers, G1 to M3 fields, closed by two propulsion rows",
    ...
    "a2_d5_archetype_cardinality": "Archetype cardinality at nine levels; a level ending in c adds the propulsor count",
    "a2_d9_architecture_by_window": "Class shares per window, four largest classes",
    "a2_d14_firm_proximity": "Technological proximity: each firm's architecture profile and the firm closest to it",
    "a2_d13_flagship_check": "Flagship check: labels of the largest filers against their public products",
    "a2_d15_trl_by_class": "Technology readiness (NASA TRL) reached by the unique aircraft, per architecture class",
    "a2_d15_trl_status": "Programme status against the level reached",
    "a2_d16_public_match": "The patent's architecture class against the named public aircraft",
    "a2_d16_public_differences": "Named aircraft whose patent class differs from the public aircraft",
...
FIGURE_CAPTIONS: Dict[str, str] = {
    ...
    "codebook_classes": "the twelve architecture classes of card G1; the solid line is the part that decides the class, the dashed line the hover position",
    "codebook_dimensions": "every dimension of the label cards G1 to M3; a tick box is a flag, # a number, \"if\" a slot asked only when its condition holds, ×n a card repeated per part, pink an escape",
    "fig_03_design_space_order": "the four label cards of one unique aircraft, and the order of the design-space analysis",
    "fill_rate_heatmap": "share answered of each field, measured only on the aircraft that have its part; a value an override hides is left out",
    "archetype_levels": "the two columns that pick the archetype level; hatched, the level plus the propulsor count",
    "aircraft_spans_timeline": "every aircraft filed in more than one year, from its first to its last filing; the filled dot is the primary record, whose priority year sets the aircraft's window",
    "window_shares_two_counts": "class share per window, each aircraft counted once at its primary record (solid) or in every window from its first to its last filing (dashed)",
    "public_match_bars": "named unique aircraft: the patent's architecture class against the aircraft the firm showed or flew",
    "trl_by_class": "unique aircraft per architecture class that reached TRL 3 or higher on public evidence, by NASA level; the label gives them out of the whole class",
    "proximity_heatmap": "technological proximity between the firms with five or more unique aircraft, on the architecture class; close firms patent a similar mix of classes",
...
FIG_WIDTH: Dict[str, str] = {
    ...
    "codebook_classes": "100%", "codebook_dimensions": "100%",
    "patents_per_year": "78%",
    "fill_rate_heatmap": "66%", "archetype_levels": "96%", "class_balance_bars": "56%",
    "class_share_stacked_area": "66%", "slots_histogram": "56%", "lorenz": "36%",
    "aircraft_spans_timeline": "78%", "window_shares_two_counts": "98%", "proximity_heatmap": "76%", "trl_by_class": "74%", "public_match_bars": "74%",
}

#: figures printed on their own A4-landscape page, at the full page width
...
COLUMNS: Dict[str, List[str]] = {
    "a2_d1_rejection_reasons": ["reason", "what it means", "patents"],
...
NODES: List[Dict] = [
    ...
    dict(id="1", title="Introduction",
         ...
         text=(
             ...
             "occupied: an aircraft the review tagged *UAV but similar* or *not electric but "
             "similar* is near the boundary but outside it, and leaves. *Granted* and *pending "
    ...
    dict(id="2", title="eVTOL Data Sets: Acquisition and Refinement",
         ...
         text=("Three refinements, each at its own level, take the acquired patents to the "
               ...
               "figures the labels rest on. Figure 2.1 writes each refinement as a sum: every "
               "stage is the stage above it minus what left, and every line of the sum names the "
               "table that breaks it down."),
         code=[]),
    dict(id="2.1", title="Refinement 1 — representativity", level="patent",
         text=("A patent is representative when it shows an eVTOL aircraft and the aircraft can "
               ...
               "mode, and marks the near misses with a tag: UAV but similar, not electric but "
               "similar. A tagged aircraft leaves the analysis set. {gated_patents} approved "
               "patents lose every aircraft this way, so {representative_s} ({rep_share}) are "
               "representative."),
         code=[]),
    dict(id="2.1.1", title="Disapproval reasons",
         text=("The reasons are the labelling wizard's own, in its words and order; the tags of the "
               "identity review come after them. At labelling the two conditions themselves "
               "dominate: {r_noimg} patents have no usable, sufficient or legible aircraft image, "
               "and a domain miss removes {r_uav} as pure UAV and {r_ood} as out of the technology "
               "domain; {r_other} more are marginal, {r_wiz} in all. The review then removes "
               "{r_sim} approved patents whose every aircraft is tagged: {r_sim_uav} UAV but similar "
               "and {r_sim_el} not electric but similar. {r_wiz} + {r_sim} = {not_rep} patents are "
               "not representative, the {acquired_s} acquired minus the {representative_s} "
               "representative."),
         tables=["a2_d1_rejection_reasons"],
         code=["display(a2.d1_funnel(ds))", "display(a2.d1_approval_by_region(ds))",
               "a2.d1_rejection_reasons(ds)"]),
    dict(id="2.1.2", title="The similars: UAV but similar, not electric but similar",
         text=("Two kinds of near miss sit next to the boundary, each caught from the text, "
               "because neither occupancy nor powertrain can be read from a drawing. Each tag "
               "names an aircraft, not a patent: a patent that draws several aircraft keeps the "
               "ones without a tag, and an O1 or O2 observation carries the tags of the aircraft "
               "it repeats. The tags remove {gated_aircraft} unique aircraft ({uav_primary} UAV but "
               "similar, {notel_primary} not electric but similar; {two_tags} carry both). The last "
               "column is the count Table 2.1.1 carries: a patent that loses every aircraft is "
               "counted once, under its first tag.{stol_note} What stays is deliberate: V/STOL, "
    ...
    dict(id="2.1.4", title="Filing status",
         text=("Granted and pending are the office's decision on the claims, not on the drawing, "
               ...
               "{withdrawn_pri} were withdrawn, refused or suspended. *In force* and *since "
               "lapsed* are both granted patents; a lapsed one reached the end of its term or its "
               "owner stopped paying the renewal fees. The analysis never separates the two, "
               "because a lapsed grant is still a design the office accepted; the split is shown "
               "for completeness, and the sensitivity line below pools them."),
    ...
    dict(id="2.2", title="Refinement 2 — observations to unique aircraft",
         ...
         text=("The {representative_s} representative patents hold {observations_s} aircraft "
               "observations: one per patent, and {rep_extra} more because {rep_multi} patents draw "
               "two to five aircraft, each a row of its own. Observations of an aircraft already in "
               "the set are removed ({o2_obs} O2 and {o1_obs} O1, {o12_removed} in all), which leaves "
               "{unique} unique aircraft: {orig_obs} originals and {s3} S3 similars, each a new "
               "aircraft. They sit on {primary} primary patents, the {representative_s} "
               "representative patents minus the {o12_patents} whose only aircraft is a repeat."),
         code=[]),
    dict(id="2.2.1", title="Same unique aircraft in different patents: O1, O2, S3",
         text=("O1 and O2 are the same aircraft seen again; their labels are inherited from the "
               ...
               "full, and counts as one new unique aircraft. The table counts inside the "
               "representative set, so its rows add up to Figure 2.1: {observations_s} observations "
               "minus {o12_removed} removed leaves {unique} unique aircraft. The last column counts "
               "the same types among all approved patents, before the similar tags remove any "
               "({o1_all} O1, {o2_all} O2, {s3_all} S3). {s3_identical} of the {s3} S3 aircraft are "
               "identical to their original on all seven archetype fields; that double-count is "
               "kept as a sensitivity run."),
    ...
    dict(id="2.3.1", title="Reasons to approve a figure, and the resulting count",
         text=("A figure is approved when it shows the aircraft in a view the architecture can be "
               ...
               "(tilt mechanism, rotor) and are set aside.{nostatus_note}"),
    ...
    dict(id="3.3", title="Design space analysis", level="unique aircraft, G1 to M3",
         figures=["codebook_classes", "codebook_dimensions", "fig_03_design_space_order"],
         text=("Figure 3.3a names the twelve architecture classes of card G1 and Figure 3.3b "
               "every dimension of the four label cards G1 to M3. The design space is read from "
               "those four cards of one unique aircraft, in the order Figure 3.3c shows: from "
               "completeness to the level at which aircraft are counted, and on to the first look "
               "at movement over time."),
         code=[]),
    dict(id="3.3.1", title="Slots answered per aircraft",
         text=("A slot is one item a label card asks, as Figure 3.3b draws it: {questions} slots on the four cards G1 to "
               "M3, of which {q_dim} are dimensions (one answer from a list), {q_tick} ticks and "
               "{q_num} numbers; the tags and escapes beside them are not counted. Repeated per boom "
               "group, wing panel, propulsor host and propulsor type, they fill {cols} export "
               "columns; the {cols_off} coded columns the drawing does not show ({off_names}) are "
               "left out. Sparsity is structural, a slot is blank when the part does not "
               "exist, so an aircraft answers a median of {median_q} of the {questions} slots "
               "and {q_lt5_phrase} answered on fewer than 5 % of aircraft. At column level {informative} "
               "export columns carry the information and {near_constant} are near-constant (3.3.4)."),
         tables=["a2_d2_label_set", "a2_d2_questions_by_card"],
         code=["display(a2.d2_label_set(ds, near_constant_min_answered=F.get('near_constant_min_answered', 100),",
               ...
               "display(register.by_card(ds))",
               "display(register.questions(ds).drop(columns='column_names'))",
               "_ = figures.slots_histogram(register.per_aircraft(ds))"]),
    dict(id="3.3.2", title="Missingness",
         text=("A blank is a labelling gap only where the parent field says the part exists. On "
               ...
               "Figure 3.3.2 measures each slot only on the aircraft that have the part: a wing "
               "slot on the winged aircraft, a boom slot on the aircraft with booms. "
               "Divided by every aircraft instead, a boom slot would look answered on fewer than "
               "half of them only because most aircraft have no booms. Measured where the part "
               "exists, every field is answered on at least 95 % of the aircraft it applies to."),
    ...
    dict(id="3.3.3", title="Most common answers, G1 to M3 labels",
         text=("The table covers the aircraft-level labels G1 to M3 only. Which answers dominate "
               ...
               "Two derived rows close the table, because the propulsors drive the eVTOL taxonomy. "
               "*Propulsor units per aircraft* are given in the bins the archetypes use (3.3.6). "
               "*Propulsor groups per aircraft* count the stations that carry units: a boom group, a "
               "wing panel, the tail, the fuselage or the core layout. {grp2} aircraft carry their "
               "propulsors in two groups, typically one for lift and one for cruise, {grp1} in one "
               "and {grp3} in three. Hoverbikes and personal flying vehicles have no propulsor "
               "record and are not counted ({units_left} aircraft)."),
    ...
    dict(id="3.3.6", title="Archetype cardinality",
         text=("An archetype is the string formed by joining the chosen fields; *distinct "
               ...
               "Every level also has a *c* version that adds the number of propulsor units, "
               "because in the eVTOL taxonomy the count is a design decision and not a "
               "measurement: a four-rotor and an eight-rotor multirotor are different aircraft. "
               "The count is binned 0-3 / 4 / 5-6 / 7-8 / 9 or more. The bins follow the counts "
               "themselves: 4, 6 and 8 are the peaks ({bin_r4} aircraft have exactly four "
               "units), and 5 and 7 ({u5} and {u7} aircraft, {u5_slc} and {u7_slc} of them lift + "
               "cruise, an even lift set plus one pusher) join the next even count. Hoverbikes and "
               "personal flying vehicles keep \"no propulsor record\" as their own answer, so they "
               "stay in. A0c reaches the resolution of A1 (effective number {a0c_eff} against "
               "{a1_eff}) with {a0c_single} singletons instead of {a1_single}. A1c, A1tc and A2c "
               "cross the five-per-cent line ({a1c_single}, {a1tc_single} and {a2c_single} "
               "singletons): once the count joins the number of wings, a level starts naming "
               "individual aircraft."),
    ...
    dict(id="3.3.8", title="Class shares per window",
         text=("A first look at a shift away from tilt rotor toward lift plus cruise and combined "
               "vectored thrust, the two classes that gain most in 2020-23, and no class above "
               ...
               "Each unique aircraft is counted once, in the window of the priority year of its "
               "primary record, the original the labeller flagged, which is not always its first "
               "appearance. {sp_repeat} of the {unique} aircraft were filed again as O1 or O2: "
               "{sp_sameyear} in the same year, {sp_span} over several years (Figure 3.3.8b, up to "
               "{sp_max} years apart), {sp_multiwin} of them across two windows or more. "
               "{sp_earlier} have a re-filing older than the primary record, and counting them at "
               "their first filing would move {sp_move} to an earlier window. The other {sp_single} "
               "aircraft are a single point in time. Figure 3.3.8c counts every aircraft in each "
               "window its filings touch, first to last and the windows in between: no class share "
               "moves by more than {sp_maxdiff}, so the trend does not depend on which filing dates "
               "an aircraft."),
         tables=["a2_d9_architecture_by_window"],
         figures=["class_share_stacked_area", "aircraft_spans_timeline", "window_shares_two_counts"],
         code=["windows = a2.d9_architecture_by_window(ds)",
               ...
               "display(windows)",
               "display(a2.d9_architecture_by_window_active(ds))",
               "a2.d9_aircraft_spans(ds).query('repeats > 0').head(20)"]),
    dict(id="3.3.9", title="Technological proximity between firms",
         text=("Technological proximity asks how alike two firms' patent portfolios are (Jaffe, "
               "1986). Each firm is a vector: how many of its unique aircraft fall in each "
               "architecture class. The proximity of two firms is the cosine of the angle between "
               "their vectors: 1 when both spread their aircraft over the classes in the same "
               "proportions, whatever their size, 0 when they share no class. It is computed for "
               "the {px_firms} firms with at least {px_min_aircraft} unique aircraft ({px_aircraft} "
               "aircraft); below that a profile is too thin to compare.\n\n"
               "Figure 3.3.9 orders the firms so that proximate ones sit together. On the current "
               "numbers two groups appear: firms whose aircraft tilt their rotors (tilt rotor and "
               "combined vectored thrust, around Bell / Textron and Leonardo) and firms that "
               "separate lift from cruise (lift + cruise, around Airbus and Honda), with a few "
               "mixed portfolios between them. The mean proximity over the {px_pairs} pairs is "
               "{px_mean_class}; {px_pairs_high_class} pairs are at 0.9 or above and "
               "{px_pairs_zero_class} share no class at all. The class alone is coarse: "
               "{px_top_pair} are identical on it ({px_top_val}) but fall to {px_top_rot} once the "
               "propulsor count is added, and the mean over all pairs falls to {px_mean_rotors}. "
               "The table gives each firm's closest firm on both profiles.\n\n"
               "Proximity describes portfolios, not influence: it says which firms patent the same "
               "kinds of aircraft, not that one follows another. It is also a snapshot over the "
               "whole period; whether firms converge over time is a question for the evolution "
               "chapter."),
         tables=["a2_d14_firm_proximity"], figures=["proximity_heatmap"],
         code=["px = a2.d14_firm_proximity(ds)", "print(px.attrs)", "px"]),
    # ================================================================ 4
    ...
    dict(id="4.1", title="Flagship check",
         text=("{flagship_companies} companies with two or more aircraft are checked and the "
               "first eight are shown. {flagship_public} of them have a public product, and for "
               "{flagship_match} the top label matches it. The other {flagship_mismatch} "
               "({flagship_mismatch_names}) file alternative embodiments next to the product they "
               "fly, which is itself a finding: a firm's patents cover more of the design space "
               "than its public aircraft."),
    ...
    dict(id="4.2", title="Technology readiness of the unique aircraft",
         text=("A patent says what was invented, not whether it was built. The readiness of each "
               "unique aircraft is therefore read from outside the patent record and placed on the "
               "nine Technology Readiness Levels of NASA (NPR 7123.1D, Appendix E), quoted verbatim. "
               "No public list assigns a TRL to named eVTOL aircraft, so the evidence rule is this "
               "study's own: a subscale flight is TRL 5; a full-scale prototype flown only tethered, "
               "in a hangar or in hover is TRL 6; free flight through the working mode, transition "
               "for a winged aircraft, is TRL 7; a type certificate is TRL 8; service is TRL 9. Only "
               "reported outcomes count, never plans. An aircraft with no public evidence of hardware "
               "stays at TRL 2, whose NASA description (\"invention begins ... no experimental proof "
               "... is available\") fits a patent on its own.\n\n"
               "{trl_evidence} of the {trl_aircraft} aircraft have a ruling with a quoted source, "
               "mostly the evtol.news directory, and {trl_above} ({trl_above_share}) reached a level "
               "above 2: {trl_35} at TRL 3 to 5, {trl_67} at TRL 6 or 7, and {trl_89} at TRL 8, the "
               "one type-certified aircraft. {trl_most_class} {trl_most_verb} the most aircraft above TRL 2 "
               "({trl_most_n}); among classes with twenty or more aircraft, {trl_best_class} has the "
               "largest share ({trl_best_share}). No aircraft of {trl_zero_classes} has left paper.\n\n"
               "The level is the highest reached; a closed programme keeps it. Of the aircraft above "
               "TRL 2, {trl_st_active} belong to active programmes, {trl_st_paused} to paused ones, "
               "{trl_st_superseded} were superseded by a successor from the same firm, "
               "{trl_st_ended} ended and {trl_st_unknown} have no news after 2023. The counts favour "
               "firms that name their aircraft: an unnamed design cannot be linked to a public "
               "record, so TRL 2 means no public evidence of hardware, not that none exists. "
               "{trl_medium} rulings rest on a source that leaves the scale or flight mode open."),
         tables=["a2_d15_trl_by_class", "a2_d15_trl_status"], figures=["trl_by_class"],
         code=["tc = a2.d15_trl_by_class(ds)", "print(tc.attrs)", "display(tc)", "a2.d15_trl_status(ds)"]),
    dict(id="4.3", title="The patent against the public aircraft",
         text=("A name links {pub_aircraft} unique aircraft to an aircraft the firm has shown or flown. "
               "For {pub_yes} ({pub_yes_share}) the class read from the patent drawing is the class of "
               "that aircraft. {pub_taxonomy} more are the same aircraft filed under another class by "
               "the directory, whose five classes draw the lines elsewhere (a cyclorotor or a slowed "
               "rotor is an electric rotorcraft there).\n\n"
               "{pub_diff} differ. In {pub_no} of them the patent's own text, read independently as "
               "the architecture ground truth, describes the same configuration as the drawing: "
               "{pub_no_names}. These patents claim an alternative embodiment next to the aircraft the "
               "firm built, as the flagship check (4.1) found at firm level; the label is right and the "
               "difference is the patent's. They keep TRL 2 in 4.2, since the configuration they "
               "describe never flew. In {pub_image_phrase} the drawing label departs from the patent "
               "text, and the text matches the public aircraft: the only disagreements with the "
               "public record that come from reading the image."),
         tables=["a2_d16_public_match", "a2_d16_public_differences"], figures=["public_match_bars"],
         code=["a2.d16_public_match(ds)", "a2.d16_public_differences(ds)"]),
    # ================================================================ 5
    dict(id="5", title="Method for the Design Space and Evolution Analysis",
         figures=["fig_05_handover"],
         text=("The class shares of section 3.3.8 move in one direction across the five windows, "
               "from tilt rotor toward lift plus cruise and combined vectored thrust, and at the "
               "same time no class "
    ...
    dict(id="5.3", title="The archetype and the distance",
         text=("The same aircraft is represented two ways, because the measures need different "
               ...
               "answer is concrete. Card M3 (propulsion) fills {cols_M3} export columns from "
               "{q_M3} slots, repeated per host and propulsor type; card G1 (architecture) "
               "fills {cols_G1}. That ratio records how much detail the "
               ...
               "two aircraft would come out similar mainly because their propulsor columns match. "
    ...
    dict(id="5.7", title="The dominant-design criterion, fixed in advance",
         text=("A dominant design is the configuration an industry settles on after a period of "
               ...
               "result stands on its own: across every family published since 2005, a bound set "
               "about a decade before the commercial inflection of around 2016 so that the phase "
               "of variety lies inside the corpus, the sector has not converged on a single "
               "architecture.\n\n"
