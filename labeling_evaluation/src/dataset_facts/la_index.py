...
each with its Source and How-to-read line. Since the reorganisations of 2026-09-23 the running
order is: Chapter 1 preliminary analysis (convergence, filers, region, design drivers),
Chapter 2 low-level analysis, then Appendix A data-set construction, Appendix B data quality,
Appendix C the questions the corpus could not answer, Appendix D the taxonomy. Change wording
here and the document follows.
"""
...
#: drawn as a row of small multiples (Figures 1.3.2a/b) or split over two strips.
...
FIGURE_PANELS: Dict[str, List] = {
    ...
    # 1.4: one panel per drawn trace of la_tables.TRACES, in that register's order (the key cell is empty)
...
ROW_CAP: Dict[str, int] = {
    **_pa.ROW_CAP,
    ...
    "la_driver_verdicts": 30,   # 1.4.3: every labelled trace and every unlabelled one, printed whole
...
COLUMNS: Dict[str, List[str]] = {
    **_pa.COLUMNS,
    ...
    # 1.4: `verdict class` stays in the CSV and feeds the takeaway counts
...
NODES: List[Dict] = [
    ...
    # The taxonomy followed them on 2026-09-23 ("regarding the two pages of taxonomy, put them in the
    # end, in the appendix"): it was Chapter 1 and is now Appendix D, the last thing in the document,
    # which moved Preliminary Analysis from 2 to 1 and Low-level from 3 to 2. Every cross-reference in
    # this module, in ``la_tables`` and in ``la_figures`` was renumbered from an old->new map built off
    # the NODES walk taken before that move; a stale number would resolve silently, because the move
    # RECYCLES numbers (old 3.1 became 2.1, and 2.1 was already in use).
    # ================================================================ 1  (was 2, was 4)
    ...
    dict(id="1", title="Preliminary Analysis", level="unique aircraft",
         text="How the data set was built, and every check on its quality, are in Appendix A and "
              ...
              "and the vocabulary every finding below is stated in — the twelve architecture classes "
              "and the 42 label slots — is the codebook reference of Appendix D."),
    dict(id="1.1", title="What is being designed, and whether it converges", level="unique aircraft"),
    dict(id="1.1.1", title="Filings per year", figures=["filings_per_year"], tables=["la_filings_per_year"]),
    ...
    dict(id="1.1.2", title="Class shares per window", figures=["atlas_arch_time"]),
    # 1.1.3 reads in three steps (user, 2026-09-22): what is evaluated and why, then the
    # conditions each stated in full, then the result. The archetype tables come before the test.
    dict(id="1.1.3", title="The dominant-design test", level="unique aircraft"),
    dict(id="1.1.3.1", title="What is evaluated: the archetype levels",
         ...
         after=("*How to read the two tables.* An archetype is a combination of label answers. At a "
                ...
                "archetype; and it is the level on which 1.1.6, 1.1.9 and 1.3.3 are built, so it is "
                ...
                "for the reader.")),
    dict(id="1.1.3.2", title="The conditions, fixed in advance", tables=["la_dd_conditions"]),
    dict(id="1.1.3.3", title="The result", figures=["dominant_design", "dominant_design_q"],
         tables=["la_dd_result", "la_dominant_design", "la_dd_q"]),
    dict(id="1.1.4", title="Within-class convergence", figures=["class_configs"], tables=["la_class_configs"]),
    dict(id="1.1.5", title="Dimension drift inside the classes", figures=["dimension_drift"], tables=["la_dimension_drift"]),
    # 2026-09-23, moved here from the end of 1.1 (it was 4.1.9) on the author's ruling: the crowded and
    # open zones explain the propulsor-count against architecture relation that 1.1.4 and 1.1.5 raise,
    ...
    dict(id="1.1.7", title="Where each class lives in time", figures=["class_cycles"]),
    dict(id="1.1.8", title="Lead and lag by region and by class", figures=["lead_lag"],
         tables=["la_lead_lag_region", "la_lead_lag_class", "la_class_region_timing"]),
    dict(id="1.1.9", title="Diversity per window", figures=["hill"], tables=["la_hill_by_window"]),
    dict(id="1.1.10", title="Abandonment: patents no longer in force", figures=["abandonment"],
         tables=["la_abandonment_by_class"]),
    dict(id="1.2", title="Who designs it, and how they compete", level="firm"),
    # 1.2.1: the tier table (la_firm_tiers) was removed on the author's ruling of 2026-09-23 — the
    ...
    dict(id="1.2.1", title="Coverage, and whether market standing tracks patenting",
         figures=["coverage"], tables=["la_market_vs_patents"]),
    dict(id="1.2.2", title="The body of filers over time, and what new firms enter with",
         figures=["filers_over_time"], tables=["la_filers_by_window", "la_cohort_mix"]),
    dict(id="1.2.3", title="Who filed, and which firms can move the reading",
    ...
    dict(id="1.2.5", title="IP strategy: depth, breadth, claims and citations", figures=["ip_strategy", "spans_by_firm"],
    ...
    # the first table of the section below) is cut — "take that out, it is not necessary, it is already all that I need in the heat
    ...
    dict(id="1.2.6", title="Technological proximity between firms, and region", figures=["proximity_region"],
    ...
    dict(id="1.2.7", title="The AAM Reality Index firms: score, funding, and the patent clock against the market clock",
    ...
    dict(id="1.2.8", title="TRL-ranked firms", tables=["la_trl_placeholder"]),
    ...
    dict(id="1.3.3", title="Regional specialisation by archetype", figures=["specialisation"]),
    # 2026-09-23, author's ruling: the table of this section is cut, the figure stays.
    dict(id="1.3.4", title="Country and class", figures=["country_class"]),
    # ================================================================ 1.4  (new 2026-09-23)
    # The author's frame: the design drivers are unobserved, each predicts a TRACE in the morphology,
    # and the traces are what the aircraft are labelled with. Placed as 1.4 rather than as 2.1.11
    # because it reads across the whole chapter — what moved (1.1), in which class (1.1.2) and how
    ...
    # is renumbered: 1.4 is the last section of the chapter.
    dict(id="1.4", title="Design drivers and their traces", level="unique aircraft",
         text=("The drivers of eVTOL design — the technology that moved over the window, the physics that "
               ...
               "separate them. So the section runs traces first and drivers second. 1.4.1 measures what "
               "moved inside each class, with the machinery of 1.1.5 extended to every labelled trace. 1.4.2 "
               "tests what is coupled across fields, which is the only prediction a fixed physical driver "
               "makes. 1.4.3 then states, for each trace, what moved, which drivers predict it and how many: "
               ...
               "movement inside a class is kept apart from the shift of the class mix (1.1.2), which can "
    ...
    dict(id="1.4.1", title="What moved inside the classes", figures=["trace_drift"]),
    dict(id="1.4.2", title="What is coupled: the physical drivers' test", figures=["trace_couplings"],
         tables=["la_trace_couplings"]),
    dict(id="1.4.3", title="The verdicts", tables=["la_driver_verdicts", "la_driver_questions"]),
    # ================================================================ 2  (was 3, was 5)
    dict(id="2", title="Low-level Analysis", level="unique aircraft and image"),
    # 2026-09-23, author's ruling: the table of this section is cut ("take it off"); the figure carries
    # the same shares.
    dict(id="2.1", title="Hybrid against electric, not-stated included", figures=["atlas_powertrain"]),
    dict(id="2.2", title="Propulsive units and their arrangement", figures=["atlas_units"]),
    ...
    dict(id="2.6", title="Examination outcome as a regulatory signal", figures=["examination"],
    ...
    # in Chapters 1 and 2, not before them.
    ...
    # ================================================================ Appendix D  (was chapter 1)
    # Moved here 2026-09-23 on the author's instruction ("regarding the two pages of taxonomy, put
    # them in the end, in the appendix"). It is a reference, not a finding: a reader consults it while
    # reading the chapters rather than reading it first, and it cost the front of the document two
    # landscape pages before a single number was shown. Chapter 1 points at it in its opening text.
    dict(id="D", title="Taxonomy",
         text="The vocabulary every finding in this document is stated in: the twelve architecture "
              "classes of card G1 and the 42 label slots of the codebook. Nothing here is a result — "
              "it is the reference the chapters are read against, which is why it sits at the end "
...
PROVENANCE: Dict[str, Dict[str, str]] = {
    ...
    # 1.1.3's own tables (la_archetype_levels, la_archetype_choice, la_dd_conditions,
    ...
    "dimension_drift": dict(
        ...
        transform="median propulsive units (an order statistic, not a mean); panels (i)–(iv) are the four "
                  "fields of the 1.1.4 configuration and divide by the class's own aircraft in that window, "
    ...
    "filer_weight": dict(
        ...
        transform="the share of the class (or of the class-window) held by its largest filer, and by "
                  ...
                  "1.1.6a's `filers` column says how many filers an archetype has and cannot say how "
    ...
    "linked_check": dict(
        unit="all {unique_s} unique aircraft, split into the ones linked to an evtol.news page and "
             "the ones with no page — the test behind every other item of 2.5",
...
TAKEAWAY_NUMBERS: Dict[str, Dict] = {
    ...
    # 1.1.6b, the filer-weight figure and table (2026-09-23)
    ...
    # hand-written table resolves; since 2026-09-23 Chapter 1 counts every organisation that filed.
    ...
    # --- 1.2.1 market standing against patenting (author's ruling 2026-09-23)
    ...
    # --- 1.2.3 firm leverage. tk_lev_firm / tk_lev_pp / tk_conc_max are defined once, further down
    ...
    # --- 1.2.2 entrants against the field
...
TAKEAWAY: Dict[str, str] = {
    ...
    "a2_d13_flagship_check": (
        ...
        "listed hold three or more. Every firm-level class statement in Chapter 1 is a mix, never "
    ...
    "la_dd_conditions": (
        ...
        "before this corpus was read. That is what makes 1.1.3.3 a test whose answer could have "
    ...
    "la_lead_lag_class": (
        ...
        "(CVT {tk_cvt_50}) are the ones still growing in 1.1.2, and the earliest (Tilt Rotor "
    ...
    "filers_over_time": (
        ...
        "Normalised, panel (ii) says what the aircraft counts of 1.1.2 cannot: in 2020-23 "
...
#
#     Question: Q2 what is gaining and what is dying — direct
#
# The eight questions are questions about the eVTOL SECTOR, not the four research
# questions of the methodology framework (Part G.1). RQ1-RQ3 are folded into Q2-Q6;
...
#: RENUMBERED 2026-09-23 on the author's instruction ("change the numbers of the Q to put
#: them in order"): the brief reads them in the order below, so the numbers now follow the
#: reading order instead of the order they were first proposed in. The permutation applied,
...
QUESTION: Dict[str, str] = {
    ...
    # ================================================================ 1 data sets
    "fig_02_refinement_funnel": "Q1 indicator value — premise (what the 665 are)",
    "atlas_removal": "Q1 indicator value — caveat (the domain boundary is a reading)",
    "a2_d1_similars": "Q1 indicator value — caveat (where a reviewer can push)",
    "a2_d9_publication_lag_region": "Q1 indicator value — caveat (fixes the 2023 cut-off)",
    "a2_d1_filing_status": "Q1 indicator value — premise (the three patent levels)",
    "atlas_duplicates": "Q7 one company or one design — premise (repetition is removed)",
    "a2_d7_duplicates": "Q7 one company or one design — premise (de-duplication is arithmetic)",
    "a2_d7_aircraft_per_patent": "Q7 one company or one design — premise (1 patent = 1 aircraft)",
    ...
    # ================================================================ Appendix D taxonomy
    "design_space_cards": "Q4 what is designed — premise (duplicates the class drawing)",
    "codebook_classes": "Q4 what is designed — premise (the twelve classes defined)",
    "codebook_dimensions": "Q4 what is designed — premise (the 42 slots defined)",
    ...
    "atlas_flagship": "Q8 patents vs what is built — direct (12 of 19 firms match)",
    "a2_d13_flagship_check": "Q8 patents vs what is built — direct (restates the figure)",
    "la_name_arch_check": "M figure vs text — direct (51 aircraft where sources disagree)",
    "a2_d3_selected_fields": "Q4 what is designed — direct (the modal design)",
    "atlas_fields": "Q4 what is designed — premise (label depth is a design property)",
    "a2_d2_informative_fields": "Q4 what is designed — premise (24 fields carry information)",
    "atlas_arch_gt": "M figure vs text — direct (where the figure alone fails)",

    # ================================================================ 1.1
    "filings_per_year": "Q1 indicator value — direct (eVTOL against all B64 filing)",
    "la_filings_per_year": "Q1 indicator value — caveat (which years are complete)",
    "atlas_arch_time": "Q2 what is gaining — direct (the class mix per window)",
    "a2_d9_architecture_by_window": "Q2 what is gaining — direct (the same, in numbers)",
    "firm_weighted": "Q7 one company or one design — direct (one vote per filer)",
    "la_firm_weighted_shares": "Q7 one company or one design — direct",
    "two_counts": "Q7 one company or one design — caveat (re-filing moves nothing)",
    "la_archetype_levels": "Q3 converging — premise (the level decides the answer)",
    "la_archetype_choice": "Q3 converging — premise (which levels can carry a species)",
    "la_dd_conditions": "Q3 converging — premise (the thresholds, fixed in advance)",
    "dominant_design": "Q3 converging — direct (conditions 1 and 2)",
    "dominant_design_q": "Q3 converging — direct (condition 3)",
    "la_dd_result": "Q3 converging — direct (the verdict: no dominant design)",
    "la_dominant_design": "Q3 converging — direct (top share and balance per window)",
    "la_dd_q": "Q3 converging — direct (the spread of the space per window)",
    "class_configs": "Q3 converging — direct (no settling inside a class either)",
    "la_class_configs": "Q3 converging — direct (restates the figure)",
    "dimension_drift": "Q4 what is designed — direct (units and booms rise, ducting flat)",
    "la_dimension_drift": "Q4 what is designed — direct (restates the figure)",
    "class_cycles": "Q2 what is gaining — direct (each class's lifetime)",
    "la_class_cycles": "Q2 what is gaining — direct (restates the figure)",
    "lead_lag": "Q6 where — caveat (the 2-year gap is the publication lag)",
    "la_lead_lag_region": "Q6 where — caveat (region ordering, small)",
    "la_lead_lag_class": "Q2 what is gaining — restates the class shares one window coarser",
    "la_class_region_timing": "Q6 where — direct (which region took a class up first)",
    "hill": "Q3 converging — direct, but no window separates from any other",
    "la_hill_by_window": "Q3 converging — direct, every band overlaps every other",
    "zones": "Q4 what is designed — direct (the crowded core and the empty rim)",
    "la_zones": "Q4 what is designed — direct (restates the figure)",
    "filer_weight": "Q7 one company or one design — direct (how much is one firm)",
    "la_class_filer_weight": "Q7 one company or one design — direct (effective filers per class)",
    "abandonment": "Q1 indicator value — direct (46 % no longer in force)",
    "la_abandonment_by_class": "Q1 indicator value — direct (the base rate, not the class)",

    # ================================================================ 1.2
    "coverage": "Q5 who designs it — direct (no top tier; 85 firms hold 47 %)",
    "la_market_vs_patents": "Q8 patents vs what is built — direct (rated firms file more)",
    "la_coverage_segments": "Q5 who designs it — direct (restates the curve)",
    "la_firm_tiers": "Q5 who designs it — direct (restates the curve)",
    "filers_over_time": "Q5 who designs it — direct (the field industrialises)",
    "la_filers_by_window": "Q5 who designs it — direct (entry dominates every window)",
    "la_cohorts": "Q5 who designs it — direct (firm cohorts)",
    "la_cohort_mix": "Q2 what is gaining — direct (the shift arrives with new firms)",
    "atlas_filers": "Q5 who designs it — direct (filing is spread, not concentrated)",
    "firm_tiles": "Q5 who designs it — direct (each firm's own window trace)",
    "firm_influence": "Q7 one company or one design — direct (leave-one-firm-out)",
    "la_firm_leverage": "Q7 one company or one design — direct (largest shift 2.6 pp)",
    "la_class_concentration": "Q7 one company or one design — direct",
    "la_filer_mix": "Q5 who designs it — direct (under half the corpus is a company)",
    "a2_d8_filer_mix": "Q5 who designs it — direct (the same split, per patent)",
    "a2_d8_concentration": "none — name-cleaning artefact, a data-quality note",
    "transitions": "Q5 who designs it — direct (firms deepen, they do not explore)",
    "la_transitions": "Q5 who designs it — direct, but 31 of 55 cells hold 1-2 pairs",
    "ip_strategy": "Q5 who designs it — direct (no single IP strategy)",
    "la_ip_strategy": "Q5 who designs it — direct (depth against breadth, 17 firms)",
    "la_ip_by_class": "none — citations follow age, not class; its own takeaway says so",
    "spans_by_firm": "Q7 one company or one design — caveat (re-filing changes nothing)",
    "proximity_region": "Q6 where — direct, and the answer is null (region does not organise)",
    "a2_d14_firm_proximity": "Q6 where — direct (per-firm proximity)",
    "la_proximity_region": "Q6 where — direct, null result on 136 pairs from 17 firms",
    "ari": "Q8 patents vs what is built — direct (the commercial end of the field)",
    "ari_history": "Q8 patents vs what is built — the index moves with funding, not filings",
    "ari_clock": "Q1 indicator value — direct (the patent clock leads the market clock)",
    "la_ari_firms": "Q8 patents vs what is built — direct (the 18 index firms)",
    "la_ari_correlation": "none — 57 tests on 9-18 firms, 0 survive the correction",
    "la_ari_gap": "Q8 patents vs what is built — direct (how the rated firms differ)",
    "la_ari_timeline": "Q1 indicator value — direct (years from first patent to first flight)",
    "la_ari_clock_region": "Q6 where — direct, but no row rests on more than 5 firms",
    "la_trl_placeholder": "Q8 patents vs what is built — nothing measured; the data exists",

    # ================================================================ 1.3
    "atlas_region": "Q6 where — direct (three countries hold two thirds)",
    "region_grid": "Q6 where — direct (North America keeps the tilting architectures)",
    "region_grid_b": "Q5 who designs it — direct, by region (powertrain and ducting left the figure)",
    "specialisation": "Q6 where — direct, readable only above ~10 aircraft per archetype",
    "la_specialisation": "Q6 where — direct (restates the figure)",
    "country_class": "Q6 where — direct (the US tilts, China lifts and cruises)",
    "la_country_class": "Q6 where — direct (restates the figure)",

    # ================================================================ 5
    "atlas_powertrain": "Q4 what is designed — restates the electric gate; 1.1.5 (v) has it per window",
    "la_powertrain_by_class": "Q4 what is designed — restates the figure",
    "atlas_units": "Q4 what is designed — direct (no standard rotor count; ducting is U-shaped in it)",
    "atlas_design_heatmaps": "Q4 what is designed — only class x tail type; the rest repeats the class",
    ...
    "linked_check": "Q8 patents vs what is built — caveat (the linked set is NOT the corpus)",
    "la_class_fold": "Q8 patents vs what is built — premise (the fold is lossy)",
    ...
    "la_mission_power source": "Q8 patents vs what is built — confirms the electric gate only",
    "la_mission_status": "Q8 patents vs what is built — direct (a patent describes a concept)",
    ...
    "examination": "Q1 indicator value — direct on the office panel; the class panel is null",
    "la_examination_office": "Q1 indicator value — direct (offices differ, designs do not)",
    "la_examination_class": "none — every class sits at the same grant rate",

    # ================================================================ 1.4 design drivers (2026-09-23)
    "trace_drift": "Q4 what is designed — direct (which traces move, inside which class)",
    "trace_couplings": "Q4 what is designed — direct (which predicted couplings the record carries)",
    "la_trace_couplings": "Q4 what is designed — direct (restates the figure with the percentiles)",
    "la_driver_verdicts": "Q4 what is designed — direct (what moved, and how many drivers predict it)",
    "la_driver_questions": "Q4 what is designed — direct (three answers, generated)",
}
...
# fourth grey line:
#
#     Question: Q2 what is gaining — direct (each class's lifetime) · weak, low
...
#      figure: a legitimate null answer to Q6). Those two keep their earned grade.
...
#                            carries. A row added there prints under "Q4 · design drivers" with no
...
#: The GROUPS, however, print in the order of :data:`QUESTIONS` (``la_tables.open_questions``
#: walks ``list(QUESTIONS)``), not in the order the blocks happen to sit in below — which is
#: why the ``# ===`` banners below still run Q4, Q3, Q2, Q5, Q6, Q8, Q1 after the renumbering
#: of 2026-09-23 and Appendix C still prints Q1 first.
OPEN_QUESTIONS: Dict[str, Dict] = {
    ...
    "noise_wind_as_drivers": dict(
        ...
        tried="nothing could be built for noise or wind as such — the record holds nothing to build one "
              ...
              "(1.4) reads the three traces the author's table gives community noise — propulsor count, "
    ...
    "entrants_bring_the_shift": dict(
        ...
        cannot="Only the two windows with 61 and 85 entrants can be read, and they are: the entrants "
               "of 2020-23 lead the field on Lift + Cruise by 2.6 points. Before 2016 the entering "
...
#: OPEN_QUESTIONS (``asked``, ``items``, ``tried``, ``cannot``, ``would``; ``q`` defaults to "Q4"
#: and ``sub`` to "design drivers"). Rows print under "Q4 · design drivers", after the two driver
...
UNLABELLED_DRIVERS: Dict[str, Dict] = {
    # Filled 2026-09-23 by the design-drivers section (1.4). One entry per trace the author's drivers
    ...
    "cabin_dimensions": dict(
        ...
        tried="nothing could be built for the dimension; the fuselage shape and the ground contact are "
              "labelled and read in 1.4, and the capacity stated on the directory page is in "
