...
def fig_lead_lag(v: pd.DataFrame, path: Path) -> Path:
    ...
    *is* Tables 4.1.7a/b — one row per group, the bar running from its 25 % year to its 75 %
    ...
    return _save(fig, path, SRC + "; complete priority years only (≤ 2023), so 2024-26 is excluded "
                 ...
                 "corpus; a cell of the matrix with fewer than 5 aircraft is printed “–”; Tables 4.1.7a/b/c. "
                 "NOT normalised against B64 aviation patenting, and it does not need to be: every group is "
                 "divided by its own total, and the stored baseline (4.1.1) has one series for all offices, "
...
def fig_hill(v: pd.DataFrame, path: Path) -> Path:
    """4.1.8 made self-explaining (author, 2026-09-23: "I don't really know the meaning of D1,
    ...
    return _save(fig, path, SRC + "; unit = unique aircraft; Table 4.1.8; A0 = the architecture class "
                 ...
                 "(Figure 4.1.1): the field grew hard and did not widen. Whether the early windows were already narrower "
                 "than chance is a different question, and 4.1.3.3 answers it against a null.")
...
def fig_filer_weight(v: pd.DataFrame, path: Path) -> Path:
    ...
    Nothing else in the document answers it. Table 4.1.9a counts *distinct filers* per archetype,
    which says how many there are and not how unevenly they divide it, and the one-vote-per-filer
    check of 4.1.2 was cut on 2026-09-22. So this figure carries the answer in two readings: how
    ...
    return _save(fig, path, SRC + "; unit = unique aircraft, so a firm that filed four patents on one "
                 ...
                 "numbers; a filer is a named firm, and an individual or unattributed patent is its own "
                 "filer, so no two lone inventors are merged; Table 4.1.9b, and per window "
                 ...
                 "Table 4.1.9b counts the same thing a third way: give every filer one vote instead of "
...
def fig_zones(v: pd.DataFrame, path: Path) -> Path:
    ...
    _legend_arch(fig, ARCH_ORDER + ["Other"])
    return _save(fig, path, SRC + "; A0c archetypes (class × propulsive-unit bin) with 5 or more aircraft; Table 4.1.9a.",
...
#: what each PatSeer legal status means, spelled out under Figure 4.1.10 (author, 2026-09-23:
...
# ------------------------------------------------------- 4.2 provenance ----
#: A percentage of a handful of aircraft is not a trend. Every cell of Figure 4.3.2 that holds
...
#: Rule: PatSeer's assignee string -> ``company_canonical``
#: (``Patent-Labelling-Tools/src/grouper.py::_normalise_company``, stored in
#: ``0_labelling/inputs/reference/family_map.csv``): first assignee only, trailing country code
#: stripped, lowercased; exact match against a hand-written table of ~110 canonical companies;
#: then rapidfuzz ``ratio`` >= 85 against that table's keys and against the canonical names; if
#: nothing matches, a string of 2-5 words with no digits and no organisation keyword becomes
#: "Individual Inventor" and everything else "Unknown / Independent". This document then splits the
#: matched companies into company and university with the regex :data:`la_tables.INSTITUTE_RE` over
#: the *canonical* name.
#: Audit, 2026-09-23, against the raw assignee string of the same 665 aircraft: person against
#: organisation is right 98 % of the time (637/651 readable strings); the three-way split is right
#: 75 % (499/665). The error is one-directional and known: "unattributed / independent" holds no
#: unattributed patent at all — all 138 carry a named assignee, 117 plainly a company, 13 a
#: university, 8 unreadable — it only means the string never matched the list, and universities
#: outside the list land there too (28 on the raw strings against 25 the rule finds).
FILER_RULE = ("filer type from company_canonical (grouper.py: first assignee, country code stripped, "
              "exact then fuzzy >= 85 against a hand-written list of ~110 companies; unmatched "
              "2-5-word strings become individual inventors, the rest 'unattributed'), split into "
              "company and university by the INSTITUTE_RE regex over the canonical name. Audited "
              "2026-09-23: person vs organisation right 98 %, the three-way split 75 %. 'Assignee "
              "not in the list' is the stored category 'unattributed / independent'; it is not "
              "unattributed — all of its aircraft name an assignee on the patent, most of them a "
              "company the list does not hold, so read that band as coverage of the list and never "
              "as independent filing")
...
def fig_region_grid(v: pd.DataFrame, path: Path, variables: Optional[List[str]] = None,
    ...
    # The fourth filer type is printed by what the rule actually tests, not by what it was named:
    # every one of its aircraft carries a named assignee on the patent — the string simply did not
    # match the company list. Audited 2026-09-23, see FILER_RULE. The other places in this document
    # still print the stored name "unattributed / independent"; the Source line reconciles the two.
    names = {True: "yes", False: "no", "blank": "not determinable", "Yes": "electric", "Hybrid": "hybrid",
             "Unknown": "not stated", "No": "non-electric",
             "Unattributed / independent": "assignee not in the list"}
    ...
    if ratio is not None and ratio.notna().any():
        ...
        norm += (f"The corpus as a whole went from {got.iloc[0]:.1f} patents per 1 000 B64 patents in "
                 ...
                 f"of every raw rise here is the general one and a part is not (Figure 4.1.1). ")
    return _save(fig, path,
...
def fig_coverage(v: pd.DataFrame, path: Path) -> Path:
    ...
    for i, (_, r) in enumerate(seg.iterrows()):
        ...
        mid = (x0 + x1) / 2
        ylab = 0.95 - 0.135 * (i % 2)         # the label goes in the empty upper half, staggered
        xlab = min(max(mid, 8.0), len(sizes) - 7)
        ax.plot([mid, xlab], [yb + 0.02, ylab - 0.005], color=GRID, lw=0.6, zorder=1)
        ax.text(xlab, ylab,
                f"{int(r['firms'])} firms · {int(r['aircraft'])} aircraft\n({r['aircraft per firm']} aircraft each)",
                ha="center", va="top", fontsize=7, color=INK, linespacing=1.2,
    ...
    return _save(fig, path, SRC + "; Table 4.2.1 and Table 4.2.7a; the segment counts of (i) are "
...
def fig_filers_over_time(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; Tables 4.2.2a/b; named firms = companies and institutes.",
                 ...
                 "AIRCRAFT (the same window's class mix, the quantity Figure 4.1.2 draws). Where the left bar runs "
...
def fig_firm_influence(v: pd.DataFrame, path: Path) -> Path:
    ...
    Replaces the firm tiles at 4.2.3 (author, 2026-09-23: "I don't really know what to take out
    ...
    cannot be one firm's portfolio. Together they answer the author's 4.1.9 question — how much of
    a class is one filer.
    """
    lev = la_tables.firm_leverage(v)
    conc = la_tables.class_concentration(v)
    top = lev.head(12).iloc[::-1]
    fig, (ax, ax2) = plt.subplots(1, 2, figsize=(W, 4.3), gridspec_kw={"width_ratios": [1, 1], "wspace": 0.52})
    y = np.arange(len(top))
    ...
    for yi, (_, r) in zip(y, top.iterrows()):
        ...
        ax.text(abs(r["shift if the firm is dropped (pp)"]) + 0.06, yi,
                f"{r['class most moved']} {r['shift if the firm is dropped (pp)']:+.1f}", va="center", fontsize=6.8,
                color=INK, zorder=5, bbox=dict(fc="white", ec="none", pad=0.2, alpha=0.9))
    ax.axvline(1.0, color=INK, lw=1, ls="--")
    ax.text(1.05, len(top) - 0.35, "1 pp", fontsize=6.8, color=INK2, va="top")
    ...
    for yi, (_, r) in zip(y2, cc.iterrows()):
        ax2.text(r["its share of the class"] + 0.008, yi - 0.19, f"{r['largest firm'].replace(' / ', '/')} "
                 f"{r['its share of the class']:.0%}", va="center", fontsize=6.4, color=INK)
        top3 = float(r["top 3 firms' share"])
        ax2.text(top3 + 0.008, yi + 0.19, f"{top3:.0%}", va="center", fontsize=6.4, color=INK2)
    ...
    ax2.spines[["left"]].set_visible(False)
    ax2.legend(fontsize=7, loc="center right", framealpha=0.9)
    _hgrid(ax2, "x")
...
def fig_proximity_region(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    """4.2.6 — the proximity matrix with the firms grouped in region blocks (user ruling 2026-09-23)."""
...
def fig_ari_history(v: pd.DataFrame, path: Path) -> Path:
    """4.2.7b — how each firm's score moved, and funding against patented aircraft.
    ...
    return _save(fig, path, ARI_SRC + "; Table 4.2.7a. " + str(len(disclosed)) + " of the " + str(len(fc))
...
def fig_ari_clock(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, ARI_SRC + "; corpus priority years; Tables 4.2.7d and 4.2.7e. "
                 ...
                 "is the years from a firm's first patent here to its first flight, and Table 4.2.7e groups that gap by "
...
def fig_mission(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, EVN_SRC + "; G1 class folded as Table 5.5a (VT vectored thrust, LC lift + cruise, WM wingless, "
                 ...
                 "publicly announced aircraft of the corpus, not a sample of it — Figure 5.5c tests how far they stand "
...
def fig_linked_check(v: pd.DataFrame, path: Path) -> Path:
    """Figure 5.5c: whether the aircraft linked to an evtol.news page stand for the whole set.

    Section 5.5 is a description of the linked aircraft; the author's ruling of 2026-09-23 is that
    ...
    return _save(
        ...
        + f" — {med}. The reason is in how a link is made: an aircraft can only be linked when the "
          ...
          "Section 5.5 therefore describes announced eVTOL programmes, and its class mix and its "
...
def fig_transitions(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + f"; each named firm's aircraft in priority order, every consecutive pair counted "
                 ...
                 "a class the field is leaving from a class the field never stays in — and read with Figure 4.2.2 it "
...
def fig_ip_strategy(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    """4.2.5a — depth against breadth, then claims and citations per class.
...
def fig_specialisation(v: pd.DataFrame, path: Path) -> Path:
    """4.3.3 as a coloured dot chart, one row per archetype, one dot per region.
...
#: ISO 3166-1 alpha-2 written out. The author read the country axis of Figure 4.3.1 and did not
...
def _post_region(fig, ds: Dataset) -> None:
    """Figure 4.3.1, left panel: spell the country codes out and print each country's share of the
...
#: the three classes whose propulsors move — the split Figure 5.4 turns out to measure
TILTING_CLASSES = ("TR", "CVT", "TW")


def _post_state_by_arch(fig, ds: Dataset) -> None:
    """Figure 5.4: say what the flight state is actually evidence of.
...
def fig_units(ds: Dataset, av: pd.DataFrame, path: Path, base_v: Optional[pd.DataFrame] = None) -> Path:
    ...
    Panel (iii) was added 2026-09-23. The author asked of Figure 4.3.2b whether ducting
    ...
    fig = plt.figure(figsize=(W, 8.5))
    gs = fig.add_gridspec(3, 1, height_ratios=[1.25, 1, 0.72], hspace=0.62)
    ax = fig.add_subplot(gs[0])
    ...
    # sides of it — the convention of Figure 4.3.2: a thin cell is suppressed visibly, never
    ...
    if np.isfinite(overall):
        ...
        ax3.text(len(xs) - 0.42, overall, f" every band together, {overall:.0%}", ha="left",
    ...
    if len(bands.dropna(subset=["share ducted"])) >= 3:
        ...
        p_band = dt.attrs.get("band", {}).get("p")
        p_reg = dt.attrs.get("region", {}).get("p")
        p_win = dt.attrs.get("window", {}).get("p")
        duct_take = (f"Ducting is not a regional or a period choice — it does not separate the regions "
                     f"(chi-square p = {p_reg:.2f}) and it does not move over the windows "
                     f"(p = {p_win:.2f}), which is why it is drawn here and not in Figure 4.3.2 — but it "
                     f"tracks rotor count closely (p = {p_band:.5f}): {bands.loc[lo, 'share ducted']:.0%} "
                     f"of the {lo}-unit aircraft duct something against "
                     f"{bands.loc[hi, 'share ducted']:.0%} of the {hi} ones, with the corpus at "
                     f"{overall:.0%}. Ducting is what the two ends of the range have in common and the "
                     f"middle does not: the small counts duct to make a few large rotors safe to sit "
                     f"beside, the large counts duct because a rotor that has become small enough to "
                     f"repeat nine or more times is also small enough for a ring to pay for itself. The "
                     f"middle of the range — four to eight open rotors on booms — is the arrangement "
                     f"that needs no ring at all. ")
        bgc = dt.attrs.get("band_given_class")
        if bgc and np.isfinite(bgc.get("p", float("nan"))):
            duct_take += (f"It is not the class mix in disguise: with the architecture class held "
                          f"fixed the band still moves the duct answer (likelihood ratio "
                          f"{bgc['chi2']:.1f} on {bgc['dof']} df, p = {bgc['p']:.4f}, "
                          f"{bgc['aircraft']} aircraft). ")
        if len(cls) >= 2:
            c_hi, c_lo = cls["share ducted"].idxmax(), cls["share ducted"].idxmin()
            duct_take += (f"The same variable also splits the classes (panel (ii), duct row): "
                          f"{cls.loc[c_hi, 'class']} ducts {cls.loc[c_hi, 'share ducted']:.0%} of "
                          f"{int(cls.loc[c_hi, 'aircraft'])} aircraft against "
                          f"{cls.loc[c_lo, 'share ducted']:.0%} of "
    ...
    return _save(fig, path, SRC + "; propulsive_units, propulsion states (rules of 2026-09-19); class codes as "
                 "Figure 2.1a; the band shares of (iii) are tables/la_duct_units.csv, with the chi-square "
                 "tests behind them.",
                 why_median +
                 ...
                 "a tilting boom is read from the boom tick. Bottom: the same duct answer against the number of "
                 "propulsive units. Read the shape of the line first, then the level: each point is the share of "
                 f"that band's AIRCRAFT carrying at least one ducted unit — not a share of rotors, so an aircraft "
                 f"with one ducted fan among twelve open rotors counts once, exactly like one that ducts all "
                 f"twelve — and the dashed line is the same share over every band together. The n under each band "
                 f"is its base, and every aircraft of the five bands answers the duct question, so nothing is "
                 f"hidden behind a missing value; the {int(dt[dt['group'].eq(la_tables.NO_UNIT_CARD)]['no duct answer'].sum())} "
                 f"aircraft with no propulsor card and the one recorded with 0 units have no duct answer and form "
                 f"no band. A band resting on fewer than {la_tables.DUCT_THIN} aircraft would be drawn as a hollow "
                 f"marker with its own n and the line would break either side of it; no band is that thin here. "
                 + take + duct_take)
...
def render_all(ds: Dataset, out_dir: Path, partial_window_start: int = 2024, **_) -> Dict[str, Path]:
    ...
    jobs = {
        ...
        # 2026-09-23, author's ruling: Figure 4.3.2b keeps filer type alone. Powertrain and ducting
        # left the pair for the sections that own them (5.1 and 5.2) — the reasons are written out
        # beside the parked entries of la_tables.GRID_VARIABLES. Kept as a figure of its own rather
        # than merged into 4.3.2a for a mechanical reason: fig_region_grid is 2.45 in per variable
        # row, so a fourth row puts Figure 4.3.2a at roughly 13.5 in of drawn height against the
        ...
        # parked 2026-09-23 (author's ruling on 4.2.3): replaced at 4.2.3 by "firm_influence", which
