...
TABLE_CAPTIONS: Dict[str, str] = {
    **_pa.TABLE_CAPTIONS,
    ...
    "la_name_arch_check": (
        ...
        "agrees; only the disagreements are listed. The directory works in five classes, so the G1 and "
        "whole-patent codes carry their fold where a public class exists"),
...
PROVENANCE: Dict[str, Dict[str, str]] = {
    ...
    "la_name_arch_check": dict(
        ...
        base="the aircraft where at least two of the three sources give a class; the first row "
             "collapses every aircraft whose sources agree, and only the disagreements are listed",
    ...
    "filings_per_year": dict(
        unit="two units on one pair of axes — unique aircraft (n {unique_s}) in the bars, acquired "
             "patents (n {acquired_s}) in the line",
        base="each priority year on its own; an aircraft sits in the year of its primary record",
        transform="raw counts per year, NOT normalised against overall patenting — a rise here may be "
                  "a rise in patenting at large, and the acquired-patent line is the only baseline on "
                  "the figure; hatched years are still incomplete at the snapshot"),
    "la_filings_per_year": dict(
        unit="two units in the same table — unique aircraft (n {unique_s}) by region, and acquired "
             "patents (n {acquired_s})",
        base="each priority year on its own",
        transform="raw counts per year, NOT normalised against overall patenting; `complete` is true "
                  "when the snapshot lies at least the 90th-percentile publication lag after the end "
                  "of the year"),
