...
def fig_filings_per_year(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    if normalised:
        ...
        if ratio_col in ti.columns:
            ...
            ax2.plot(done.index, done[ratio_col], color=INK, lw=2, marker="o", ms=4.0)
            first_yr, last_c = int(done.index.min()), int(done.index.max())
            first_val, last_val = float(done.loc[first_yr, ratio_col]), float(done.loc[last_c, ratio_col])
            ax2.annotate(f"{first_yr}: {first_val:.1f} per 1 000", (first_yr, first_val),
                         textcoords="offset points", xytext=(2, 9), fontsize=7.5, color=INK2)
            ax2.annotate(f"{last_c}: {last_val:.1f} per 1 000", (last_c, last_val),
                         textcoords="offset points", xytext=(-4, -13), ha="right", fontsize=7.5, color=INK2)
            ax2.set_ylim(0)
        else:
        ...
        if ratio_col in ti.columns:
            ax2.set_ylabel("eVTOL patents per 1 000 B64 patents (same offices)")
            ax2.set_title("The corpus's share of aeronautics patenting, by priority year — "
                          "eVTOL patents per 1 000 B64 patents of the same offices", fontsize=9.5)
        else:
        ...
        if ratio_col in ti.columns:
            ...
            read += (f" (ii) asks whether the rise is the sector's own: aviation patenting also grew, so (ii) "
                     f"divides the corpus's own patent count by all B64 patents of the same nine offices and "
                     f"priority year, per 1 000. The ratio rises from {r.get('ratio_start')} per 1 000 in the "
                     f"{lo}-{hi} base window to {r.get('ratio_end')} by {last}, {mult:g}× higher, and has stayed "
                     f"above the base-window level since {gained} — so the rise is not the tide. The hatched "
                     f"years are incomplete on both sides and the ratio there would read as a rise that is not "
                     f"real, so they are not drawn; the baseline covers only the nine publication offices the "
                     f"corpus draws on. (ii) does not pool: its first point is {FILINGS_FIRST_YEAR} alone, "
                     f"because a ratio of single years cannot take a seven-year bucket as a point.")
        else:
