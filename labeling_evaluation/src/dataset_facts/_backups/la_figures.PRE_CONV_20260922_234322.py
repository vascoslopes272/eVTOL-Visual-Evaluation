...
def fig_dominant_design(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; Table 4.1.3; rarefied to 40 aircraft; 200 permutations; Q (condition 3) needs the Gower distance and is not yet computed.",
...
def fig_class_configs(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; Table 4.1.4 names the modal configuration per class and window; windows with fewer than 5 aircraft of the class blank.",
...
def fig_dimension_drift(v: pd.DataFrame, path: Path) -> Path:
    ...
    fig.suptitle("Dimension drift inside the four largest classes, per window", x=0.01, y=1.06, ha="left",
                 fontsize=10, fontweight="bold")
    return _save(fig, path, SRC + "; Table 4.1.5; windows with fewer than 5 aircraft of the class left out.",
...
def _atlas(fn: Callable, ds: Dataset, av: pd.DataFrame, n: Dict, path: Path, size: tuple = (W, 5.0),
    ...
    try:
        with plt.rc_context({**STYLE, "font.size": 8.5, "axes.titlesize": 9.5, "legend.fontsize": 7.5}):
            ...
            stem = Path(path).stem
            if stem in ATLAS_DROP:           # graphs this document does not print (the atlas keeps them)
                ...
                if note:                     # the shared note describes both graphs: replace it here
                    fig._atlas_note = tuple(s.format(src=atlas.TABLES_SRC, aircraft=len(av),
                                                     ...
                                                     gt=len(atlas._ground_truth(ds))) for s in note)
