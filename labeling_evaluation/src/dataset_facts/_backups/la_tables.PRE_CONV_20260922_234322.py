...
def name_arch_check(v: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in w[hit].iterrows():
        ...
        rows.append({
            ...
            "public class": f"{FOLD5_NAMES.get(r['pub'], r['pub'])} ({r['pub']})" if has_pub else "—",
    ...
    head = pd.DataFrame([{
        "aircraft": f"{agree} of {len(w)} aircraft — every source available agrees, not listed",
        "company": "—", "G1 class": "—", "whole-patent class": "—",
        "public class": f"{int(w['pub'].notna().sum())} aircraft carry one",
        "sources that disagree": f"none ({blind} aircraft a source does not classify)",
    }])
