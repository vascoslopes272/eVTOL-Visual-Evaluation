...
def base(ds: Dataset) -> pd.DataFrame:
    ...
    # archetype key is built, so every rendered A0c key follows -- 2.1.3, 2.1.6, 2.1.9 and the
...
GRID_VARIABLES = {
    ...
    # category silently drops out of Figure 2.3.2a
    "propulsive units": ("rotorBin", ["1-3", "4", "5-6", "7-8", "9+"]),
    "tilting unit": ("any_tilting", [True, False]),
    # Parked 2026-09-23, author's ruling on Figure 2.3.2b ("why do I need to talk about powertrain?
    ...
    #                 a real finding and is made in 3.1; against region it is a measurement artefact.
    ...
    #                 the propulsive-unit bands (p = 0.00012), so it is drawn in 3.2 instead, where
...
def firm_frame(v: pd.DataFrame) -> pd.DataFrame:
    ...
    of 2.2.1 are taken on. Index is the canonical company."""
    w = v[v["named"]]
...
def firm_leverage(v: pd.DataFrame, min_aircraft: int = 3) -> pd.DataFrame:
    ...
    2.1.6 and 2.2.3. Done on the whole set and again inside the two windows that carry most of the
...
def cohort_mix(v: pd.DataFrame, printed: bool = True) -> pd.DataFrame:
    """The normalised form of the entry cohorts (author's ruling 2026-09-23: 2.2.2 must be
    ...
    The two shares are what makes the section say something 2.1.2 does not: 2.1.2 counts aircraft,
...
def ari_listed(v: pd.DataFrame) -> set:
    ...
    flag that 2.2.1 compares the other named firms against."""
    _, _, cmap = _load_ari()
...
#: What Spearman's rho is, printed under Table 2.2.7b. Kept here so the table, the figure and the
...
def ari_gap(ds: Dataset, v: pd.DataFrame) -> pd.DataFrame:
    ...
    Eighteen firms is too few to correlate anything reliably (Table 2.2.7b), but those eighteen firms
...
def linked_representativeness(v: pd.DataFrame) -> pd.DataFrame:
    ...
    Section 3.5 describes the linked aircraft only, and the author's ruling of 2026-09-23 is that
...
# ------------------------------------------------------- 2.1 new: dominant design, configs, drift
...
# ------------------------------------------- 2.1.3.1 what the test evaluates: the archetypes
...
ARCHETYPE_ROLE: Dict[str, str] = {
    "A0": "the reference level: every per-class reading (2.1.2, 2.1.4, 2.1.5, 2.1.7, 2.1.8, 2.1.9 (i), 2.1.10, 2.3.4, 3.1)",
    "A0c": "co-reported second level of 2.1.3.3, conditions 1 to 3, and the only level at which condition 2 fires; 2.1.9 (ii) diversity; 2.1.6 zones; 2.3.3 regional specialisation",
    ...
    "A1t": "the design species: 2.1.3.3 conditions 1 to 3, and the level Preliminary Analysis 5.1 and 5.7 are written at",
    "A1tc": "not read here",
    "A2": "not read here; 2.1.4 reads the same fields inside one class, as a configuration",
    "A2c": "not read here; 2.1.4 reads the same fields inside one class, as a configuration",
}
...
#: aircraft are mostly alone cannot carry a share, and the five-aircraft line is the one 2.1.6
...
#: species, A0c is reported beside it at every step of 2.1.3 and is not a robustness run.
...
# ------------------------------------------- 2.1.3.2 / 2.1.3.3 the conditions and the result
...
# ---------------------------------------- 2.1.4 / 2.1.5: which classes can be drawn ----
#: A class earns a line in 2.1.4 and 2.1.5 when it can carry one: at least ``MIN_CELL``
...
def class_selection(v: pd.DataFrame, min_cell: int = MIN_CELL,
    ...
    """The classes 2.1.4 and 2.1.5 draw, and the numbers that justify the selection.
...
def drawn_classes(v: pd.DataFrame, min_cell: int = MIN_CELL, min_windows: int = MIN_WINDOWS) -> List[str]:
    """The class codes 2.1.4 and 2.1.5 draw, largest first (see :func:`class_selection`)."""
...
#: across it rather than leaving it out (Figure 3.2 (iii)).
DUCT_THIN = 20
...
def duct_by_units(v: pd.DataFrame) -> pd.DataFrame:
    ...
    and by architecture class — the frame behind Figure 3.2 (iii).
    ...
    two negatives (region and window) that moved this variable out of Figure 2.3.2b.
    """
    w = v.copy()
    w["band"] = unit_band(w)
    duct = w["any_ducted"]
...
def dimension_drift(v: pd.DataFrame, classes: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    configuration defined in 2.1.4, and by powertrain.
...
# ------------------------------------------------------- 2.4 design drivers and their traces
...
#: marks the four traces Figure 2.1.5 already draws: they are computed here by the same formula
...
def trace_frame(ds: Dataset, v: pd.DataFrame) -> pd.DataFrame:
    ...
    type by type as :func:`a2.propulsion_states` reads them. The four Figure 2.1.5 traces are the
    ...
    # the four Figure 2.1.5 traces, by the formulas of dimension_drift
...
def trace_drift(ds: Dataset, v: pd.DataFrame, classes: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    rests on — the frame behind Figure 2.4.1. Same rules as :func:`dimension_drift`: the drawn
...
def trace_trends(ds: Dataset, v: pd.DataFrame, classes: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    classes with the class shift of 2.1.2 and is read as such.
    """
    from scipy import stats as _st
    classes = classes or drawn_classes(v)
    t = trace_frame(ds, v).dropna(subset=["window", "topType"])
    ...
    for tr in TRACES:
        ...
        if direction == "0":
            ...
            verdict = ("moves, but only fixed physical drivers predict this trace — they predict a coupling, not a "
                       "trend, and are read in the coupling test (2.4.2); the movement is not a driver trace the "
                       "frame can read")
        ...
        rows.append({"trace": tr["label"] + ("" if tr["kind"] == "share" else " (median)"),
                     ...
                     "where drawn": ("Figure 2.1.5" if tr.get("from_215") else "Figure 2.4.1" if tr.get("drawn") else "tables only"),
...
def driver_questions(ds: Dataset, v: pd.DataFrame, trends: Optional[pd.DataFrame] = None) -> pd.DataFrame:
    ...
    q3 = ("Inside the tilting classes the joint count rises — " + "; ".join(cell("tilting joint groups", c, "median") for c in j)
          ...
          f"class growing fastest (Lift + Cruise, 2.1.2) has none. The opposed pair therefore splits by level — transition wins inside a "
...
# ------------------------------------------------------- 2.2 new: transitions, IP, cohorts, timeline
...
def ip_strategy(ds: Dataset, v: pd.DataFrame, min_aircraft: int = FIRM_MIN) -> pd.DataFrame:
    ...
    which is what the bubbles of Figure 2.2.5a are sized by, and the age-normalised median cohort rank
...
def ip_by_class(v: pd.DataFrame, ds: Optional[Dataset] = None) -> pd.DataFrame:
    ...
    *Forward citations* are printed as a **mean and a median** because their distribution is very
    ...
    total attention the class has drawn (and is what the bubbles of Figure 2.2.5a are sized by); the
...
#: Where the two market dates come from, printed under Figure 2.2.7c and Tables 2.2.7d/e.
ARI_DATE_SRC = (
...
def flags() -> pd.DataFrame:
    ...
    rows = [
        # Numbers here follow the reorganisation of 2026-09-23 (findings first, construction and
        # quality as Appendix A and Appendix B); the "section" column is the section as it is now
        ...
        ("Figure 1.1a the four cards", "1", "repeats Figure 1.1b"),
        ...
        # 2026-09-22: the two robustness figures of 2.1.2 and its table were cut on the author's ruling,
        # so they are no longer candidates. Their builders are kept (fig_firm_weighted, fig_two_counts).
        # 2026-09-23: the table of 2.1.7 was cut on the author's ruling ("basically the same thing as
        # the figure"), so it is no longer a candidate; class_cycles() still writes the CSV. 2.1.8 comes
        # off this list at the same time: its class panel was the part that repeated 2.1.7, and it
        # has been replaced by the class × region matrix, which nothing else in the document shows.
        ("2.1.9 diversity per window, figure and table", "2.1.9",
         ...
         "reported in 2.1.3.3, against a null instead of a sampling band"),
        ("Table 2.1.6a zones", "2.1.6", "20 rows; the figure carries it"),
        # 2026-09-23, author's ruling on 2.2.1: the firm-tier table was cut once the curve started
        # printing the firms and the aircraft of each segment. firm_tiers() still writes the CSV.
        # 2026-09-23, 2.2.3: the firm tiles were replaced by the firm-influence figure; fig_firm_tiles
        # is kept and parked in la_figures.render_all.
        ("2.2.2 the body of filers over time, figure and both tables", "2.2.2",
         "DECIDE (2026-09-23): the section was kept and normalised rather than cut. Panel (i) and "
         "Table 2.2.2a are firm demographics that nothing else shows; panel (ii) and Table 2.2.2b "
         "now compare the class mix of the entering FIRMS with the class mix of the window's "
         "AIRCRAFT, which is the one claim 2.1.2 cannot make. If the section still has to go, the "
         "part to keep is panel (ii) — it could be folded into 2.2.4, whose successions answer the "
         "same question from the other side"),
        ("Figure 2.2.5b re-filing spans", "2.2.5", "its one useful number is in the IP figure"),
        ...
        # The figure now groups the firms in region blocks and Table 2.2.6 gives the region-pair means.
        # 2026-09-23, author's ruling: the TABLE of 2.3.4 is cut, and so are the tables of 2.3.3, 3.1
        # and the industry-by-class table of 3.5. Only the figure of 2.3.4 is still a candidate, so the
        # row says so.
        ("Figure 2.3.4 country × class", "2.3.4", "the specialisation index (2.3.3) answers it at archetype level; its table was cut on 2026-09-23"),
        ("3.4 image-level answers", "3.4", "T2 detail; keep only if the image chapter needs it here"),
        ("Tables 3.5b–f mission crosstabs", "3.5", "five small tables the mission figure already shows"),
    ]
...
def build_all(ds: Dataset) -> Dict[str, pd.DataFrame]:
    ...
    # written to tables/ and read by Figure 2.1.6b; not placed in a node of its own
    ...
    # parked 2026-09-23 (author's ruling on 2.2.1: the curve now carries the counts, so the tier
    # table is not printed). Still built, so the numbers behind the curve stay checkable in
    # tables/la_firm_tiers.csv; put the name back in node 2.2.1 of la_index.NODES to print it.
    ...
    # written to tables/ and read by Figure 3.5c; not placed in a node of its own
    ...
    # 2.4 design drivers and their traces (2026-09-23). la_trace_drift and la_trace_trends are
    # written to tables/ and read by Figure 2.4.1 and by the verdict table; they are not placed in
    ...
    # markdown table (the author's complaint about the table of 2.2.4, which he called "table 4.2.4"
    # before the 2026-09-23 renumbering, is about the figure, which is the
    # matrix), while the takeaway of Figure 2.2.4 reads its pair counts out of this frame and the CSV
    # keeps them checkable. To print it, add "la_transitions" to node 2.2.4 in la_index.NODES.
