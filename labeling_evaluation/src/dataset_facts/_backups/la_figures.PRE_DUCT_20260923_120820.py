...
def fig_units(ds: Dataset, av: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; propulsive_units, propulsion states (rules of 2026-09-19); class codes as Figure 2.1a.",
                 why_median +
                 ...
                 "dropped. HB and PFV have no propulsor card and are left out, never counted as 0. Bottom: share of "
                 "each class with the feature, over the aircraft where it can be read (read / total under the code); "
                 "a tilting boom is read from the boom tick. " + take)
...
def render_all(ds: Dataset, out_dir: Path, partial_window_start: int = 2024, **_) -> Dict[str, Path]:
    ...
    jobs = {
        ...
        "region_grid_b": lambda p: fig_region_grid(v, p, ["ducted unit", "powertrain", "filer type"], ds),
