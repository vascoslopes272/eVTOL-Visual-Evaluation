...
def fig_codebook_dimensions(ds, v, n):
    ...
    _note(fig, "the labelling codebook, cards G1 to M3 (drawing, not data).",
          "A tick box is a flag, # a number, \"if\" a question asked only when its condition holds, "
...
def fig_fields(ds, v, n):
    ...
    ax.legend(loc="upper left", ncol=2, title="the question the column answers", title_fontsize=8.5)
    ...
    ax2.invert_yaxis()
    ax2.set_xlabel(f"questions answered per aircraft (of {n['questions']})")
    ax2.set_title("Label depth by architecture")
    _hgrid(ax2)
    _note(fig, f"{TABLES_SRC}; the questions are the rows of the dimension register "
               f"(assets/codebook/dimension_register.csv, the drawing of Figure 3.3b); {len(per)} unique aircraft.",
          f"Left: one dot per export column; a question repeated per boom group, wing panel, propulsor host or "
          f"propulsor type gives one dot per repeat. Right: how many of the {n['questions']} questions each "
          f"aircraft answers; a question is answered when any of its columns holds a value (an unticked box "
          f"on a card that exists is an answer).")
    return fig, "3.3.5", "Which columns carry information, and how many questions each aircraft answers"
