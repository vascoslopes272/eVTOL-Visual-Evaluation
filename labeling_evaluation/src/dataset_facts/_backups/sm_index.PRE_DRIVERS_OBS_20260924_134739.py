...
OLD_NODES: List[Dict] = [
    ...
    dict(id="Q1",
         text="**The unit, stated once for the whole document.** {acquired_s} patents were acquired "
              ...
              "**Out of force** = the PatSeer legal status of the aircraft's primary patent reads "
    ...
    dict(id="drivers", title="What moved inside the classes, and which drivers predict it",
         text="A design driver — the technology that moved, the physics that did not, a requirement, "
              ...
              "class mix moves on its own. **Medians, not means**, because units per aircraft is "
              "skewed — a few 16- and 20-rotor designs would drag a mean; a median is the middle "
              "aircraft.\n\n"
              "**Observations.** (1) *Tilting joint groups* — the number of propulsor groups that "
              "tilt — rise inside Tilt Rotor (TR {tk_dr_tr_joints_mov}) and CVT (CVT "
              "{tk_dr_cvt_joints_mov}); the pooled line is {tk_dr_all_joints_mov} only because Lift + "
              "Cruise, the class that grows, has no joints. Inside the tilting classes the joint "
              "count rises: transition wins inside a class, cost wins in the mix. (2) *Propulsive "
              "units*: the median rises in TR ({tk_dr_tr_units_first} → {tk_dr_tr_units_last}) and "
              "CVT ({tk_dr_cvt_units_first} → {tk_dr_cvt_units_last}) and stays at "
              "{tk_dr_slc_units_first} in Lift + Cruise — the tilting classes are adopting distributed "
              "propulsion late, where Lift + Cruise was born with it; three drivers predict the rise "
              "(distributed control, failure tolerance, community noise) and the record cannot "
              "separate them. (3) *A ducted unit* moved AGAINST the one driver that predicts it: "
              "noise predicts more ducting, and inside CVT ducting falls from {sm_dr_cvt_duct_first} "
              "to {sm_dr_cvt_duct_last}. Stated, not explained: a duct buys noise and hover thrust "
              "at the price of weight and cruise drag, and CVT is the class that cruises on its "
              "lift units, so the trade may go the other way there — to be checked against the "
              "patents' own text, not asserted. (4) *Propulsor types* = distinct kinds of "
              "propulsor on one aircraft, read from the M3 groups: a set of lift rotors and one "
              "cruise propeller is 2, one tilting set is 1. It rises in TR ({sm_dr_tr_types_first} "
              "→ {sm_dr_tr_types_last}): the tilt-rotor is acquiring a second, non-tilting set — "
              "it is moving toward CVT, which is the same reading as (1). (5) Of the couplings the "
              "fixed physical drivers predict, one exists — empennage type against wing "
              "configuration (V {tk_dr_emp_wing_v}); the others are no stronger than a random pair.",
         figures=["dimension_drift"],
...
FOLD = {
    "Q1": ("1", 1, "The record itself: how early, how complete, how live"),
    "Q8": ("1", 2, "The record against what the industry actually builds"),
