...
def _bw_table():
    ...
    _BW[to_hex(OTHER)] = ("oo", (0, (3, 1, 1, 1)), "h")
    return _BW
        ...
        for a in arts:
            if isinstance(a, Rectangle) and a.get_width() and a.get_height():
                ...
                if fc[3] == 0:
                    continue
                key = to_hex(fc[:3])
                if key in tab and tab[key][0] and not a.get_hatch():
                ...
                if len(fcs):
                    ...
                    if len(keys) == 1:
                        ...
                        if key in tab and tab[key][0]:
                            ...
                            a.set_edgecolor(ink)
                            a.set_linewidth(0.4)
            elif isinstance(a, Line2D) and len(a.get_xdata()) >= 2:
                key = to_hex(a.get_color())
                if key in tab and a.get_linestyle() == "-" and not getattr(fig, "_bw_keep_dash", False):
                    a.set_linestyle(tab[key][1])
                if key in tab and a.get_marker() not in ("None", None, "", " ", "|"):
...
def fig_spans_by_firm(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    ax.set_xticks(ticks, labels, rotation=55, ha="right", fontsize=7.5)
    ax.set_xlim(-1, x - 0.5)
    ...
    for xi, (_, r) in zip(xx, g.iterrows()):
        ax2.text(xi, r["share"] + 0.02, f"{int(r['refiled'])}/{int(r['aircraft'])}", ha="center", va="bottom", fontsize=7)
    ax2.set_xticks(xx, list(g.index), rotation=55, ha="right", fontsize=7.5)
    ax2.set_ylim(0, 1.1)
            ...
            if sub is not None:
                for col in sub.columns:
                    ...
                    for xi, (b, val) in enumerate(zip(bottom, vals)):
                        if val >= 0.15:
                            ax.text(xi, b + val / 2, f"{val:.0%}", ha="center", va="center", fontsize=7, color="white", fontweight="bold")
                    bottom += vals
...
def fig_filers_over_time(v: pd.DataFrame, path: Path) -> Path:
    ...
    ax.set_xticks(x, [f"{WIN_SHORT[w]}\nn {int(r['unique aircraft'])}\nind. {r['share individual inventors']:.0%}"
                      f"\nunattr. {r['share unattributed']:.0%}" for w, (_, r) in zip(WINDOW_NAMES, t.iterrows())],
                  fontsize=7)
    ...
    return _save(fig, path, SRC + "; Tables 4.2.2a/b; named firms = companies and institutes.",
                 ...
                 "window: its aircraft and the share filed by individual inventors and by unattributed filers. Right: the "
                 "firms that enter in each window, coloured by the class of their first aircraft.")
def fig_firm_tiles(v: pd.DataFrame, path: Path) -> Path:
    ...
    ax.set_title("Firms with 5 or more aircraft: aircraft per window (size, number) and the class filed most (colour)",
                 fontsize=9)
    ax.spines[["left", "bottom"]].set_visible(False)
    _hgrid(ax, "both")
    _legend_arch(fig, ARCH_ORDER + ["Other"])
...
def fig_proximity_region(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    for i in range(len(m)):
        for j in range(len(m)):
            val = float(m.iat[i, j])
            if i != j and val >= 0.5:
                ax.text(j, i, "1" if val >= 0.995 else f"{val:.2f}"[1:], ha="center", va="center", fontsize=7,
    ...
    fig.legend(handles=handles, loc="upper center", ncol=3, bbox_to_anchor=(0.5, 0.02), fontsize=7.5)
...
def fig_mission(v: pd.DataFrame, path: Path) -> Path:
    ...
    gs = fig.add_gridspec(3, 2, height_ratios=[1, 1, 1.05], hspace=1.05, wspace=0.55)
    ...
    for k, field in enumerate(fields):
        ...
        _stack_h(ax, share, pal, min_label=0.16)
        for t_ in ax.texts:
            t_.set_fontsize(7)
        ax.set_title(field, fontsize=9)
        ax.tick_params(axis="both", labelsize=7.5)
        ax.legend(fontsize=7, loc="upper center", bbox_to_anchor=(0.5, -0.16), ncol=3, handlelength=1.2, columnspacing=0.8)
    ...
    return _save(fig, path, EVN_SRC + "; G1 class folded as Table 5.5a (VT vectored thrust, LC lift + cruise, WM wingless, "
                 ...
                 "Four panels: each bar is 100 % of the linked aircraft of one folded class. Bottom: how often the folded "
...
def fig_filings_per_year(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    for xi, (_, r) in zip(t.index, t.iterrows()):
        if not r["complete"]:
            ...
            top = max(r["unique aircraft"], r["patents acquired"] if abs(r["patents acquired"] - r["unique aircraft"]) < 8 else 0)
            ax.text(xi, top + 3, f"{int(r['unique aircraft'])}", ha="center", fontsize=7, color=INK2)
...
def fig_ip_strategy(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    fig, (ax, ax2) = plt.subplots(1, 2, figsize=(W, 4.3), gridspec_kw={"width_ratios": [1.25, 1], "wspace": 0.35})
    ...
    _hgrid(ax, "both")
    handles = [matplotlib.patches.Patch(color=REGION_COLOR[r], label=r) for r in REGIONS]
    ax.legend(handles=handles, fontsize=7, loc="upper center", bbox_to_anchor=(0.5, -0.2), ncol=3)
    ax.set_xlim(4, 160)
    ...
    ax2.legend(fontsize=7, loc="upper center", bbox_to_anchor=(0.5, -0.08), ncol=2)
...
def fig_c1_removal(ds: Dataset, path: Path) -> Path:
    ...
    ax2.set_xticks(xx, list(tab.index), fontsize=7.5)
...
def _atlas(fn: Callable, ds: Dataset, av: pd.DataFrame, n: Dict, path: Path, size: tuple = (W, 5.0)) -> Path:
    ...
    atlas._stack_h = lambda ax, frame, colors, min_label=0.06, pct=True: stack(ax, frame, colors, max(min_label, 0.13), pct)
    try:
