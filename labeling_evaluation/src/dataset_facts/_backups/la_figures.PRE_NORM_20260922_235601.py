...
def _selection_line(t) -> str:
    ...
    a = t.attrs
    return (f"Classes drawn — the rule, not a size ranking: every class holding at least {a['min_cell']} unique "
            f"aircraft in at least {a['min_windows']} of the five windows, which is {', '.join(a['names'])}, "
            f"{a['aircraft']} of the {a['classified']} classified aircraft ({a['share']:.0%}). A hollow marker rests on "
            f"{a['min_cell']}–{THIN_CELL - 1} aircraft; a gap is a window under {a['min_cell']}, left blank, never joined up.")
...
def fig_dimension_drift(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; Table 4.1.5; windows with fewer than 5 aircraft of the class are left out.",
                 "A line is an architecture class (the G1 top type), not an archetype; every point counts unique aircraft. "
...
ATLAS_NOTE: Dict[str, tuple] = {
    ...
    "atlas_arch_gt": (
        ...
        "figures alone, over the {gt} aircraft that carry both.",
