...
def proximity_by_region(ds: Dataset, v: pd.DataFrame, min_aircraft: int = FIRM_MIN,
    ...
    summary = (p.groupby("pair")["proximity"]
               .agg(**{"firm pairs": "size", "mean proximity": "mean", "median proximity": "median",
                       "lowest pair": "min", "highest pair": "max"}).round(2))
    ...
    for x in xs:
        for y in ys:
            ...
            if len(d) >= 6 and d[x].nunique() > 2 and d[y].nunique() > 2:
                rho, p = spearmanr(d[x], d[y])
                rows.append({"index side (x)": x, "patent side (y)": y, "n firms": int(len(d)),
...
ARI_GAP_VARS = [("cohort citation rank", "cohort citation rank", "number"),
                ("no longer in force", "lapsed", "share"),
                ("propulsive units per aircraft", "units", "number"),
                ("has booms", "_boomed", "share"),
                ("has a ducted unit", "any_ducted", "share"),
                ("has a tilting unit", "any_tilting", "share"),
                ("has a wing", "_winged", "share"),
                ("claims", "claim_count", "number"),
                ("family size", "family_size", "number"),
                ("priority year", "year", "number"),
                ("architecture class (12)", "topType", "mix"),
                ("architecture class (5 folded)", "class5", "mix"),
                ("applicant region", "region3", "mix")]
...
def ari_gap(ds: Dataset, v: pd.DataFrame) -> pd.DataFrame:
    ...
    def one(sub: pd.DataFrame, col: str, kind: str):
        ...
        if t.shape[0] != 2 or t.shape[1] < 2 or (t.to_numpy() < 5).mean() > 0.4:
            return None
        return np.nan, np.nan, float(chi2_contingency(t)[1])

    us = w[w["pub_office"].eq("US")]
    rows = []
    for label, col, kind in ARI_GAP_VARS:
        r = one(w, col, kind)
        if r is None:
            continue
        ru = one(us, col, kind)
        fmt = (lambda z: "" if not np.isfinite(z) else (f"{z:.0%}" if kind == "share" else f"{z:g}"))
    ...
    out["holds?"] = np.where(out["q (BH)"] >= 0.05, "no",
                             np.where(out["p, US patents only"].fillna(1) < 0.05,
                                      "yes", "office effect"))
    out["p"] = out["p"].round(4)
    out["p, US patents only"] = out["p, US patents only"].round(4)
    out["q (BH)"] = out["q (BH)"].round(4)
