...
from . import a2, la_baseline, metrics
        ...
        for name in WINDOW_NAMES:
            ...
            if len(sub) < MIN_CELL:
                rows.append({"class": metrics.ARCH_NAMES.get(c, c), "code": c, "window": name,
                             ...
                             "share with a ducted unit": np.nan, "share with a tilting unit": np.nan,
                             "share with booms": np.nan, "median wings": np.nan,
                             "propulsive units answered": 0, "ducted answered": 0,
                             "tilting answered": 0, "booms answered": 0})
                continue
