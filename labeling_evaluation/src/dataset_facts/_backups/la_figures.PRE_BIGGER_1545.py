...
def fig_class_cycles(v: pd.DataFrame, path: Path) -> Path:
    ...
    fig, axes = plt.subplots(nrow, ncol, figsize=(W, 1.7 * nrow), sharey=True)
    x = np.arange(len(WINDOW_NAMES))
...
def fig_proximity_region(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    n = len(m)
    fig, ax = plt.subplots(figsize=(W, W * 0.86))
    ...
    ax.set_title("Technological proximity between firms (class profile); strip = the firm's region", fontsize=9)
    ...
    handles = [matplotlib.patches.Patch(facecolor=REGION_COLOR[r_], hatch=hatch_reg[r_], edgecolor=(0, 0, 0, 0.55), label=r_)
               for r_ in REGIONS]
    _, y0, _ = _fig_bottom(fig)
    fig.legend(handles=handles, loc="upper center", ncol=3, bbox_to_anchor=(0.5, y0 - 0.01), fontsize=7.5)
    fig._bw_off = True
