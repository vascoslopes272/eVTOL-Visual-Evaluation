...
def fig_filings_per_year(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    if normalised:
        ...
        if ratio_col in ti.columns:
            # 2026-09-24, author's request: panel (ii) draws the COUNTS side by side on a log axis —
            # this corpus's patents (the line of (i)), all aeronautics (B64) patents, all CPC
            # section B patents and all patents, each at the same nine offices and priority year —
            # so the ratio is visible as the gap between two lines instead of stated as a
            # percentage. Complete years only: both sides are truncated by the publication lag
            # and the baseline empties faster, so the incomplete years read as a rise that is
            # not real. The two wider baselines are drawn only when the fetch has stored them.
            done = ti[ti["complete"].astype(bool)]
            base = la_baseline.load().reindex(done.index)
            series = [("patents acquired", done["patents acquired"], INK, "-", "o", "eVTOL patents, this corpus"),
                      ("b64", done["B64 patents (same offices)"], MUTED, "--", "s", "all aeronautics patents (CPC B64)")]
            if "cpcB_offices" in base.columns and base["cpcB_offices"].notna().any():
                series.append(("cpcB", pd.to_numeric(base["cpcB_offices"], errors="coerce"), INK2, "-.", "^",
                               "all CPC section B patents (operations, transport)"))
            if "all_offices" in base.columns and base["all_offices"].notna().any():
                series.append(("all", pd.to_numeric(base["all_offices"], errors="coerce"), "#7a7a7a", ":", "D",
                               "all patents"))
            for _, ys, col, ls, mk, lab in series:
                ax2.plot(done.index, ys, color=col, lw=1.8, ls=ls, marker=mk, ms=3.2, label=lab + " — same nine offices")
            ax2.set_yscale("log")
            # the ratio, printed once at the last complete year as the gap between the two lines
            _e = pd.to_numeric(done["patents acquired"], errors="coerce")
            _b = pd.to_numeric(done["B64 patents (same offices)"], errors="coerce")
            _ok = done.index[_e.notna() & _b.notna() & _b.gt(0)]
            last_c = int(_ok.max())
            e_last, b_last = float(_e.loc[last_c]), float(_b.loc[last_c])
            ax2.annotate(f"{last_c}: {e_last:.0f} eVTOL patents of {b_last:,.0f} B64 = "
                         f"{1000 * e_last / b_last:.0f} per 1 000",
                         (last_c, e_last), textcoords="offset points", xytext=(-6, -14), ha="right",
        ...
        if ratio_col in ti.columns:
            ax2.set_ylabel("patents per priority year (log scale)")
            # between the eVTOL line (~10²) and the B64 line (~10⁴), where nothing is drawn
            ax2.legend(loc="center left", bbox_to_anchor=(0.0, 0.40), fontsize=7, ncol=1,
                       handlelength=2.0, frameon=True)
            ax2.set_title("The corpus's patents beside all aeronautics, all section-B and all patenting "
                          "of the same offices — the gap between two lines is the ratio", fontsize=9.5)
        else:
