...
* **One to three items per question**, every one of them graded ``core`` in
  :data:`la_index.GRADE`. The answer is stated first, in prose; the items are the evidence
  under it, and the selection rule is enforced by the render script, not by eye.
...
# while reading it. Three items are printed that are not core, each named in
# :data:`NOT_CORE` with its reason, and the render script fails on any fourth.
#
...
NODES: List[Dict] = [
    ...
    # ---------------------------------------------------------------- Q2
    dict(id="Q2",
         text="**No, and the test could have said yes.** Against three conditions fixed in the "
              "Preliminary Analysis (5.7) before any curve of this corpus was drawn, the largest "
              "archetype of any window reaches {tk_dd_top_share} against a 50 % line, condition 2 "
              "fires in {tk_dd_below} of {tk_dd_cells} level-windows and only in the two "
              "earliest, condition 3 in {tk_q_below} of {tk_q_cells} cells and only under one of "
              "two weightings — and no window meets more than one of the three. Nor do the "
              "classes settle internally: {top_class}'s {tk_slc_n_2023} aircraft of 2020-23 take "
              "{tk_slc_configs_2023} different configurations between them.",
...
TEXTS: Dict[str, str] = {
    "how":
        ...
        "graded core — the vocabulary drawing, and the two tables the map never reached — its "
...
AFTER: Dict[str, str] = {
    "Q2": "*The three thresholds, and where each came from, are Table 2.1.3.2 of the full "
          "document. None of them was chosen after the curves were drawn.*",
...
#: The three items printed here that ``la_index.GRADE`` does not grade **core**, each with the
#: reason it is printed anyway. The render script refuses any item outside this list that is
#: not core, so the selection rule cannot quietly slip.
