...
def refinement_funnel_svg(n: Dict, path: Path) -> Path:
    ...
    refinements = [
        ...
        ("Refinement 2 — patents to unique aircraft (Tables 2.2.1, 2.2.2)", [
            ...
            ("+", n["rep_extra"], f"further aircraft: {n['rep_multi']} of them draw two to five", False),
            ...
            ("=", n["unique"], f"unique aircraft = {n['orig_obs']} originals + {n['s3']} S3 similars "
             "(a new aircraft)", True),
            ("", n["primary"], f"primary patents = {n['representative_s']} − {n['o12_patents']} patents "
             "that only repeat an aircraft", False),
        ]),
