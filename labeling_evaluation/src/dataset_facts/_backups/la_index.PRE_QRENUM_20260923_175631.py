...
TABLE_CAPTIONS: Dict[str, str] = {
    **_pa.TABLE_CAPTIONS,
    ...
    "la_proximity_region": (
        "Mean technological proximity by pair of regions. Every pair of the firms in Figure 2.2.6 is put in "
        ...
        "than another, and the Source line of Figure 2.2.6 carries the permutation test that answers it"),
    ...
    "la_ari_correlation": (
        ...
        "otherwise. With at most 18 firms this table is not where the answer is; Table 2.2.7c is"),
    "la_ari_gap": (
    ...
    "la_open_questions": "Questions the patent record was asked and could not answer, one row per question, grouped by sector question. `what tried to answer it` names the figures and tables built for the question, numbered as this render numbers them, and the analyses probed on the built tables without being drawn; `why the patent record cannot` quotes, word for word, the grade printed under each of those items and adds the number the attempt returned; `what would answer it` names the record that would. The closing row of each group lists the items graded weak under that question that raise no question of their own — each restates an item that stays, is a check that passed, or describes the drawings — so that every weak item of the document is accounted for here exactly once",
    ...
    "la_ip_strategy": (
        ...
        "bubbles of Figure 2.2.5a are sized by; it rewards age, because a patent from 2013 has had thirteen "
    ...
    "la_ip_by_class": (
        ...
        "attention the class has drawn, and the bubble size of Figure 2.2.5a) and a median (what a typical "
    ...
    "la_ari_clock_region": (
        "Which regions and authorities move from patent to flight fastest: the medians of Table 2.2.7d, "
    ...
    # 2.4 design drivers and their traces (2026-09-23). The three printed tables are generated from
    # tables/la_trace_trends.csv, the frame behind Figure 2.4.1, so no number in them is typed.
    "la_trace_couplings": "The couplings the fixed physical drivers (A2) predict, as bias-corrected Cramér's V between "
                          ...
                          "the table, for scale. A pair is read against that distribution, never against zero",
    "la_driver_verdicts": "The verdict table — one row per trace of the drivers table: what moved, with the numbers per "
                          ...
                          "frame as Figure 2.4.1, so the two cannot disagree. Traces the label set does not carry close "
                          "the table",
...
FIGURE_CAPTIONS: Dict[str, str] = {
    **_pa.FIGURE_CAPTIONS,
    ...
    # 2026-09-23, author's ruling: down from three variables to filer type alone (see TAKEAWAY)
    "region_grid_b": "filer type per region and window",
    ...
    "dimension_drift": "dimension drift inside each architecture class, per window: panels (i)–(iv) are the four label "
                       "fields the configuration of Figure 2.1.4 is made of, read one at a time, then powertrain. A line "
    ...
    "trace_drift": "the labelled traces inside each architecture class, per window: one panel per trace, ordered by "
                   ...
                   "cost). The machinery of Figure 2.1.5 extended from four dimensions to the trace set; a line is a "
    ...
    "trace_couplings": "the couplings the fixed physical drivers predict, as bias-corrected Cramér's V between the two "
                       "label fields, read against every other pair of the same fields; the definitional pairs are "
                       "named, drawn hollow and kept out of the distribution",
...
#: drawn as a row of small multiples (Figures 2.3.2a/b) or split over two strips.
...
FIGURE_PANELS: Dict[str, List] = {
    ...
    # --- 2.1 what is designed
    # 2.1.1 gains (ii) only when the stored aviation baseline is installed; without it the figure
    ...
    # 2026-09-23: ducted unit and powertrain dropped, so region_grid_b is filer type over the share
    # row. The share-of-the-window row is deliberately kept although region_grid already carries it: the
    # author asked for it under the variable rows of this section, and without it a mix panel
    # cannot be turned back into "did the region grow".
    "region_grid_b": [("filer type", 3), ("share of the window", 3)],
    # --- 5 low level
    "atlas_powertrain": [("by architecture", 1), ("by priority window", 1)],
    # (iii) added 2026-09-23: ducting has no regional or period signal, so it left Figure 2.3.2b
    ...
    # 2.4: one panel per drawn trace of la_tables.TRACES, in that register's order (the key cell is empty)
    "trace_drift": [("median tilting joint groups", 1), ("% fuselage kinematics not fixed", 1),
                    ("% with a retracting or blade-folding unit", 1), ("median propulsive units", 1),
                    ("% with units on booms", 1), ("% wing units at the leading edge", 1),
                    ("% with a pusher unit", 1), ("% with two or more wing panels", 1),
                    ("% swept or delta main wing", 1), ("% ground contact drawn", 1),
                    ("median propulsor types", 1)],
}
...
FIG_WIDTH: Dict[str, str] = {
    ...
    **{k: "100%" for k in FIGURE_CAPTIONS if k.startswith("atlas_")},
    ...
    "linked_check": "100%",
    "trace_drift": "91%", "trace_couplings": "99%",
}
...
ROW_CAP: Dict[str, int] = {
    **_pa.ROW_CAP,
    ...
    "la_cohort_mix": 24,
    "la_open_questions": 60,    # Appendix C prints whole: one row per question plus one closing row per group
    "la_driver_verdicts": 30,   # 2.4.3: every labelled trace and every unlabelled one, printed whole
    "la_trace_couplings": 12,
}
COLUMNS: Dict[str, List[str]] = {
    **_pa.COLUMNS,
    # Appendix C: the hidden columns (kind, key, weak items) stay in the CSV and feed the takeaway count
    "la_open_questions": ["sector question", "the question asked", "what tried to answer it",
                          "why the patent record cannot", "what would answer it"],
    ...
    # 2.4: `verdict class` stays in the CSV and feeds the takeaway counts
    "la_driver_verdicts": ["trace", "driver kind", "where drawn", "what moved", "drivers that predict it", "verdict"],
    "la_trace_couplings": ["driver", "predicted pair", "fields", "V", "aircraft",
...
NODES: List[Dict] = [
    ...
    # ids ARE the printed numbers (``figure_number`` / ``table_number`` build every "Figure 2.1.6a" from
    ...
    # The taxonomy stays in front, as Chapter 1: every finding is stated in its vocabulary.
    # ================================================================ 1  (was 2)
    dict(id="1", title="Taxonomy",
         figures=["design_space_cards", "codebook_classes", "codebook_dimensions"]),
    # ================================================================ 2  (was 4)
    # one line of navigation, added with the reorganisation of 2026-09-23: the findings now come
    # before the audit trail, so the reader is told once where the data set is described. Delete
    # this ``text`` to take it out; nothing else depends on it.
    dict(id="2", title="Preliminary Analysis", level="unique aircraft",
         text="How the data set was built, and every check on its quality, are in Appendix A and "
              "Appendix B at the end of this document; the questions this record was asked and could "
              "not answer, each with the item that tried and the reason, are listed in Appendix C."),
    dict(id="2.1", title="What is being designed, and whether it converges", level="unique aircraft"),
    dict(id="2.1.1", title="Filings per year", figures=["filings_per_year"], tables=["la_filings_per_year"]),
    ...
    dict(id="2.1.2", title="Class shares per window", figures=["atlas_arch_time"]),
    # 2.1.3 reads in three steps (user, 2026-09-22): what is evaluated and why, then the
    # conditions each stated in full, then the result. The archetype tables come before the test.
    dict(id="2.1.3", title="The dominant-design test", level="unique aircraft"),
    dict(id="2.1.3.1", title="What is evaluated: the archetype levels",
         ...
         after=("*How to read the two tables.* An archetype is a combination of label answers. At a "
                ...
                "archetype; and it is the level on which 2.1.6, 2.1.9 and 2.3.3 are built, so it is "
                ...
                "for the reader.")),
    dict(id="2.1.3.2", title="The conditions, fixed in advance", tables=["la_dd_conditions"]),
    dict(id="2.1.3.3", title="The result", figures=["dominant_design", "dominant_design_q"],
         tables=["la_dd_result", "la_dominant_design", "la_dd_q"]),
    dict(id="2.1.4", title="Within-class convergence", figures=["class_configs"], tables=["la_class_configs"]),
    dict(id="2.1.5", title="Dimension drift inside the classes", figures=["dimension_drift"], tables=["la_dimension_drift"]),
    # 2026-09-23, moved here from the end of 2.1 (it was 4.1.9) on the author's ruling: the crowded and
    # open zones explain the propulsor-count against architecture relation that 2.1.4 and 2.1.5 raise,
    # so the section reads straight after the configuration (2.1.4) and its decomposition (2.1.5) —
    # those two are a pair and stay together, which is why this goes after 2.1.5 and not between them.
    # It also gained the filer-weight figure and table the same day: they answer how much of a class is
    # one company repeating itself, which Table 2.1.6a cannot, because a count of distinct filers says
    # how many there are and not how unevenly they divide the class.
    dict(id="2.1.6", title="Crowded and open zones of the design space, and the weight of each filer",
         figures=["zones", "filer_weight"], tables=["la_zones", "la_class_filer_weight"]),
    # 2026-09-23, author's ruling: the table of this section is cut ("basically the same thing as the
    # figure"), and the figure is now one longitudinal bar with the percentage per window instead of
    # twelve panels. la_tables still builds la_class_cycles, so the CSV keeps every number the table
    # printed; putting the name back in ``tables`` here is all it takes to print it again.
    dict(id="2.1.7", title="Where each class lives in time", figures=["class_cycles"]),
    dict(id="2.1.8", title="Lead and lag by region and by class", figures=["lead_lag"],
         tables=["la_lead_lag_region", "la_lead_lag_class", "la_class_region_timing"]),
    dict(id="2.1.9", title="Diversity per window", figures=["hill"], tables=["la_hill_by_window"]),
    dict(id="2.1.10", title="Abandonment: patents no longer in force", figures=["abandonment"],
         tables=["la_abandonment_by_class"]),
    dict(id="2.2", title="Who designs it, and how they compete", level="firm"),
    # 2.2.1: the tier table (la_firm_tiers) was removed on the author's ruling of 2026-09-23 — the
    ...
    dict(id="2.2.1", title="Coverage, and whether market standing tracks patenting",
         figures=["coverage"], tables=["la_market_vs_patents"]),
    dict(id="2.2.2", title="The body of filers over time, and what new firms enter with",
         figures=["filers_over_time"], tables=["la_filers_by_window", "la_cohort_mix"]),
    dict(id="2.2.3", title="Who filed, and which firms can move the reading",
         figures=["atlas_filers", "firm_influence"],
         tables=["la_filer_mix", "a2_d8_concentration", "la_firm_leverage"]),
    dict(id="2.2.4", title="Within-firm successions", figures=["transitions"],
         after=("*How to read this section.* A **succession** is one firm's aircraft and the next "
                ...
                "makes k − 1 successions, so the unit of Figure 2.2.4 is the pair — not the "
                ...
                "aircraft, the same firms as the “2 aircraft or more” segment of Figure 2.2.1; a "
                "firm with a single aircraft makes no pair and is absent by construction, which is "
                "why the firm count under this figure is lower than the one under Figure 2.2.1. "
                ...
                "*Why it is worth a section.* Section 2.1 shows the class mix of the corpus moving, "
                ...
                "measures the first and Figure 2.2.2 (ii) the second. A class whose bar sits above "
    ...
    dict(id="2.2.5", title="IP strategy: depth, breadth, claims and citations", figures=["ip_strategy", "spans_by_firm"],
    ...
    # Table 2.2.6a) is cut — "take that out, it is not necessary, it is already all that I need in the heat
    ...
    dict(id="2.2.6", title="Technological proximity between firms, and region", figures=["proximity_region"],
    ...
    dict(id="2.2.7", title="The AAM Reality Index firms: score, funding, and the patent clock against the market clock",
         ...
         text=("**ARI and AAM are one index, not two.** The AAM Reality Index — AAM for Advanced Air "
               ...
               "corpus's unique aircraft, and Table 2.2.7c shows how they differ from the other 515 "
               ...
               "Table 2.2.7c instead. Table 2.2.7b is nevertheless the weaker of the two — with 18 firms "
    ...
    dict(id="2.2.8", title="TRL-ranked firms", tables=["la_trl_placeholder"]),
    dict(id="2.3", title="Where", level="patent → unique aircraft"),
    dict(id="2.3.1", title="Region, country and publication office", figures=["atlas_region"]),
    # 2026-09-23, author's ruling: down from six variables to four — powertrain and ducting left
    # Figure 2.3.2b for the sections that own them (3.1 and 3.2 (iii)); the title follows the count.
    dict(id="2.3.2", title="Region over time, four variables", figures=["region_grid", "region_grid_b"]),
    ...
    # writes tables/la_specialisation.csv, which the figure's Source line points at for the per-cell counts.
    dict(id="2.3.3", title="Regional specialisation by archetype", figures=["specialisation"]),
    # 2026-09-23, author's ruling: the table of this section is cut, the figure stays.
    dict(id="2.3.4", title="Country and class", figures=["country_class"]),
    # ================================================================ 2.4  (new 2026-09-23)
    # The author's frame: the design drivers are unobserved, each predicts a TRACE in the morphology,
    # and the traces are what the aircraft are labelled with. Placed as 2.4 rather than as 2.1.11
    # because it reads across the whole chapter — what moved (2.1), in which class (2.1.2) and how
    # the sector's mix shifted — and closes it with the one reading the author asked for: which
    # drivers each movement is consistent with. A new 2.1.11 would have made it one more item of a
    # section whose items are single measurements; this is a synthesis over them. Nothing after it
    # is renumbered: 2.4 is the last section of the chapter.
    dict(id="2.4", title="Design drivers and their traces", level="unique aircraft",
         text=("The drivers of eVTOL design — the technology that moved over the window, the physics that "
               "did not, the requirements and the life-cycle costs — are not observed in a patent. What a "
               "patent drawing shows is the trace each driver leaves in the morphology, and the traces are "
               "what the {unique_s} aircraft are labelled with. The table below is the author's, in "
               "substance: **A1 technological, moved over the window** — regime transition → hover-cruise "
               "joint location, fuselage kinematics, propulsor tilt and retraction; distributed propulsion "
               "and rotor control → propulsor count and placement, rotor diameter, hub mechanism. **A2 "
               "physical, fixed** — hover load path → boom presence and arrangement, wing characteristics, "
               "propulsor placement; wing structural and aeroelastic limits → wing characteristics, "
               "propulsor placement; cruise stability and control → empennage presence and type, wing "
               "characteristics; flow interference and drag → propulsor placement relative to the wing, "
               "retraction or folding, boom arrangement. **B requirement** — mission range and speed → wing "
               "presence and characteristics; payload → fuselage, ground contact; occupant presence → cabin, "
               "fuselage kinematics; failure tolerance → propulsor count, wing presence; community noise → "
               "propulsor count, rotor diameter, rotor enclosure; operating environment → ground contact "
               "type, dimensions, wing and boom folding. **C life-cycle cost** — development and "
               "certification cost → propulsor type count, number of tilting joints; production cost → "
               "propulsor type count; maintenance cost → hub mechanism, tilting joints, ground contact type. "
               "Battery specific energy is left out on the author's ruling: it is a sector-level enabler with "
               "no trace of its own, and Appendix C holds it.\n\n"
               "One trace is usually predicted by several drivers — more propulsive units is consistent with "
               "distributed control, with redundancy and with community noise — and the patents cannot "
               "separate them. So the section runs traces first and drivers second. 2.4.1 measures what "
               "moved inside each class, with the machinery of 2.1.5 extended to every labelled trace. 2.4.2 "
               "tests what is coupled across fields, which is the only prediction a fixed physical driver "
               "makes. 2.4.3 then states, for each trace, what moved, which drivers predict it and how many: "
               "a trace with one plausible driver yields a sharp statement, a trace with three yields *the "
               "sector moved this way; three drivers predict it; the record cannot say which*. Nothing here "
               "is a cause. A driver is consistent with a movement, never shown to have produced it, and a "
               "movement inside a class is kept apart from the shift of the class mix (2.1.2), which can "
               "move a pooled trace on its own. The traces the drivers table predicts that no label carries "
               "— rotor diameter, hub mechanism, cabin dimensions, aircraft size — close the verdict table "
               "and are listed in Appendix C with what would carry them.")),
    dict(id="2.4.1", title="What moved inside the classes", figures=["trace_drift"]),
    dict(id="2.4.2", title="What is coupled: the physical drivers' test", figures=["trace_couplings"],
         tables=["la_trace_couplings"]),
    dict(id="2.4.3", title="The verdicts", tables=["la_driver_verdicts", "la_driver_questions"]),
    # ================================================================ 3  (was 5)
    dict(id="3", title="Low-level Analysis", level="unique aircraft and image"),
    # 2026-09-23, author's ruling: the table of this section is cut ("take it off"); the figure carries
    # the same shares.
    dict(id="3.1", title="Hybrid against electric, not-stated included", figures=["atlas_powertrain"]),
    dict(id="3.2", title="Propulsive units and their arrangement", figures=["atlas_units"]),
    dict(id="3.3", title="The design space as paired fields", figures=["atlas_design_heatmaps"]),
    dict(id="3.4", title="Image-level answers", figures=["atlas_state_by_arch"], tables=["a2_d2_figure_slot_answers"]),
    dict(id="3.5", title="Mission of the aircraft linked to evtol.news, and the industry named in the text",
         # Figure 3.5c added 2026-09-23: the section describes a subset, so the first thing it owes
         # the reader is whether that subset stands for the rest. It is placed last so that the
         # numbers of 3.5a and 3.5b do not move under the author's own review notes.
         ...
         # from a text classifier he has no way of verifying, and he does not want to. Figure 3.5b
    ...
    dict(id="3.6", title="Examination outcome as a regulatory signal", figures=["examination"],
         tables=["la_examination_office", "la_examination_class"]),
    # ================================================================ Appendix A  (was chapter 1)
    # Moved to the back 2026-09-23: this is the data set's own audit trail, not a finding. It answers
    # no analysis question, so a reader meets it when they want to check the provenance of the numbers
    # in Chapters 1 to 3, not before them.
    dict(id="A", title="Data Sets Construction",
         figures=["fig_02_refinement_funnel", "atlas_removal"]),
    dict(id="A.1", title="Refinement 1 — representativity", level="patent",
         tables=["a2_d1_similars", "a2_d9_publication_lag_region", "a2_d1_filing_status"]),
    dict(id="A.2", title="Refinement 2 — observations to unique aircraft", level="unique aircraft",
         figures=["atlas_duplicates"], tables=["a2_d7_duplicates", "a2_d7_aircraft_per_patent"]),
    dict(id="A.3", title="Refinement 3 — figure approval", level="unique aircraft → image",
         figures=["atlas_figure_approval"], tables=["a2_d11_figure_approval"]),
    # ================================================================ Appendix B  (was chapter 3)
    dict(id="B", title="Data Quality and Facts", level="unique aircraft, G1 to M3"),
    dict(id="B.1", title="Missingness", figures=["atlas_fill"], tables=["a2_d4_missingness"]),
    dict(id="B.2", title="Intra-rater relabel of 50 patents", tables=["la_relabel_protocol"]),
    dict(id="B.3", title="Flagship check", figures=["atlas_flagship"],
         tables=["a2_d13_flagship_check", "la_name_arch_check"]),
    dict(id="B.4", title="Most common answers", tables=["a2_d3_selected_fields"]),
    dict(id="B.5", title="Field inventory", figures=["atlas_fields"], tables=["a2_d2_informative_fields"]),
    dict(id="B.6", title="Image label against the whole-patent reading", figures=["atlas_arch_gt"]),
    # ================================================================ Appendix C  (new 2026-09-23)
    # The questions the corpus was asked and could not answer, one row per QUESTION rather than per
    # item, built by ``la_tables.open_questions`` from ``OPEN_QUESTIONS``, ``UNLABELLED_DRIVERS`` and
    # ``GRADE`` at the end of this module. Every item number in the table is resolved from this NODES
    # walk when the document is built, so a renumbering cannot detach a question from its item. The
    # ``{sector_questions}`` placeholder is filled by ``text()`` from ``QUESTIONS``, so the eight
    # questions are printed here in the wording that register holds, and typed nowhere else.
    dict(id="C", title="Questions asked, and what the corpus could not answer",
         text=("Every figure and table of this document carries a grade, and the items graded weak "
               "are discounted where they stand: the number cannot carry the claim, or the item adds "
               "nothing to another that stays. A weak item is not a weak question. This appendix turns "
               "the discounted items back into the questions that were asked, one row per question, "
               "and states for each what was built to answer it, why the patent record cannot — with "
               "the number that stops it — and what record would. The analyses that were probed on the "
               "built tables and rejected before being drawn are on the same list with the number that "
               "rejected each, and so are the design drivers whose trace no label carries. Three kinds "
               "of item are absent as questions and present by name instead, in the closing row of "
               "their group: an item that restates one that stays, a check that passed, and a "
               "description of the drawings — so every weak item of the document is accounted for here "
               "exactly once. Questions the corpus can answer and the document has not yet built are a "
               "different list and are kept out of this one.\n\n"
               "The sector questions the rows are grouped under: {sector_questions}")),
    dict(id="C.1", title="The inventory, by sector question", tables=["la_open_questions"]),
]
...
def heading(node_id: str, values: Optional[Dict] = None) -> str:
    ...
    d = depth(node_id)
    if node_id.split(".")[0] in APPENDIX:          # "A" -> Appendix A; "A.1" -> ### A.1 …
        return f"## Appendix {node_id} — {title}" if d == 0 else f"{'#' * (2 + d)} {node_id} {title}"
    if not node_id[0].isdigit():                   # a node outside the numbering, e.g. "flags"
        return f"## {title}"
    if d == 0:
...
def text(node_id: str, values: Optional[Dict] = None) -> str:
    vals = dict(values or {})
    # Appendix C prints the eight sector questions out of ``QUESTIONS`` (defined further down; this
    # runs at render time). No other node uses the placeholder, so no other text can change.
    vals.setdefault("sector_questions", sector_questions_line())
    return _pa._fill(node(node_id).get("text", ""), vals)
...
N_FROM_TABLE: Dict[str, str] = {
    ...
    "n_ari": "la_ari_firms",
    "n_zones": "la_zones",
    "n_pairs": "la_trace_couplings_all",
...
PROVENANCE: Dict[str, Dict[str, str]] = {
    ...
    # Appendix C — built from the registers of this module; see OPEN_QUESTIONS at the end of the file
    "la_open_questions": dict(
        unit="questions asked of the sector (one row each), and the figures, tables and probed analyses "
             "of this document that were built to answer them",
        base="not a measurement — every reason quoted is the grade printed under the item itself, read "
             "out of the same register when the document is built, with the number the attempt returned",
        transform="none; item numbers are resolved from the running order at build time, so a "
                  "renumbering cannot detach a question from its item"),
    ...
    "atlas_arch_gt": dict(
        ...
        base="each row is one ground-truth class. The 3 aircraft left out are named in the table of "
             "section B.3: one whose patent states no class, and two the wizard left unlabelled, one "
    ...
    "a2_d9_architecture_by_window": dict(
        ...
        transform="share of the window; only the four largest classes are printed, so the rows do not "
                  "sum to 1"),
    "firm_weighted": dict(
        unit="two units on the same axes — unique aircraft (n {unique_s}), and filers (an organisation is "
             "one filer, an individual inventor's patent is its own filer)",
    ...
    # 2.1.3's own tables (la_archetype_levels, la_archetype_choice, la_dd_conditions,
    ...
    "dimension_drift": dict(
        ...
        transform="median propulsive units (an order statistic, not a mean); panels (i)–(iv) are the four "
                  "fields of the 2.1.4 configuration and divide by the class's own aircraft in that window, "
    ...
    "filer_weight": dict(
        unit="unique aircraft (n {unique_s}), attributed to a filer — a named firm is one filer, an "
             "individual inventor's patent is its own filer, so no two lone inventors are merged "
        ...
        transform="the share of the class (or of the class-window) held by its largest filer, and by "
                  ...
                  "2.1.6a's `filers` column says how many filers an archetype has and cannot say how "
    ...
    "la_market_vs_patents": dict(
        unit="named firms — every organisation that filed, split by whether the AAM Reality Index rates "
    ...
    "la_coverage_segments": dict(
        ...
        transform="raw counts per disjoint segment — unlike the nested tiers of la_firm_tiers, these "
                  "rows do add up to every firm that filed and its aircraft"),
    "la_firm_tiers": dict(
    ...
    "la_filers_by_window": dict(
        ...
        transform="raw counts; the individual-inventor and university columns are shares of the window's "
    ...
    "firm_influence": dict(
        ...
        transform="(i) a leave-one-firm-out difference: the class shares of the analysis set minus the "
                  ...
                  "largest filer and by its three largest — the individual inventors' filings of the "
                  "class are in the denominator, so the bars do not reach 100 %"),
    ...
    "la_class_concentration": dict(
        ...
        transform="raw counts of the named firms filing in the class, and the share of the class held "
                  ...
                  "by named firms at all, the rest being individual inventors' filings"),
    "la_cohort_mix": dict(
    ...
    "la_filer_mix": dict(
        ...
        transform="raw counts and the share on each denominator; the last column divides the two — "
                  ...
                  "aircraft. The distinct-filer count exists only for the organisation rows: an individual "
                  "inventor's filing is not resolved to a person"),
    ...
    "la_ip_strategy": dict(
        ...
        transform="ratios and order statistics — patents per aircraft, median family size, median claims, "
                  "mean forward citations (raw, not age-normalised, and the bubble size of Figure 2.2.5a) "
    ...
    "la_proximity_region": dict(
        ...
        transform="mean, median and range of the proximities in each box; the pair count is printed "
                  ...
                  "Figure 2.2.6, on 10 000 shuffles of the region labels over the firms"),
    "ari": dict(
    ...
    "la_ari_clock_region": dict(
        ...
        transform="medians of the columns of Table 2.2.7d; no test is run and none would be supportable "
    ...
    "region_grid": dict(
        ...
        transform="(i)–(iii) share of the cell, each region × window row summing to 100 %; (iv) the "
                  ...
                  "absolute; Figure 2.1.1 carries the corpus-wide normalisation. A cell under 10 "
    ...
    "region_grid_b": dict(
        ...
        transform="shares as in Figure 2.3.2a, with the same n < 10 guard and the same absence of any "
                  ...
                  "inclusion gate — a non-electric aircraft leaves the set on an ElectricSimilar tag "
                  "— so the variable can only read electric, hybrid or not stated, there is no "
                  "non-electric column and there never can be one, and nearly all of what moved "
                  "across regions and windows was the not-stated band. That share is a real finding "
                  "about the patent text and it is made where it belongs. Ducting went to panel (iii) "
                  "of the propulsive-units figure: "
                  "it does not separate the regions (chi-square p = 0.12 over the four region "
                  "groups, 0.06 over the three main ones) and it does not move over the windows "
                  "(p = 0.81, near 37 % in every one), but it does separate the architecture classes "
                  "(p = 0.0013) and the propulsive-unit bands (p = 0.00012), so it is drawn against "
                  "rotor count instead. Filer type is read off the raw PatSeer assignee string: "
                  "first assignee, country code stripped; a university, institute or national "
                  "research agency by regex; otherwise an organisation if the string carries a "
                  "legal form of any office in the corpus, an activity word, a digit or more than "
                  "five name words; otherwise an individual inventor. Until 2026-09-23 the type "
                  "came instead from company_canonical (grouper.py's hand-written list of ~110 "
                  "companies), which made it a measure of that list's coverage: the band then drawn "
                  "'assignee not in the list' held 138 aircraft, 118 of them companies and 16 "
                  "universities or institutes, and not one of them anonymous. Audited against a "
                  "reading of all 393 assignee strings of these 665 aircraft: the old rule was "
                  "right 97 % on person against organisation and 77 % on the three-way split, this "
                  "one agrees with the reading on all 663 readable aircraft. The fourth band, 'no "
                  "assignee named', is empty — every aircraft here names an assignee"),
    "specialisation": dict(
    ...
    "atlas_units": dict(
        ...
        base="(i) each class's own aircraft; (ii) only those where the feature can be read, printed "
             "as read / total under the class code; (iii) each propulsive-unit band's own aircraft, "
             "n printed under the band — every one of the 632 banded aircraft answers the duct "
             "question, and the aircraft with no propulsor card and the one recorded with 0 units "
             "have no duct answer and form no band, which is why the bottom band counts one "
             "aircraft fewer than the bottom bin quoted in the takeaway",
        transform="(i) one dot per aircraft, the bar spanning the middle half of the class (25th to "
                  ...
                  "they stand at render time; (ii) share of that readable base carrying the feature; "
                  "(iii) SHARE OF AIRCRAFT, NEVER OF ROTORS — any_ducted is one answer per aircraft, "
                  "so an aircraft with a single ducted fan among twelve open rotors counts exactly as "
                  "much as one that ducts all twelve. The dashed line is the same share over every "
                  "band together and the bands are the cuts of the A0c archetype level, so the panel "
                  "can be read straight against the archetype-level and regional-specialisation "
                  "readings, which use the same cuts. A band resting on fewer than 20 "
                  "aircraft would be drawn hollow with its own n and the line broken either side of "
                  "it; none is. Shares and the chi-square tests behind the reading are in "
    ...
    "mission": dict(
        ...
        transform="(i)–(iv) share of the folded class, each bar 100 %; the bottom panel is the share "
                  ...
                  "Whether these aircraft stand for the whole analysis set is tested in Figure 3.5c, "
    ...
    "linked_check": dict(
        unit="all {unique_s} unique aircraft, split into the ones linked to an evtol.news page and "
             "the ones with no page — the test behind every other item of 3.5",
    ...
    # ---------------------------------------------------------------- 2.4 design drivers (2026-09-23)
    "trace_drift": dict(
        unit="unique aircraft, one architecture class per line — not an archetype",
        base="the aircraft of one class in one window that answer the trace; a cell under 5 of them is blank, "
             "a marker on 5–9 is hollow. Classes drawn by rule: 5+ aircraft in 3+ of the 5 windows (SLC, TR, CVT, "
             "TW, MR). Two panels divide by a narrower base and say so: wing units at the leading edge by the "
             "aircraft with units on a wing, the planform panel by the aircraft with a planform",
        transform="a share of the row's aircraft for a yes/no trace, a median for a count (tilting joint groups, "
                  "propulsive units, propulsor types); a value an override hides is left out, never read as 0 "
                  "(rule 1). Never a share of rotors. The trend test is not in the picture: tables/la_trace_trends.csv"),
    "trace_couplings": dict(
        unit="pairs of label fields (n {n_pairs}: every pair of the Gower shortlist of Preliminary Analysis 5.3, "
             "unpooled, plus the retraction and propulsor-type traces), each measured on the unique aircraft that "
             "answer both fields",
        base="every non-definitional pair of the same fields: a predicted pair is read against that distribution's "
             "median, upper quartile and 90th percentile, never against zero",
        transform="bias-corrected Cramér's V (Bergsma, 2013) on the crosstab of the two fields, counts banded; "
                  "φ² shrunk by (k−1)(r−1)/(n−1) and the table dimensions likewise, so a sparse pair cannot score on "
                  "its sparseness. Definitional pairs (the codebook's own construction) are named and excluded"),
    "la_trace_couplings": dict(
        unit="one row per predicted pair of label fields, then the three strongest pairs of the whole matrix",
        base="every non-definitional pair of the same fields (the `percentile` column is the share of them below "
             "the row's V)",
        transform="bias-corrected Cramér's V on the aircraft answering both fields; `reading` compares V with the "
                  "median, upper quartile and 90th percentile of the distribution"),
    "la_driver_verdicts": dict(
        unit="one row per trace of the drivers table: the labelled ones first, then the ones no label carries",
        base="per class, the aircraft of the class with a value for the trace and a complete priority year (to 2023); "
             "a class row is read on 20 or more of them, and the first value is the earliest window with 5 or more",
        transform="`what moved` is generated from tables/la_trace_trends.csv: Spearman ρ between the priority year "
                  "and the trace over the class's aircraft, `rises`/`falls` at p < 0.05, else `flat`; a median prints "
                  "its mean beside it. The verdict follows a fixed rule: a fixed physical driver (A2) is never a "
                  "candidate for a movement; among the rest, one driver consistent with the direction = single "
                  "driver, several = overdetermined, drivers of opposite sign = opposed, no movement anywhere = the "
                  "trace does not move"),
    "la_driver_questions": dict(
        unit="three questions about the traces",
        base="the class rows and the pooled row of tables/la_trace_trends.csv",
        transform="one generated sentence per question; every number in it is that table's own"),
}
...
#: · ``min`` · ``share`` (sum of ``col`` over ``where`` / sum over ``over``) · ``mean``
#: (average of ``col`` over the matching rows, weighted by the column ``weight`` names when
#: the spec gives one — use it on a table of group summaries, where the unweighted average
...
TAKEAWAY_NUMBERS: Dict[str, Dict] = {
    ...
    "tk_open_weak_n": dict(table="la_open_questions", how="sum", col="weak items", fmt="int"),
    "tk_open_weak_placed_n": dict(table="la_open_questions", how="sum", col="weak items placed", fmt="int"),
    "tk_open_weak_unplaced_n": dict(table="la_open_questions", how="sum", col="weak items unplaced", fmt="int"),
    # ---------------------------------------------------------------- Appendix A
    ...
    # ``la_tables`` does not build — the specs resolved to nothing and Figure 2.1.4 and Table
    # 2.1.4 printed an ellipsis where the number belongs. Corrected against
    ...
    "tk_slc_modal_1619": dict(table="la_class_configs",
                              where=[("code", "SLC"), ("window", "2016-19")], col="share in it", fmt="pct"),
    ...
    # booms`. All five resolved to nothing, so Table 2.1.5's takeaway printed four ellipses.
    ...
    # 2.1.6b, the filer-weight figure and table (2026-09-23)
    ...
    "tk_named_share": dict(table="la_firm_tiers", where=("rule", "all named companies"),
                           col="share of the analysis set", fmt="pct"),
    # The firm count of THIS document. numbers.live's {named_companies} is the Preliminary
    # Analysis figure, built by a2 on company_canonical, and counts only the firms grouper.py's
    # hand-written table resolves; since 2026-09-23 Chapter 4 counts every organisation that filed.
    "tk_firms_all": dict(table="la_firm_tiers", where=("rule", "all named companies"),
                         col="firms", fmt="int"),
    "tk_firms_air": dict(table="la_firm_tiers", where=("rule", "all named companies"),
                         col="aircraft", fmt="int"),
    ...
    # --- 2.2.1 market standing against patenting (author's ruling 2026-09-23)
    ...
    # --- 2.2.3 firm leverage. tk_lev_firm / tk_lev_pp / tk_conc_max are defined once, further down
    ...
    # --- 2.2.2 entrants against the field
    ...
    # `within` or `across` in `within a region?`. Both specs matched nothing, so Figure 2.2.6
    # and Table 2.2.6 printed "… against …" in place of the null they exist to report. The
    ...
    "tk_prox_diff": dict(table="la_proximity_region", how="mean",
                         where=("within a region?", "across"),
                         col="mean", weight="pairs", fmt="2f"),
    "tk_prox_same": dict(table="la_proximity_region", how="mean",
                         where=("within a region?", "within"),
                         col="mean", weight="pairs", fmt="2f"),
    ...
    # so "TR · 0-3" matched no row and Figure 2.3.3's takeaway named its clearest example
    ...
    # Figure 3.2 (iii), ducting against rotor count. Every share is a share of AIRCRAFT; the class
    ...
    "tk_duct_b78": dict(table="la_duct_units", where=[("level", "propulsive units"), ("group", "7-8")],
                        col="share ducted", fmt="pct"),
    "tk_duct_b9": dict(table="la_duct_units", where=[("level", "propulsive units"), ("group", "9+")],
                       col="share ducted", fmt="pct"),
    "tk_duct_all": dict(table="la_duct_units", where=[("level", "propulsive units"), ("group", "all bands")],
                        col="share ducted", fmt="pct"),
    "tk_duct_cls_hi": dict(table="la_duct_units", how="max", col="share ducted", fmt="pct",
                           where=[("level", "architecture class"), ("reported", "True")]),
    "tk_duct_cls_hi_name": dict(table="la_duct_units", how="max", col="share ducted", out="class",
                                where=[("level", "architecture class"), ("reported", "True")]),
    "tk_duct_cls_hi_n": dict(table="la_duct_units", how="max", col="share ducted", out="aircraft",
                             fmt="int", where=[("level", "architecture class"), ("reported", "True")]),
    "tk_duct_cls_lo": dict(table="la_duct_units", how="min", col="share ducted", fmt="pct",
                           where=[("level", "architecture class"), ("reported", "True")]),
    "tk_duct_cls_lo_name": dict(table="la_duct_units", how="min", col="share ducted", out="class",
                                where=[("level", "architecture class"), ("reported", "True")]),
    "tk_duct_cls_lo_n": dict(table="la_duct_units", how="min", col="share ducted", out="aircraft",
                             fmt="int", where=[("level", "architecture class"), ("reported", "True")]),
    ...
    "tk_exc_tr_n": dict(table="la_examination_class", where=("topType", "Tilt Rotor"),
                        col="patents", fmt="int"),
    # ---------------------------------------------------------------- new items (2.2/2.3 rework)
    "tk_name_arch_n": dict(table="la_name_arch_check", how="rows", fmt="int"),
    # 2026-09-23: the column is `shift if the firm is dropped (pp)`, never `points moved`.
    "tk_lev_max": dict(table="la_firm_leverage", how="max",
                       col="shift if the firm is dropped (pp)", out="firm"),
    ...
    # ---------------------------------------------------------------- 2.4 design drivers (2026-09-23)
    # tables/la_trace_trends.csv: one row per trace and class; `first value` is the earliest reportable
    # window, `2020-23 value` the last complete one, `movement` the Spearman verdict
    "tk_dr_tr_units_first": dict(table="la_trace_trends", where=[("trace", "propulsive units"), ("code", "TR")],
                                 col="first value", fmt="1f"),
    "tk_dr_tr_units_last": dict(table="la_trace_trends", where=[("trace", "propulsive units"), ("code", "TR")],
                                col="2020-23 value", fmt="1f"),
    "tk_dr_cvt_units_first": dict(table="la_trace_trends", where=[("trace", "propulsive units"), ("code", "CVT")],
                                  col="first value", fmt="1f"),
    "tk_dr_cvt_units_last": dict(table="la_trace_trends", where=[("trace", "propulsive units"), ("code", "CVT")],
                                 col="2020-23 value", fmt="1f"),
    "tk_dr_slc_units_first": dict(table="la_trace_trends", where=[("trace", "propulsive units"), ("code", "SLC")],
                                  col="first value", fmt="1f"),
    "tk_dr_slc_units_mov": dict(table="la_trace_trends", where=[("trace", "propulsive units"), ("code", "SLC")],
                                col="movement", fmt="raw"),
    "tk_dr_mr_units_mov": dict(table="la_trace_trends", where=[("trace", "propulsive units"), ("code", "MR")],
                               col="movement", fmt="raw"),
    "tk_dr_tr_joints_mov": dict(table="la_trace_trends", where=[("trace", "tilting joint groups"), ("code", "TR")],
                                col="movement", fmt="raw"),
    "tk_dr_tr_joints_rho": dict(table="la_trace_trends", where=[("trace", "tilting joint groups"), ("code", "TR")],
                                col="rho", fmt="2f"),
    "tk_dr_cvt_joints_mov": dict(table="la_trace_trends", where=[("trace", "tilting joint groups"), ("code", "CVT")],
                                 col="movement", fmt="raw"),
    "tk_dr_all_joints_mov": dict(table="la_trace_trends", where=[("trace", "tilting joint groups"), ("code", "ALL5")],
                                 col="movement", fmt="raw"),
    "tk_dr_slc_retract_first": dict(table="la_trace_trends",
                                    where=[("trace", "a propulsor retracts or folds its blades"), ("code", "SLC")],
                                    col="first value", fmt="pct"),
    "tk_dr_slc_retract_last": dict(table="la_trace_trends",
                                   where=[("trace", "a propulsor retracts or folds its blades"), ("code", "SLC")],
                                   col="2020-23 value", fmt="pct"),
    "tk_dr_slc_retract_mov": dict(table="la_trace_trends",
                                  where=[("trace", "a propulsor retracts or folds its blades"), ("code", "SLC")],
                                  col="movement", fmt="raw"),
    "tk_dr_all_booms_first": dict(table="la_trace_trends", where=[("trace", "units carried on booms"), ("code", "ALL5")],
                                  col="first value", fmt="pct"),
    "tk_dr_all_booms_last": dict(table="la_trace_trends", where=[("trace", "units carried on booms"), ("code", "ALL5")],
                                 col="2020-23 value", fmt="pct"),
    "tk_dr_all_fus_first": dict(table="la_trace_trends",
                                where=[("trace", "fuselage kinematics not fixed"), ("code", "ALL5")],
                                col="first value", fmt="pct"),
    "tk_dr_all_fus_last": dict(table="la_trace_trends",
                               where=[("trace", "fuselage kinematics not fixed"), ("code", "ALL5")],
                               col="2020-23 value", fmt="pct"),
    "tk_dr_tr_types_mov": dict(table="la_trace_trends", where=[("trace", "propulsor types"), ("code", "TR")],
                               col="movement", fmt="raw"),
    # tables/la_trace_couplings.csv
    "tk_dr_emp_wing_v": dict(table="la_trace_couplings", where=("predicted pair", "empennage type ↔ wing panel count"),
                             col="V", fmt="2f"),
    "tk_dr_emp_wing_pct": dict(table="la_trace_couplings", where=("predicted pair", "empennage type ↔ wing panel count"),
                               col="percentile among non-definitional pairs", fmt="pct"),
    "tk_dr_emp_pos_v": dict(table="la_trace_couplings",
                            where=("predicted pair", "empennage type ↔ longitudinal wing position"), col="V", fmt="2f"),
    "tk_dr_hover_v": dict(table="la_trace_couplings", where=("predicted pair", "boom presence ↔ wing panel count"),
                          col="V", fmt="2f"),
    "tk_dr_retract_v": dict(table="la_trace_couplings",
                            where=("predicted pair", "chordwise position of the wing units ↔ retraction"),
                            col="V", fmt="2f"),
    "tk_dr_plan_v": dict(table="la_trace_couplings", where=("predicted pair", "units on the main wing ↔ planform"),
                         col="V", fmt="2f"),
    "tk_dr_pairs_n": dict(table="la_trace_couplings_all", how="rows", fmt="int"),
    "tk_dr_pairs_def": dict(table="la_trace_couplings_all", how="count", where=("definitional", "True"), fmt="int"),
    # tables/la_driver_verdicts.csv: the verdict classes, counted
    "tk_dr_n_single": dict(table="la_driver_verdicts", how="count", where=("verdict class", "single driver"), fmt="int"),
    "tk_dr_n_over": dict(table="la_driver_verdicts", how="count", where=("verdict class", "overdetermined"), fmt="int"),
    "tk_dr_n_opposed": dict(table="la_driver_verdicts", how="count", where=("verdict class", "opposed drivers"), fmt="int"),
    "tk_dr_n_still": dict(table="la_driver_verdicts", how="count", where=("verdict class", "trace does not move"),
                          fmt="int"),
    "tk_dr_n_fixed": dict(table="la_driver_verdicts", how="count",
                          where=("verdict class", "moves, but only fixed physical drivers predict this trace"), fmt="int"),
    "tk_dr_n_against": dict(table="la_driver_verdicts", how="count",
                            where=("verdict class", "moved against every driver that predicts a direction"), fmt="int"),
    "tk_dr_n_mixed": dict(table="la_driver_verdicts", how="count", where=("verdict class", "mixed by class"), fmt="int"),
    "tk_dr_n_unlab": dict(table="la_driver_verdicts", how="count", where=("verdict class", "not labelled"), fmt="int"),
    "tk_dr_n_rows": dict(table="la_driver_verdicts", how="rows", fmt="int"),
...
TAKEAWAY: Dict[str, str] = {
    ...
    # Appendix C
    "la_open_questions": (
        "{tk_open_questions_n} questions were asked that the patent record cannot answer, and none "
        "of them is unanswered for want of trying: what stops each one is the record — a cell too "
        "thin to read, a field no patent states, a status with no date, a dimension no label "
        "carries — and the last column names the record that would carry it. All {tk_open_weak_n} "
        "items graded weak — {tk_open_weak_placed_n} in the running order and "
        "{tk_open_weak_unplaced_n} built but placed in no section — are accounted for on the list, "
        "as the attempt behind a question or, in the closing row of their group, as a restatement "
        "of an item that stays."),
    ...
    "a2_d13_flagship_check": (
        ...
        "listed hold three or more. Every firm-level class statement in Chapter 2 is a mix, never "
    ...
    "la_dd_conditions": (
        ...
        "before this corpus was read. That is what makes 2.1.3.3 a test whose answer could have "
    ...
    "la_lead_lag_class": (
        ...
        "(CVT {tk_cvt_50}) are the ones still growing in 2.1.2, and the earliest (Tilt Rotor "
    ...
    # added 2026-09-23 with Figure 2.1.6b and Table 2.1.6b
    "filer_weight": (
    ...
    "coverage": (
        ...
        "the next {tk_seg_59_firms} hold {tk_seg_59_air}, and the {tk_firms_all} firms that filed hold "
        "{tk_named_share} of the set between them — but market standing does pick the filers out: "
    ...
    "filers_over_time": (
        ...
        "Normalised, panel (ii) says what the aircraft counts of 2.1.2 cannot: in 2020-23 "
        "{tk_ent_slc_2023} of the entering firms enter with Lift + Cruise against "
        "{tk_air_slc_2023} of the window's aircraft, and {tk_ent_tr_w1} of the earliest entrants "
        "came in with Tilt Rotor against {tk_air_tr_w1} of its aircraft. The early entrants ran "
        "far ahead of the field; the 2020-23 entrants only a few points ahead, so arrival "
        "explains the early shift much better than the recent one. (Before the filer rule was "
        "corrected on 2026-09-23 this gap read 14 pp instead of 4: the firm population was then "
        "the ~85 firms of a hand-written list, which is itself weighted towards the well-known "
        "Lift + Cruise startups.)"),
    "la_filers_by_window": (
    ...
    "atlas_filers": (
        ...
        "aircraft and the Lorenz curve stays close to the diagonal. {tk_firms_all} firms hold "
        "{tk_named_share} of the set and the rest is individual inventors, and two thirds of those "
        "firms hold a single aircraft, which is why no firm-level result here can stand for the "
        "field."),
    "la_class_concentration": (
        ...
        "together hold about seven aircraft in ten of each of the large classes, the rest being "
        "individual inventors' filings. No class in this corpus is one company's product line."),
    "firm_tiles": (
    ...
    "a2_d8_filer_mix": (
        ...
        "type off company_canonical: its 'unattributed or independent' row is the band Chapter 2 "
        "no longer draws, and the two are reconciled in the Source line of Figure 2.3.2b."),
    "a2_d8_concentration": (
    ...
    "la_filer_mix": (
        ...
        "per aircraft — the two units give the same answer. What moved is the answer itself: "
        "while the filer type was read off the canonical company list, a fifth of the set sat in "
        "a band called 'unattributed' that held no unattributed patent at all, and the corporate "
        "share read under half. Read off the assignee string the patent actually names, most of "
        "the corpus belongs to an organisation — but to a great many small ones."),
    "la_cohort_mix": (
        ...
        "aircraft. Entrants over-index on the class of their moment, which is what makes 2.1.2 a "
        "population effect rather than a set of firms redesigning."),
    # ``filer_weight`` and ``la_class_filer_weight`` are entered above, with Figure 2.1.6b
    "la_firm_leverage": (
    ...
    "region_grid_b": (
        "Filer type is the one thing in this pair that region decides, and it is why the figure now "
        "holds it alone: North America is the region of named companies while Asia-Pacific carries "
        "the universities. The two variables that left said nothing here. Powertrain could not — "
        "electric is the gate this corpus is selected on, so the panel could only ever show "
        "electric against not-stated, and the not-stated share is a finding about the patent text, "
        "made in the powertrain section. Ducting genuinely does not vary by region or over time, "
        "but it varies strongly with the number of propulsive units, and it is drawn against rotor "
        "count in panel (iii) of the propulsive-units figure. Read this panel for who files, not "
        "for what they build."),
    "specialisation": (
    ...
    "atlas_units": (
        ...
        "{units_n} aircraft put their units on exactly two stations. Panel (iii) is the reason "
        "ducting is drawn here and not against region: ducting does not follow the map or the "
        "calendar, it follows rotor count, and not in a straight line. It falls from "
        "{tk_duct_b13} of the aircraft with three or fewer units to {tk_duct_b78} at seven or "
        "eight, then jumps to {tk_duct_b9} at nine or more — highest where the rotors are fewest "
        "and where they are most numerous, rarest in the middle, against {tk_duct_all} over the "
        "whole corpus. The design statement is that a duct is bought for two different reasons at "
        "the two ends of the range: at low counts to make a few large rotors safe to stand beside "
        "and to turn a rotor into a usable thrust unit, and at high counts because a rotor small "
        "enough to be repeated nine or more times is also small enough for a ring to pay for its "
        "own weight — and because nine open rotors in close company interfere. The middle of the "
        "range, four to eight open rotors on booms, is the arrangement that buys neither. The "
        "split is not the class mix in disguise: the middle dip is there inside Lift + Cruise, "
        "Tilt Rotor and Combined vectored thrust taken one at a time, and the figure's own "
        "How-to-read line reports the test with the class held fixed. The class reading sits in "
        "panel (ii): {tk_duct_cls_hi_name} ducts "
        "{tk_duct_cls_hi} of {tk_duct_cls_hi_n} aircraft against {tk_duct_cls_lo} of "
        "{tk_duct_cls_lo_n} for {tk_duct_cls_lo_name}."),
    ...
    # ================================================================ 2.4 design drivers (2026-09-23)
    "trace_drift": (
        "What moves, moves inside the tilting classes: Tilt Rotor goes from a median of {tk_dr_tr_units_first} "
        "propulsive units to {tk_dr_tr_units_last} and CVT from {tk_dr_cvt_units_first} to {tk_dr_cvt_units_last}, "
        "and their tilting joint groups rise with them (TR {tk_dr_tr_joints_mov}, CVT {tk_dr_cvt_joints_mov}), "
        "while Lift + Cruise, which started at {tk_dr_slc_units_first} units and zero joints, stays where it "
        ...
        "Read every slope with the test in Table 2.4.3a — the picture shows candidates, the table which "
        "survive."),
    "trace_couplings": (
        "Of the four couplings the physical drivers predict, one is there: empennage type against the wing "
        "configuration (V {tk_dr_emp_wing_v} with the wing count, {tk_dr_emp_pos_v} with the wing position), "
        "in the strongest tenth of all pairs — a canard or tandem layout goes tailless, a single wing takes a "
        "tail. The other three are no stronger than a random pair of fields: boom presence against the wing "
        "(V {tk_dr_hover_v}), units on the wing against its planform ({tk_dr_plan_v}), and the position of the "
        "wing units against retraction ({tk_dr_retract_v}). The strongest pairs in the matrix are the "
        "codebook's own construction and are excluded, never reported."),
    "la_trace_couplings": (
        "Cruise stability and control leaves the one coupling the record confirms; hover load path, wing "
        "limits and drag leave none that a random pair would not. A fixed driver whose predicted coupling is "
        "absent is not refuted — the coupling may sit in a field the label set does not carry (rotor "
        "diameter, wing loading) — but it leaves no trace here."),
    "la_driver_verdicts": (
        "{tk_dr_n_rows} traces, and {tk_dr_n_single} sharp statements about a moving driver: units carried on "
        "booms and the stations carrying units rise with distributed propulsion alone (hover load path predicts "
        "the coupling, not the trend). {tk_dr_n_over} traces are overdetermined — propulsive units by three "
        "drivers, ground contact by three — so the sector moved and the record cannot say why. "
        "{tk_dr_n_opposed} traces have opposed drivers, and both went with regime "
        "transition or occupancy rather than with cost: tilting joints rise inside the tilting classes, "
        "moving fuselages fall. {tk_dr_n_against} moved against the only direction a driver gave (propulsor "
        "types rise where cost predicts fewer; ducting falls in CVT where noise predicts more). "
        "{tk_dr_n_still} do not move at all, {tk_dr_n_fixed} move with only fixed drivers behind them, "
        "{tk_dr_n_mixed} moves both ways by class, and {tk_dr_n_unlab} are not labelled. These are facts "
        "about the traces, not answers about the drivers; what makes them interesting is how few of them "
        "point at one driver."),
    "la_driver_questions": (
        "The three answers in one line each: the propulsor-count rise is a tilting-class rise (TR, CVT), "
        "not a Multirotor one and not general; retraction inside Lift + Cruise is present at "
        "{tk_dr_slc_retract_last} and {tk_dr_slc_retract_mov}; tilting joints rise inside the tilting classes "
        "(TR {tk_dr_tr_joints_mov}, CVT {tk_dr_cvt_joints_mov}) and the pooled row is {tk_dr_all_joints_mov}, "
        "because the class that grows has none — transition wins inside a class, cost wins in the mix."),
}
...
def _tk_number(spec: Dict, tables: Optional[Dict]) -> Optional[str]:
    ...
    try:
        ...
        if how == "mean":
            # The average of ``col`` over the matching rows, weighted by ``weight`` when the
            # spec names a column (a table of group summaries needs the group sizes to give
            # the mean over the underlying units, not the mean of the group means). Added
            # 2026-09-23 for the region-proximity takeaways; no earlier spec uses it, so this
            # branch cannot change any number the document printed before.
            wcol = spec.get("weight")
            rows = [(x, (_tk_float(r.get(wcol)) if wcol else 1.0) or 0.0)
                    for r in sel for x in [_tk_float(r.get(col))] if x is not None]
            den = sum(w for _, w in rows)
            return _tk_fmt(sum(x * w for x, w in rows) / den, fmt) if den else None
        if how in ("max", "min"):
...
# --------------------------------------------------------------------------
# question — which of the eight sector questions an item answers, and in what way
# --------------------------------------------------------------------------
# Author's ruling 2026-09-23: "we need to start saying which question each graph answers,
# and in each way, and say which graphs are not important". Every figure and every table
# therefore prints a third grey line:
#
#     Question: Q3 what is gaining and what is dying — direct
#
# The eight questions are questions about the eVTOL SECTOR, not the four research
# questions of the methodology framework (Part G.1). RQ1-RQ3 are folded into Q1-Q5;
# RQ4, "does the figure alone carry the architecture the text states", is methodological
# and is carried here as ``M`` so that it is never counted as a sector finding.
#
# The four ways an item can serve a question, and nothing else is allowed:
#   direct    the item IS the answer, or a substantial part of it
#   premise   it establishes something the answer needs — a base, a level, a definition
#   caveat    it bounds what may be claimed — truncation, lag, thin cells, office effect
#   describes it describes the data set only; no question rests on it
#
# Keyed by the internal figure or table name, never by the printed number, so a section
# being renumbered, reordered or split cannot break it. ONE ENTRY PER KEY.
# The full reasoning, the representativeness numbers and the cut list live beside the
# rendered document in ``QUESTION_MAP.md``.

#: printed for an item that has not been mapped to a question yet — honest, never a guess
NO_QUESTION = "not mapped to a question yet — add it to QUESTION in la_index.py"

#: the eight sector questions, plus the methodological marker. Proposal of 2026-09-23 —
#: the author confirms or rewrites them; the keys are what the per-item register uses.
QUESTIONS: Dict[str, str] = {
    "Q1": "What is being designed — which architectures the patent record holds, in what "
          "proportion, and which label dimensions actually separate one design from another",
    "Q2": "Is the space converging on a dominant design, or still branching",
    "Q3": "Which architectures are gaining and which are being abandoned, and when",
    "Q4": "Who designs it — firms, individuals, universities; who enters and who stays",
    "Q5": "Where is it designed, and does region change what is designed",
    "Q6": "Is any of this one company, or one design filed many times",
    "Q7": "Does the patent record describe the aircraft the industry actually builds",
    "Q8": "What is the patent record worth as an indicator of the sector — how early, how "
          "complete, how live",
...
#: answer and is printed as such, because an item that answers no question is exactly what
#: the author asked to be shown.
QUESTION: Dict[str, str] = {
    # ---------------------------------------------------------------- front matter
    "la_flags": "none — editorial list, not a measurement",
    "la_open_questions": "Q1-Q8 every sector question — caveat (what the record cannot answer, and why)",

    # ================================================================ 1 data sets
    "fig_02_refinement_funnel": "Q8 indicator value — premise (what the 665 are)",
    "atlas_removal": "Q8 indicator value — caveat (the domain boundary is a reading)",
    "a2_d1_similars": "Q8 indicator value — caveat (where a reviewer can push)",
    "a2_d9_publication_lag_region": "Q8 indicator value — caveat (fixes the 2023 cut-off)",
    "a2_d1_filing_status": "Q8 indicator value — premise (the three patent levels)",
    "atlas_duplicates": "Q6 one company or one design — premise (repetition is removed)",
    "a2_d7_duplicates": "Q6 one company or one design — premise (de-duplication is arithmetic)",
    "a2_d7_aircraft_per_patent": "Q6 one company or one design — premise (1 patent = 1 aircraft)",
    "atlas_figure_approval": "M figure vs text — caveat (29 % rest on one figure)",
    "a2_d11_figure_approval": "M figure vs text — premise (a whole-vehicle set by construction)",

    # ================================================================ 2 taxonomy
    "design_space_cards": "Q1 what is designed — premise (duplicates the class drawing)",
    "codebook_classes": "Q1 what is designed — premise (the twelve classes defined)",
    "codebook_dimensions": "Q1 what is designed — premise (the 42 slots defined)",

    # ================================================================ 3 quality
    "atlas_fill": "none — data quality; bounds nothing that is claimed",
    "a2_d4_missingness": "none — data quality; restates the figure",
    "la_relabel_protocol": "M figure vs text — caveat (the reliability number still owed)",
    "atlas_flagship": "Q7 patents vs what is built — direct (12 of 19 firms match)",
    "a2_d13_flagship_check": "Q7 patents vs what is built — direct (restates the figure)",
    "la_name_arch_check": "M figure vs text — direct (51 aircraft where sources disagree)",
    "a2_d3_selected_fields": "Q1 what is designed — direct (the modal design)",
    "atlas_fields": "Q1 what is designed — premise (label depth is a design property)",
    "a2_d2_informative_fields": "Q1 what is designed — premise (24 fields carry information)",
    "atlas_arch_gt": "M figure vs text — direct (where the figure alone fails)",

    # ================================================================ 2.1
    "filings_per_year": "Q8 indicator value — direct (eVTOL against all B64 filing)",
    "la_filings_per_year": "Q8 indicator value — caveat (which years are complete)",
    "atlas_arch_time": "Q3 what is gaining — direct (the class mix per window)",
    "a2_d9_architecture_by_window": "Q3 what is gaining — direct (the same, in numbers)",
    "firm_weighted": "Q6 one company or one design — direct (one vote per filer)",
    "la_firm_weighted_shares": "Q6 one company or one design — direct",
    "two_counts": "Q6 one company or one design — caveat (re-filing moves nothing)",
    "la_archetype_levels": "Q2 converging — premise (the level decides the answer)",
    "la_archetype_choice": "Q2 converging — premise (which levels can carry a species)",
    "la_dd_conditions": "Q2 converging — premise (the thresholds, fixed in advance)",
    "dominant_design": "Q2 converging — direct (conditions 1 and 2)",
    "dominant_design_q": "Q2 converging — direct (condition 3)",
    "la_dd_result": "Q2 converging — direct (the verdict: no dominant design)",
    "la_dominant_design": "Q2 converging — direct (top share and balance per window)",
    "la_dd_q": "Q2 converging — direct (the spread of the space per window)",
    "class_configs": "Q2 converging — direct (no settling inside a class either)",
    "la_class_configs": "Q2 converging — direct (restates the figure)",
    "dimension_drift": "Q1 what is designed — direct (units and booms rise, ducting flat)",
    "la_dimension_drift": "Q1 what is designed — direct (restates the figure)",
    "class_cycles": "Q3 what is gaining — direct (each class's lifetime)",
    "la_class_cycles": "Q3 what is gaining — direct (restates the figure)",
    "lead_lag": "Q5 where — caveat (the 2-year gap is the publication lag)",
    "la_lead_lag_region": "Q5 where — caveat (region ordering, small)",
    "la_lead_lag_class": "Q3 what is gaining — restates the class shares one window coarser",
    "la_class_region_timing": "Q5 where — direct (which region took a class up first)",
    "hill": "Q2 converging — direct, but no window separates from any other",
    "la_hill_by_window": "Q2 converging — direct, every band overlaps every other",
    "zones": "Q1 what is designed — direct (the crowded core and the empty rim)",
    "la_zones": "Q1 what is designed — direct (restates the figure)",
    "filer_weight": "Q6 one company or one design — direct (how much is one firm)",
    "la_class_filer_weight": "Q6 one company or one design — direct (effective filers per class)",
    "abandonment": "Q8 indicator value — direct (46 % no longer in force)",
    "la_abandonment_by_class": "Q8 indicator value — direct (the base rate, not the class)",

    # ================================================================ 2.2
    "coverage": "Q4 who designs it — direct (no top tier; 85 firms hold 47 %)",
    "la_market_vs_patents": "Q7 patents vs what is built — direct (rated firms file more)",
    "la_coverage_segments": "Q4 who designs it — direct (restates the curve)",
    "la_firm_tiers": "Q4 who designs it — direct (restates the curve)",
    "filers_over_time": "Q4 who designs it — direct (the field industrialises)",
    "la_filers_by_window": "Q4 who designs it — direct (entry dominates every window)",
    "la_cohorts": "Q4 who designs it — direct (firm cohorts)",
    "la_cohort_mix": "Q3 what is gaining — direct (the shift arrives with new firms)",
    "atlas_filers": "Q4 who designs it — direct (filing is spread, not concentrated)",
    "firm_tiles": "Q4 who designs it — direct (each firm's own window trace)",
    "firm_influence": "Q6 one company or one design — direct (leave-one-firm-out)",
    "la_firm_leverage": "Q6 one company or one design — direct (largest shift 2.6 pp)",
    "la_class_concentration": "Q6 one company or one design — direct",
    "la_filer_mix": "Q4 who designs it — direct (under half the corpus is a company)",
    "a2_d8_filer_mix": "Q4 who designs it — direct (the same split, per patent)",
    "a2_d8_concentration": "none — name-cleaning artefact, a data-quality note",
    "transitions": "Q4 who designs it — direct (firms deepen, they do not explore)",
    "la_transitions": "Q4 who designs it — direct, but 31 of 55 cells hold 1-2 pairs",
    "ip_strategy": "Q4 who designs it — direct (no single IP strategy)",
    "la_ip_strategy": "Q4 who designs it — direct (depth against breadth, 17 firms)",
    "la_ip_by_class": "none — citations follow age, not class; its own takeaway says so",
    "spans_by_firm": "Q6 one company or one design — caveat (re-filing changes nothing)",
    "proximity_region": "Q5 where — direct, and the answer is null (region does not organise)",
    "a2_d14_firm_proximity": "Q5 where — direct (per-firm proximity)",
    "la_proximity_region": "Q5 where — direct, null result on 136 pairs from 17 firms",
    "ari": "Q7 patents vs what is built — direct (the commercial end of the field)",
    "ari_history": "Q7 patents vs what is built — the index moves with funding, not filings",
    "ari_clock": "Q8 indicator value — direct (the patent clock leads the market clock)",
    "la_ari_firms": "Q7 patents vs what is built — direct (the 18 index firms)",
    "la_ari_correlation": "none — 57 tests on 9-18 firms, 0 survive the correction",
    "la_ari_gap": "Q7 patents vs what is built — direct (how the rated firms differ)",
    "la_ari_timeline": "Q8 indicator value — direct (years from first patent to first flight)",
    "la_ari_clock_region": "Q5 where — direct, but no row rests on more than 5 firms",
    "la_trl_placeholder": "Q7 patents vs what is built — nothing measured; the data exists",

    # ================================================================ 2.3
    "atlas_region": "Q5 where — direct (three countries hold two thirds)",
    "region_grid": "Q5 where — direct (North America keeps the tilting architectures)",
    "region_grid_b": "Q4 who designs it — direct, by region (powertrain and ducting left the figure)",
    "specialisation": "Q5 where — direct, readable only above ~10 aircraft per archetype",
    "la_specialisation": "Q5 where — direct (restates the figure)",
    "country_class": "Q5 where — direct (the US tilts, China lifts and cruises)",
    "la_country_class": "Q5 where — direct (restates the figure)",

    # ================================================================ 5
    "atlas_powertrain": "Q1 what is designed — restates the electric gate; 2.1.5 (v) has it per window",
    "la_powertrain_by_class": "Q1 what is designed — restates the figure",
    "atlas_units": "Q1 what is designed — direct (no standard rotor count; ducting is U-shaped in it)",
    "atlas_design_heatmaps": "Q1 what is designed — only class x tail type; the rest repeats the class",
    "atlas_state_by_arch": "M figure vs text — caveat (the figures draw end states)",
    "a2_d2_figure_slot_answers": "M figure vs text — describes the image set",
    "mission": "none — 95 self-selected aircraft; the mission field is not the corpus",
    "linked_check": "Q7 patents vs what is built — caveat (the linked set is NOT the corpus)",
    "la_class_fold": "Q7 patents vs what is built — premise (the fold is lossy)",
    "la_mission_capacity": "none — 95 aircraft over 5 folded classes, cells of 0-12",
    "la_mission_piloting": "none — mostly unstated; cannot be used as a variable",
    "la_mission_power source": "Q7 patents vs what is built — confirms the electric gate only",
    "la_mission_status": "Q7 patents vs what is built — direct (a patent describes a concept)",
    "la_mission_agreement": "M figure vs text — direct (the folded label matches the directory)",
    "industry_by_class": "none — 345 of 665 unspecified; the classes do not separate",
    "la_industry_by_class": "none — restates a text classifier the author cannot verify",
    "examination": "Q8 indicator value — direct on the office panel; the class panel is null",
    "la_examination_office": "Q8 indicator value — direct (offices differ, designs do not)",
    "la_examination_class": "none — every class sits at the same grant rate",

    # ================================================================ 2.4 design drivers (2026-09-23)
    "trace_drift": "Q1 what is designed — direct (which traces move, inside which class)",
    "trace_couplings": "Q1 what is designed — direct (which predicted couplings the record carries)",
    "la_trace_couplings": "Q1 what is designed — direct (restates the figure with the percentiles)",
    "la_driver_verdicts": "Q1 what is designed — direct (what moved, and how many drivers predict it)",
    "la_driver_questions": "Q1 what is designed — direct (three answers, generated)",
}


def question(name: str, values: Optional[Dict] = None, kind: str = "figure",
             tables: Optional[Dict] = None) -> str:
    """The third grey line under figure or table ``name``: which of the eight sector
    questions it answers, and in what way.

    ``name`` is the internal key, so the line follows the item through a renumbering. An
    item with no :data:`QUESTION` entry prints the marker rather than an invented mapping,
    so a figure added later is visibly unclassified instead of silently claiming a question.
    """
    text_ = QUESTION.get(name)
    if not text_:
        return NO_QUESTION
    line = _clean(_pa._fill(str(text_), dict(values or {})))
    return line or NO_QUESTION


# --------------------------------------------------------------------------
# grade — the degree of importance of an item, printed on the question line
# --------------------------------------------------------------------------
# Author's ruling 2026-09-23: the question line says in what WAY an item serves a question
# (direct / premise / caveat / describes) but not HOW MUCH the answer owes it. The grade
# says that, and it is printed as the tail of the same line so the document does not grow a
# fourth grey line:
#
#     Question: Q3 what is gaining — direct (each class's lifetime) · weak, low
#     representativeness (7 of 12 classes have 3+ window cells under 5 aircraft)
#
# THE SCALE — four levels, and nothing else is allowed:
#
#   core          the item a question's answer rests on: it is direct evidence and the map
#                 keeps it. Remove it and the answer loses a substantial part of itself.
#   supporting    apparatus the answer needs but is not: a premise, a caveat, or a
#                 description of the data set. The map keeps it, sometimes in an appendix.
#   weak, low representativeness   the n cannot carry the claim — too few units, a
#                 self-selected base, or cells too thin to read. Always names its number.
#   weak, low insight              it restates another item, it is tautological, or the
#                 finding is null and nobody needs it. Always names its reason.
#
# HOW A GRADE IS DERIVED — from ``QUESTION_MAP.md`` beside the rendered document, never
# from taste, so that this register and anything else that selects on it read the same map:
#
#   1. the map's §3 is the weak list. An item listed in §3(a) grades "weak, low
#      representativeness"; an item listed in §3(b) grades "weak, low insight"; an item in
#      both takes the §3(a) reason, because that is where the number is. The one exception
#      is a §3 row whose own ruling is to keep the item WHOLE — §3(a).12 (the flagship
#      figure: 19 firms, but every matchable firm, not a sample) and §3(a).5 (the proximity
#      figure: a legitimate null answer to Q5). Those two keep their earned grade.
#   2. otherwise, an item the map's §2 cuts, merges, replaces or demotes to a sentence
#      grades weak, with §2's own reason — low insight when the reason is restatement,
#      tautology or an uninteresting null, low representativeness when the reason is n.
#      A verdict of "appendix" alone is about PLACEMENT, not worth, and does not make an
#      item weak unless §3 also lists it.
#   3. otherwise, an item whose way is "direct" grades core.
#   4. otherwise it grades supporting.
#
# The reason in brackets is a few words and carries the map's number wherever the map gives
# one. Keyed by internal name, ONE ENTRY PER KEY, exactly like PROVENANCE, TAKEAWAY and
# QUESTION, so a renumbering cannot break it. Printed numbers of other sections are never
# quoted here — an item is referred to by what it draws, so the reorder of 2026-09-23
# cannot make a grade lie.

#: printed for an item that has not been graded yet — honest, never a guess
NO_GRADE = "not graded yet — add it to GRADE in la_index.py"

#: the scale, in one place: grade token -> what it means. The register below uses the token
#: as written here, with the weak levels carrying their reason in brackets.
GRADES: Dict[str, str] = {
    "core": "the item a question's answer rests on — direct evidence the answer cannot do "
            "without",
    "supporting": "apparatus the answer needs but is not — a premise, a caveat, or a "
                  "description of the data set",
    "weak, low representativeness": "the n cannot carry the claim — too few units, a "
                                    "self-selected base, or cells too thin to read",
    "weak, low insight": "it restates another item, it is tautological, or the finding is "
                         "null and nobody needs it",
}

#: internal item name -> grade. See the derivation rules above; every weak grade names its
#: reason, with the map's number where the map gives one.
GRADE: Dict[str, str] = {
    # ---------------------------------------------------------------- front matter
    "la_flags": "supporting",
    # Appendix C. Same day, nine weak reasons further down were recomputed against the tables built
    # after the filer-rule correction (entrants 15 / 19, transitions 31 of 55 cells, 57 ARI tests,
    # clock 3 of 9 rows, countries 4 of 8): Appendix C quotes them, so they had to be true first.
    "la_open_questions": "supporting",

    # ================================================================ data sets (App. A)
    "fig_02_refinement_funnel": "supporting",
    "atlas_removal": "supporting",
    "a2_d1_similars": "supporting",
    "a2_d9_publication_lag_region": "supporting",
    "a2_d1_filing_status": "supporting",
    "atlas_duplicates": "supporting",
    "a2_d7_duplicates": "supporting",
    "a2_d7_aircraft_per_patent": "weak, low insight (one number, 517 of 574; the funnel prints it)",
    "atlas_figure_approval": "supporting",
    "a2_d11_figure_approval": "supporting",

    # ================================================================ taxonomy
    "design_space_cards": "weak, low insight (nothing the twelve-class figure does not draw)",
    "codebook_classes": "supporting",
    "codebook_dimensions": "supporting",

    # ================================================================ quality (App. B)
    "atlas_fill": "supporting",
    "a2_d4_missingness": "weak, low insight (restates the fill figure; worst field 5 % blank)",
    "la_relabel_protocol": "weak, low insight (a protocol with no data; one sentence in the limitations)",
    "atlas_flagship": "core",
    "a2_d13_flagship_check": "weak, low insight (nothing the flagship figure does not show)",
    "la_name_arch_check": "core",
    "a2_d3_selected_fields": "core",
    "atlas_fields": "supporting",
    "a2_d2_informative_fields": "weak, low insight (restates the label-depth figure, 24 of 42 fields)",
    "atlas_arch_gt": "core",

    # ================================================================ what is designed
    "filings_per_year": "core",
    "la_filings_per_year": "supporting",
    "atlas_arch_time": "core",
    "a2_d9_architecture_by_window": "weak, low insight (identical content to the class-mix figure)",
    "firm_weighted": "weak, low insight (the kept filer-weight figure carries the one-vote-per-filer check)",
    "la_firm_weighted_shares": "weak, low insight (the kept filer-weight table carries the one-vote-per-filer check)",
    "two_counts": "weak, low insight (the two-count check moves no class share by more than 0.03)",
    "la_archetype_levels": "supporting",
    "la_archetype_choice": "supporting",
    "la_dd_conditions": "supporting",
    "dominant_design": "core",
    "dominant_design_q": "core",
    "la_dd_result": "core",
    "la_dominant_design": "core",
    "la_dd_q": "core",
    "class_configs": "core",
    "la_class_configs": "core",
    "dimension_drift": "core",
    "la_dimension_drift": "core",
    "class_cycles": "weak, low representativeness (7 of 12 classes have 3+ window cells under 5 aircraft)",
    "la_class_cycles": "weak, low insight (identical content to the class-lifetime figure)",
    "lead_lag": "supporting",
    "la_lead_lag_region": "supporting",
    "la_lead_lag_class": "weak, low insight (7 class medians span 2018-2020 against a 3-year publication lag)",
    "la_class_region_timing": "core",
    "hill": "weak, low insight (0 of 60 window pairs separate at 95 %)",
    "la_hill_by_window": "weak, low insight (no power at this n; the dominant-design test carries it)",
    "zones": "core",
    "la_zones": "weak, low representativeness (11 of 34 rows under 10 aircraft; the figure carries it)",
    "filer_weight": "core",
    "la_class_filer_weight": "core",
    "abandonment": "core",
    "la_abandonment_by_class": "weak, low insight (its own takeaway says the class column is not the finding)",

    # ================================================================ who designs it
    "coverage": "core",
    "la_market_vs_patents": "core",
    "la_coverage_segments": "weak, low insight (the coverage curve carries every segment)",
    "la_firm_tiers": "weak, low insight (every threshold is arbitrary; the curve has no elbow)",
    "filers_over_time": "weak, low representativeness (15 and 19 entering firms in the first two windows)",
    "la_filers_by_window": "core",
    "la_cohorts": "weak, low representativeness (15 and 19 entering firms in the first two windows)",
    "la_cohort_mix": "weak, low representativeness (15 and 19 entrants in the first two windows, in class cells of 1-8 firms)",
    "atlas_filers": "core",
    "firm_tiles": "weak, low representativeness (few firms reach 5 aircraft inside one window)",
    "firm_influence": "core",
    "la_firm_leverage": "weak, low insight (merges into the leave-one-firm-out figure)",
    "la_class_concentration": "weak, low insight (the effective-filers table answers it)",
    "la_filer_mix": "core",
    "a2_d8_filer_mix": "weak, low insight (the aircraft-unit table carries the split, on the newer filer field)",
    "a2_d8_concentration": "weak, low insight (HHI 0.006 is a name-cleaning artefact, not the sector)",
    "transitions": "weak, low representativeness (31 of 55 cells hold 1-2 pairs)",
    "la_transitions": "weak, low representativeness (35 pairs spread over 31 cells of 1-2; only the diagonal reads)",
    "ip_strategy": "core",
    "la_ip_strategy": "core",
    "la_ip_by_class": "weak, low insight (a table whose own takeaway says not to read it)",
    "spans_by_firm": "weak, low insight (the re-filing check moves no class share by more than 0.03)",
    "proximity_region": "core",
    "a2_d14_firm_proximity": "weak, low insight (class-level closeness is the coarseness of the class)",
    "la_proximity_region": "weak, low representativeness (the Asia-Pacific box is 6 pairs from 4 firms)",
    "ari": "core",
    "ari_history": "weak, low representativeness (the funding panel rests on 9 firms of 18)",
    "ari_clock": "core",
    "la_ari_firms": "core",
    "la_ari_correlation": "weak, low representativeness (26 of the 57 rows rest on 9 firms)",
    "la_ari_gap": "core",
    "la_ari_timeline": "core",
    "la_ari_clock_region": "weak, low representativeness (3 of 9 rows rest on one firm)",
    "la_trl_placeholder": "weak, low insight (a promise, not a measurement; the TRL table is computed)",

    # ================================================================ where
    "atlas_region": "core",
    "region_grid": "core",
    "region_grid_b": "core",
    "specialisation": "weak, low representativeness (11 of 34 archetypes hold under 10 aircraft)",
    "la_specialisation": "weak, low representativeness (11 of 34 archetypes hold under 10 aircraft)",
    "country_class": "weak, low representativeness (4 of 8 countries hold under 25 aircraft)",
    "la_country_class": "weak, low representativeness (4 of 8 countries hold under 25 aircraft)",

    # ================================================================ low-level
    "atlas_powertrain": "weak, low insight (485 of 665 electric restates the inclusion gate)",
    "la_powertrain_by_class": "weak, low insight (restates a figure that is itself the inclusion gate)",
    "atlas_units": "core",
    "atlas_design_heatmaps": "weak, low insight (5 of 6 panels are empty by construction or repeat the class)",
    "atlas_state_by_arch": "weak, low insight (a property of the drawings, not of the sector)",
    "a2_d2_figure_slot_answers": "weak, low insight (a property of the drawings; it bounds RQ4 only)",
    "mission": "weak, low representativeness (95 of 665, fails 4 of 6 tests)",
    "linked_check": "supporting",
    "la_class_fold": "supporting",
    "la_mission_capacity": "weak, low representativeness (95 of 665, a base that fails 4 of 6 tests)",
    "la_mission_piloting": "weak, low representativeness (95 of 665, and mostly unstated even there)",
    "la_mission_power source": "weak, low representativeness (95 of 665, a self-selected base)",
    "la_mission_status": "weak, low representativeness (95 of 665; one line carries it, not a table)",
    "la_mission_agreement": "core",
    "industry_by_class": "weak, low insight (the source is a text classifier the author cannot verify)",
    "la_industry_by_class": "weak, low insight (restates a figure that is itself null)",
    "examination": "core",
    "la_examination_office": "core",
    "la_examination_class": "weak, low insight (a null: examination says nothing about architecture)",

    # ================================================================ 2.4 design drivers (2026-09-23)
    # The verdict table is the reading the author asked for and rests on the drift figure; the two
    # are core. The coupling test is one positive and three nulls — an honest negative that the
    # verdicts quote — and its table restates the figure with the percentiles: supporting, both.
    "trace_drift": "core",
    "trace_couplings": "supporting",
    "la_trace_couplings": "supporting",
    "la_driver_verdicts": "core",
    "la_driver_questions": "core",
}


def grade(name: str, values: Optional[Dict] = None, kind: str = "figure",
          tables: Optional[Dict] = None) -> str:
    """The degree of importance of figure or table ``name``, printed as the tail of the
    question line: ``core``, ``supporting``, or one of the two weak grades with its reason.

    ``name`` is the internal key, so the grade follows the item through a renumbering. An
    item with no :data:`GRADE` entry prints the marker rather than an invented weight, so a
    figure added later is visibly ungraded instead of silently claiming to be core. The
    scale itself is documented once, in :data:`GRADES`.
    """
    text_ = GRADE.get(name)
    if not text_:
        return NO_GRADE
    line = _clean(_pa._fill(str(text_), dict(values or {})))
    return line or NO_GRADE


# --------------------------------------------------------------------------
# Appendix C — the questions asked, and what the corpus could not answer
# --------------------------------------------------------------------------
# Author's ruling 2026-09-23: "the graphs and tables I discarded also are important to show to my
# professor ... there are important questions that were not capable of being answered, and I
# will ask you to put in this document those questions — for me to know that all the questions
# asked were interesting, and that there are no more questions interesting to ask that are not
# asked just by negligence."
#
# So Appendix C is ONE ROW PER QUESTION, not per item. ``la_tables.open_questions`` builds the
...
#                            tried to answer it (internal names, resolved to "Figure 2.1.7" /
#                            "Table 2.2.7b" through ``item_numbers()`` at build time), the reason the
...
#   * ``UNLABELLED_DRIVERS`` the slot for the design-drivers section: drivers whose trace no label
#                            carries. A row added there prints under "Q1 · design drivers" with no
#                            change to the builder.
#   * the closing row of each group is generated: every item GRADE marks weak that no entry cites
#     is named there with its reason, so every weak item is accounted for exactly once.
#
# Fields of an entry: ``q`` the sector question id (a key of QUESTIONS); ``sub`` an optional
# sub-group label printed after the id; ``asked`` the question, as a question; ``items`` the
# internal names of the figures and tables that tried (cited by live number; the weak ones have
# their grade reason quoted); ``tried`` extra text for the second column, e.g. an analysis probed
# and not built; ``cannot`` the reason, with the number; ``would`` the record that would answer it.
# Inside ``asked`` / ``tried`` / ``cannot`` / ``would`` a ``[[name]]`` is replaced by that item's
# live number, so no printed figure or table number is ever typed here.
#
# Every number typed in ``cannot`` / ``would`` was read on 2026-09-23 out of tables/ (the CSVs the
# same render writes) or out of DEEPER_ANALYSES.md beside the rendered document, where the
# rejected analyses are recorded with the probe that killed each. ONE ENTRY PER KEY.


def item_numbers() -> Dict[str, str]:
    """Internal name -> ``"Figure 2.1.7"`` / ``"Table 2.2.7b"`` for every item in the running order,
    ...
    for n in NODES:
        ...
        for k, f in enumerate(figs):
            out[f] = "Figure " + figure_number(n["id"], k, len(figs))
        for k, t in enumerate(tabs):
            out[t] = "Table " + table_number(n["id"], k, len(tabs))
    return out


def sector_questions_line() -> str:
    """The eight sector questions and the method marker in one run of text, straight out of
    :data:`QUESTIONS` — printed by Appendix C, the one place the document spells them out."""
    return " ".join(f"**{k}** {v.rstrip('.')}." for k, v in QUESTIONS.items())


#: internal name -> the question, the items that tried, the reason and the remedy. See the note
#: above for the fields; the order here is the order printed inside each sector question.
OPEN_QUESTIONS: Dict[str, Dict] = {
    # ================================================================ Q1 what is designed
    "secondary_choices_pairing": dict(
        q="Q1",
        asked="Do the secondary choices — tail type, landing gear, ducting, booms — pair up into "
              "standard packages inside a class?",
        items=["atlas_design_heatmaps"],
        cannot="One wing is the default and 69 % of the aircraft draw no landing gear at all, so the "
               "pairings have nothing to pair; the one that carries information, class against tail "
               "type, is read in [[atlas_design_heatmaps]] as it stands.",
        would="A source that states the secondary choices instead of drawing them — specification "
              "sheets or certification data — for more than the 95 aircraft with a directory page; a "
              "patent drawing omits what the claim does not need."),
    "powertrain_as_dimension": dict(
        q="Q1",
        asked="Is the powertrain — battery, hybrid, hydrogen — a dimension that separates one design "
              "from another?",
        items=["atlas_powertrain", "la_powertrain_by_class", "dimension_drift", "la_mission_power source"],
        cannot="Electric propulsion is the inclusion gate of the corpus, so inside it the field cannot "
               "vary: the patent states a powertrain for 579 of the 663 classed aircraft, 485 electric "
               "and 94 hybrid, and the hybrid share per class and window is already drawn in "
               "[[dimension_drift]] (v). The energy store behind the powertrain is not a labelled "
               "dimension — the label set records electric, hybrid or not stated and nothing finer — "
               "and it is stated only on the directory page of the 95 linked aircraft: 57 battery, 18 "
               "hybrid, 3 hydrogen.",
        would="An energy store stated per aircraft, which specification sheets carry for the flown "
              "aircraft and the label set does not record."),
    "design_space_rim": dict(
        q="Q1",
        asked="How is the rim of the design space occupied — the archetypes that hold fewer than ten "
              "aircraft — and is any of them a species in the making?",
        items=["zones", "la_zones"],
        cannot="At the class × propulsive-unit level the corpus forms 43 archetypes; 34 hold five or "
               "more aircraft, and 11 of those hold fewer than ten, so their five window cells hold 0 to "
               "4 aircraft each and neither a zone nor a trend can be read on them. The nine archetypes "
               "below five aircraft are not printed at all.",
        would="The record growing: an archetype of ten aircraft over five windows cannot be read, and "
              "only a corpus several times larger would fill the rim; the crowded core of [[zones]] is "
              "read as it stands."),
    # ---------------------------------------------------------------- Q1 · design drivers
    # The two rejected driver analyses of DEEPER_ANALYSES.md §4. The drivers whose trace is not
    # labelled (rotor diameter, hub mechanism, cabin dimensions, aircraft size, noise itself) are
    # the design-drivers section's rows and go in UNLABELLED_DRIVERS below, not here.
    "battery_energy_vs_disc_loading": dict(
        q="Q1", sub="design drivers",
        asked="Does the rise in battery specific energy explain the move towards more, smaller "
              "propulsors — the disc-loading trade?",
        items=["dimension_drift", "atlas_units"],
        tried="a battery-against-disc-loading analysis, probed 2026-09-23 and not built",
        cannot="Disc loading needs a rotor diameter and a mass, and the label set has no size, "
               "diameter, span, area or mass field anywhere: `boomN_span` is a category and "
               "`wingN_zoneSpan` a zone, and a patent drawing carries no scale. The half that can be "
               "computed — propulsive units per class and window — is [[dimension_drift]] (i) and "
               "[[atlas_units]].",
        would="Rotor diameter and maximum take-off mass per aircraft, from specification sheets or "
              "certification data; a published Wh/kg series beside the filing curve is context, not a "
              "finding of this corpus."),
    "noise_wind_as_drivers": dict(
        q="Q1", sub="design drivers",
        asked="Do noise limits, or wind and gust tolerance, drive the choice of architecture?",
        items=["trace_drift", "la_driver_verdicts"],
        tried="nothing could be built for noise or wind as such — the record holds nothing to build one "
              "from; the question was raised in the review of 2026-09-23. The design-drivers section "
              "(2.4) reads the three traces the author's table gives community noise — propulsor count, "
              "rotor enclosure, rotor diameter — and the first two are in [[trace_drift]] and "
              "[[la_driver_verdicts]]",
        cannot="Neither noise nor wind is a labelled dimension, neither is stated in the patents as a "
               "reason for a design choice, and neither can be inferred from a drawing. A relation "
               "between architecture and either would rest on a design rationale the patent record "
               "does not carry. The traces do not decide it either: the propulsor count rises inside "
               "Tilt Rotor and CVT but is predicted by three drivers at once (distributed control, "
               "failure tolerance, noise) and the record cannot separate them; the ducted share, the one "
               "trace noise alone predicts, is flat in four classes and falls in CVT; rotor diameter is "
               "not labelled.",
        would="The applicant's stated rationale, read out of the descriptions of the 574 primary "
              "patents by hand, or certification noise data, which exists only for aircraft that have "
              "flown; for the enclosure trace, a rotor diameter per aircraft would turn the ducted "
              "share into a disc-loading reading."),
    # ================================================================ Q2 converging
    "diversity_bands_per_window": dict(
        q="Q2",
        asked="Does the variety of the design space fall from one window to the next, read as Hill "
              "numbers with sampling bands?",
        items=["hill", "la_hill_by_window", "dominant_design"],
        cannot="Every window is rarefied to 40 aircraft, and at that sample the 95 % band on the "
               "effective number of classes spans one to two classes either side (2016-19, ¹D 7.1 "
               "with a band of 5.6 to 8.7), so no pair of windows separates. The question is carried "
               "by another route: condition 2 of the test in [[dominant_design]] reads ²D against a "
               "permutation null instead of a sampling band.",
        would="Windows of several hundred aircraft each — the two earliest hold 69 and 67 — since the "
              "bands narrow only with the sample."),
    # ================================================================ Q3 gaining and abandoned
    "small_class_lifetimes": dict(
        q="Q3",
        asked="When does each of the seven small classes peak, and are their bars reliable — the "
              "empty 2012-15 window of the Personal Flying Vehicle, the 2016-19 spike of the "
              "Hoverbike, the Stopped/Slowed Rotor Wing with no story at all?",
        items=["class_cycles", "la_class_cycles"],
        cannot="Their window cells, in aircraft: PFV 3·0·2·4·1 (10 aircraft), RC 2·1·4·1·1 (9), SRW "
               "0·0·3·4·2 (9), DS 0·0·1·4·0 (5), HB 1·4·13·1·1 (20), TB 2·2·12·6·3 (25), PTC "
               "0·2·13·8·3 (26). The Hoverbike spike is 13 aircraft, the empty PFV window is 0 of 10, "
               "and nine aircraft cannot have a lifetime; the seven classes hold 104 of the 663 "
               "classed aircraft between them.",
        would="The classes themselves growing several-fold, which no re-cut of this record supplies; "
              "the five classes above 50 aircraft are read as they stand, and the 2024-26 window "
              "completes when the three-year publication lag has passed."),
    "class_arrival_order": dict(
        q="Q3",
        asked="Did the classes arrive in an order — which architecture came first, and which is the "
              "latest?",
        items=["la_lead_lag_class", "lead_lag"],
        cannot="The medians of the seven largest classes fall in 2018 (four classes), 2019 (two) and "
               "2020 (one): a two-year spread against a three-year 90th-percentile publication lag. The "
               "first-year column runs from 1999 to 2013 but rests on one aircraft per class. The order "
               "that can be read is the order of the windows, not of the classes.",
        would="A dated record that does not wait for publication — the first public announcement per "
              "class — or this table again once every window is complete; the spread would still sit "
              "inside one window."),
    "entrants_bring_the_shift": dict(
        q="Q3",
        asked="Do the firms entering in each window bring the class shift with them — in the early "
              "windows as well as the late ones?",
        items=["la_cohort_mix", "filers_over_time", "la_cohorts"],
        cannot="Only the two windows with 61 and 85 entrants can be read, and they are: the entrants "
               "of 2020-23 lead the field on Lift + Cruise by 3.6 points. Before 2016 the entering "
               "firms number 15 and 19, and the class cells they fall into hold 1 to 8 firms, which is "
               "a count, not a share.",
        would="Nothing outside the record: before 2016 the field was 15 and 19 firms, and a window "
              "with few entrants cannot be asked what they brought. The answer holds where the field "
              "existed."),
    # ================================================================ Q4 who designs it
    "firm_trajectories": dict(
        q="Q4",
        asked="Do firms explore across classes over their successive aircraft or deepen in one, and "
              "can a firm's path be read as a sequence?",
        items=["transitions", "la_transitions", "firm_tiles", "coverage"],
        tried="a sequence analysis of firm trajectories, probed 2026-09-23 and not built",
        cannot="Of the 208 named filers 134 hold one aircraft and make no succession; 74 hold two or "
               "more, 33 three or more, 17 five or more. Of the 33, the median span from first to last "
               "filing is three years and 31 change class at least once, so a sequence of three steps "
               "over three years would measure noise. The one reading the record supports is the "
               "diagonal of [[transitions]]: 119 of 256 successions stay in class.",
        would="Programme histories from the firms themselves, which run longer and are dated more "
              "finely than their patents; inside the record, only the stay-in-class share."),
    "shakeout": dict(
        q="Q4",
        asked="Has the field reached a shakeout — are firms exiting, and does the sector sit on an "
              "industry-lifecycle curve?",
        items=["la_filers_by_window", "la_cohorts"],
        tried="an Utterback-Abernathy placement, probed 2026-09-23 and not built",
        cannot="An exit needs a firm that was there: 73 firms have their last filing in 2019 or "
               "earlier, but 53 of them filed exactly once, so their leaving cannot be told from never "
               "having been a programme; only 20 firms with two or more aircraft stop. Entry is "
               "measurable — 15, 19, 61, 85 and 28 new firms by window, in [[la_filers_by_window]] — "
               "and exit is not, so a lifecycle placed on 20 firms would be the weakest claim in the "
               "document.",
        would="A programme-status record for every filer: the public directory tracks 112 of the 665 "
              "aircraft (36 active, 13 paused, 13 superseded, 33 ended, 17 unknown) and has nothing on "
              "the other 553."),
    "citations_by_class": dict(
        q="Q4",
        asked="Does one architecture draw more citations than another — is any class more "
              "influential?",
        items=["la_ip_by_class", "la_ip_strategy"],
        cannot="Forward citations accumulate with age — raw counts fall with the priority year at "
               "Spearman rho −0.53 over the 1 639 acquired patents — and the classes differ in age, so "
               "the raw columns rank the old classes first. The cohort rank removes the age (rho "
               "−0.02), but per class it is a median over 5 to 149 patents spread across the whole span "
               "of priority years, and the twelve classes then sit between 0.33 and 0.69 with nothing to "
               "separate them.",
        would="Citation cohorts thick enough to rank inside — several hundred patents per class — or "
              "the age-free measure read on the firm, where [[la_ip_strategy]] already prints it."),
    # ================================================================ Q5 where
    "region_design_proximity": dict(
        q="Q5",
        asked="Do the firms of one region design alike — is the design space organised by region?",
        items=["proximity_region", "la_proximity_region", "a2_d14_firm_proximity"],
        cannot="The answer is a null, and it holds on the 17 firms with five or more aircraft, 136 "
               "pairs: within-region means of 0.39, 0.55 and 0.64 sit inside the across-region 0.50 to "
               "0.56. Below that line a firm has no profile to compare, so the Asia-Pacific box rests on "
               "4 firms, and the per-firm reading at class level is only the coarseness of the class.",
        would="More firms above five aircraft — 17 of 208 today — which is the sector's own growth, not "
              "a re-cut of the record."),
    "country_specialisation": dict(
        q="Q5",
        asked="Does a country below the largest few specialise in a class or an archetype?",
        items=["country_class", "la_country_class", "specialisation", "la_specialisation", "region_grid"],
        cannot="Beyond the United States (228 aircraft), China (114), Germany (75) and Japan (35) every "
               "country holds fewer than 25, so a twelve-class split gives cells of 0 to 8; at archetype "
               "level 11 of the 34 archetypes hold fewer than ten aircraft, so a region cell holds 0 to "
               "4 and an index of 0.00 is one aircraft that sits elsewhere. The question is answered at "
               "the level the record supports, three regions, in [[region_grid]].",
        would="A record several times larger below the fourth country; pooling to three regions is what "
              "the document does instead."),
    # ================================================================ Q7 patents vs what is built
    "mission_and_use": dict(
        q="Q7",
        asked="What is the aircraft for — capacity, piloting, power source, development status, the "
              "industry it is aimed at — and does the mission decide the architecture?",
        items=["mission", "la_mission_capacity", "la_mission_piloting", "la_mission_power source",
               "la_mission_status", "industry_by_class", "la_industry_by_class", "linked_check"],
        cannot="A patent claims a machine, not a mission. The mission fields exist only on the "
               "directory page of the 95 linked aircraft (14 % of 665), a set that differs from the rest "
               "on filer type, priority window, priority year and propulsive units ([[linked_check]]), "
               "and even there capacity is unstated for 29 of the 95, piloting for 34 and status for "
               "27. The industry named in the text is a classifier's reading, unspecified for 345 of "
               "665 and not verifiable by the author.",
        would="A mission stated per aircraft: the directory gives one for 95, the patent text one for "
              "about half and only through a classifier; a hand reading of the 574 primary "
              "descriptions is the only route inside the record."),
    "index_score_vs_patents": dict(
        q="Q7",
        asked="Does the AAM Reality Index score, or a firm's disclosed funding, track anything on the "
              "patent side, firm by firm?",
        items=["la_ari_correlation", "ari_history", "la_market_vs_patents", "la_ari_gap"],
        cannot="The index lists 18 firms that file here and 9 of them disclose funding; over 57 "
               "firm-by-firm tests none survives the correction for multiple testing, and the one that "
               "comes closest — funding against class breadth, rho 0.67 — rests on 9 firms. What 18 "
               "firms do support is printed in [[la_market_vs_patents]] and [[la_ari_gap]]: rated firms "
               "hold a median of 4 aircraft against 1 and differ from the rest in six named ways, while "
               "the score itself predicts nothing (rho −0.20 against unique aircraft, p 0.42).",
        would="A rating that covers more of the 208 named filers than 18; at 18 the record supports a "
              "statement about order and no more."),
    "built_by_class": dict(
        q="Q7",
        asked="Do the architectures that are patented most get built — the share of each class above "
              "TRL 2?",
        items=["la_trl_placeholder"],
        tried="the TRL per aircraft is computed (`tables/a2_d15_trl_by_class.csv`) and the per-class "
              "comparison was probed 2026-09-23",
        cannot="64 of the 665 aircraft are above TRL 2 (10 %); over the five largest classes the "
               "difference in that share is χ² 4.09, p 0.39, and four classes — Hoverbike, Personal "
               "Flying Vehicle, Stopped/Slowed Rotor Wing, Rotorcraft — have none at all. With 64 events "
               "over twelve classes there is no power for a per-class claim, and there will be none from "
               "this corpus.",
        would="More flown aircraft, which is time; the base rate — one aircraft in ten leaves the "
              "drawing board — and the four classes with nothing built are readable now."),
    # ================================================================ Q8 indicator value
    "patent_to_flight_split": dict(
        q="Q8",
        asked="Does the time from first patent to first flight differ by region, certifying authority "
              "or configuration?",
        items=["la_ari_clock_region", "la_ari_timeline", "ari_clock"],
        tried="a by-configuration split, probed 2026-09-23 and not built",
        cannot="The index dates the first flight of the manufacturers it lists, and 13 of them file "
               "here, 12 with a date; split by certifying authority or region, 3 of the 9 rows rest on "
               "one firm and the largest on five, and split over twelve classes the cells are single "
               "firms. The firm-level answer, four to six years, is in [[la_ari_timeline]].",
        would="A first-flight date for every aircraft that flew — the 64 above TRL 2 — rather than for "
              "the 24 manufacturers the index lists."),
    "abandonment_by_class": dict(
        q="Q8",
        asked="Do applicants let go of one architecture more than another, and can abandonment be read "
              "as survival over time?",
        items=["la_abandonment_by_class", "abandonment"],
        tried="a survival analysis, probed 2026-09-23 and replaced by a logistic model",
        cannot="The legal status is a snapshot at 2026-06 with no lapse date, so there is no "
               "time-to-event and no survival curve. The model that fits a status at one time is a "
               "logistic one, and on the 356 aircraft with priority year 2019 or earlier it finds no "
               "configuration term: age (p 0.001), a US office (p 0.003) and a named company (p 0.018) "
               "predict the letting-go, and the class rates of 0.27 to 0.41 over the classes with 30 or "
               "more aircraft sit inside each other's noise. The base rate — 46 % of the 305 primaries "
               "of that cohort no longer in force — is the finding, and it is in [[abandonment]].",
        would="Dated legal events per patent — the INPADOC history — which the snapshot does not "
              "carry."),
    "examination_by_class": dict(
        q="Q8",
        asked="Does the examining office treat one architecture as more novel than another?",
        items=["examination", "la_examination_class", "la_examination_office"],
        cannot="The record holds the outcome and not the reasons: the classes sit at the same grant "
               "rate while the offices do not — 226 of 330 US-published primaries in force against 15 "
               "of 99 at the CNIPA, in [[la_examination_office]]. A null on the class cannot tell an "
               "examiner indifferent to architecture from claims that never turn on it.",
        would="Office actions and the grounds of rejection per patent, which the snapshot does not "
              "hold."),
    # ================================================================ M method
    "intra_rater_reliability": dict(
        q="M",
        asked="How reliable is the labeller against himself — would the same patent get the same label "
              "twice?",
        items=["la_relabel_protocol", "atlas_arch_gt", "la_name_arch_check"],
        cannot="Not a limit of the record. The relabel of 50 primary patents is designed and not run, "
               "so every agreement number in this document is against an outside source — the "
               "whole-patent reading in [[atlas_arch_gt]], the public directory in "
               "[[la_name_arch_check]] — and never between the labeller and himself.",
        would="The relabel batch: 50 primary patents stratified by class, relabelled at least four "
              "weeks after labelling closed (2026-09-19), agreement and Cohen's κ per field."),
}

#: >>> SLOT for the design-drivers section (being built the same day, 2026-09-23). <<<
#: One entry per driver whose trace is not labelled — rotor diameter, hub mechanism, cabin
#: dimensions, aircraft size, noise itself — keyed by an internal name, with the same fields as
#: OPEN_QUESTIONS (``asked``, ``items``, ``tried``, ``cannot``, ``would``; ``q`` defaults to "Q1"
#: and ``sub`` to "design drivers"). Rows print under "Q1 · design drivers", after the two driver
#: rows OPEN_QUESTIONS already holds (``battery_energy_vs_disc_loading`` and
#: ``noise_wind_as_drivers`` — extend that one rather than adding a second noise row). Nothing else
#: needs touching: ``la_tables.open_questions`` reads this dict as it stands, and a ``[[name]]``
#: in any field resolves to the live number of that figure or table.
UNLABELLED_DRIVERS: Dict[str, Dict] = {
    # Filled 2026-09-23 by the design-drivers section (2.4). One entry per trace the author's drivers
    # table predicts and no label carries; the verdict table [[la_driver_verdicts]] closes with the same
    # five rows, marked "not labelled". Every count typed here was read out of tables/ on 2026-09-23.
    "rotor_diameter": dict(
        asked="Does the rotor diameter fall as the propulsor count rises — the trace distributed propulsion "
              "and community noise both predict — and does it differ by class?",
        items=["trace_drift", "la_driver_verdicts", "atlas_units"],
        tried="the propulsor-count half of the trace, per class and window, in [[trace_drift]] (iv); the "
        ...
        cannot="No field of the codebook records a diameter, a span, an area or a scale, and a patent drawing "
               ...
               "rises inside Tilt Rotor and CVT (Table 2.4.3a), and whether the rotors shrank with it is "
               "unreadable.",
        would="A rotor diameter per aircraft — from specification sheets or the certification basis of the "
              ...
              "a mass."),
    "hub_mechanism": dict(
        asked="Is thrust controlled by blade pitch or by RPM — the hub mechanism that distributed rotor "
              "control makes possible and maintenance cost pushes towards?",
        items=["la_driver_verdicts"],
        tried="nothing could be built: the trace has no field",
        cannot="The propulsor cards record count, kinematics, articulation, tractor or pusher, enclosure and "
               "retraction; they do not record the hub, and a whole-aircraft drawing does not show it. The "
               "articulation field (Fixed · Tilt · Other) is about the mount, not the blades.",
        would="A per-aircraft hub reading from the claims and detail figures of the 574 primary patents, "
              "which the labelling protocol left out because the whole-aircraft figure was its unit; or "
              "the propulsion specification of the flown aircraft."),
    "cabin_dimensions": dict(
        asked="Do the cabin and the fuselage grow with the occupant count and the payload the mission "
              "requires?",
        items=["la_driver_verdicts", "la_mission_capacity"],
        tried="nothing could be built for the dimension; the fuselage shape and the ground contact are "
              "labelled and read in 2.4, and the capacity stated on the directory page is in "
              "[[la_mission_capacity]] for the linked aircraft",
        cannot="Occupancy was ruled not determinable from a drawing (2026-09-09) and no length, width or "
               "seat count is labelled; the fuselage fields record shape, kinematics and ground contact "
               "only. Capacity is stated on the directory page of the 95 linked aircraft and nowhere for "
               "the rest.",
        would="A stated capacity and cabin dimension per aircraft, which specification sheets carry for "
              "the flown aircraft and the patent figure does not."),
    "aircraft_size": dict(
        asked="Does the operating environment — the vertiport, the rooftop, the ship deck — bound the size "
              "of the aircraft, and does the corpus show it?",
        items=["la_driver_verdicts"],
        tried="nothing could be built: no size is labelled. The two labelled traces the same driver "
              "predicts, ground contact type and boom stowing, are read in [[la_driver_verdicts]]",
        cannot="A patent drawing carries no scale, and the label set records no span, footprint or mass; "
               "ground contact is drawn on 197 of the 639 aircraft with an answer and booms stow on 9 of the "
               "299 with booms, which is too thin to trend.",
        would="Span and footprint per aircraft from the flown aircraft's specifications, or the vertiport "
              "design standards the sector files against, joined to the corpus by aircraft name."),
    "tilting_joint_count": dict(
        asked="How many tilting joints does an aircraft carry — the count that development, certification "
              "and maintenance cost all push down?",
        items=["trace_drift", "la_driver_verdicts"],
        tried="the nearest labelled trace, tilting joint GROUPS per aircraft (a tilting wing panel, boom "
        ...
        cannot="The label set records whether a part tilts, not how many actuators tilt it: a tilting wing "
               ...
               "inside Tilt Rotor and CVT (Table 2.4.3a), which is the direction regime transition predicts "
