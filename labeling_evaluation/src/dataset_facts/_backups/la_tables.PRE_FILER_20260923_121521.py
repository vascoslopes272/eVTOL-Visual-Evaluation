...
GRID_VARIABLES = {
    ...
    # Parked 2026-09-23, author's ruling on Figure 4.3.2b ("why do I need to talk about powertrain?
    ...
    # by any figure of 4.3.2 now, and each finding was moved to the section that owns it:
    ...
    #                 a real finding and is made in 5.1; against region it is a measurement artefact.
    ...
    #                 the propulsive-unit bands (p = 0.00012), so it is drawn in 5.2 instead, where
...
#: What Spearman's rho is, printed under Table 4.2.7b. Kept here so the table, the figure and the
...
def ari_correlation(firms: pd.DataFrame, v: pd.DataFrame,
    ...
    if ds is not None:
        ...
        av = ds.approved_variants.merge(ds.identity[["patent_id", "company_canonical", "claim_count",
                                                     "family_size"]],
...
def ari_gap(ds: Dataset, v: pd.DataFrame) -> pd.DataFrame:
    ...
    Eighteen firms is too few to correlate anything reliably (Table 4.2.7b), but those eighteen firms
...
def linked_representativeness(v: pd.DataFrame) -> pd.DataFrame:
    ...
    Section 5.5 describes the linked aircraft only, and the author's ruling of 2026-09-23 is that
...
#: across it rather than leaving it out (Figure 5.2 (iii)).
DUCT_THIN = 20
...
def duct_by_units(v: pd.DataFrame) -> pd.DataFrame:
    ...
    and by architecture class — the frame behind Figure 5.2 (iii).
    ...
    two negatives (region and window) that moved this variable out of Figure 4.3.2b.
    """
    w = v.copy()
    w["band"] = unit_band(w)
    duct = w["any_ducted"]
...
def ip_strategy(ds: Dataset, v: pd.DataFrame, min_aircraft: int = FIRM_MIN) -> pd.DataFrame:
    ...
    which is what the bubbles of Figure 4.2.5a are sized by, and the age-normalised median cohort rank
...
def ip_by_class(v: pd.DataFrame, ds: Optional[Dataset] = None) -> pd.DataFrame:
    ...
    *Forward citations* are printed as a **mean and a median** because their distribution is very
    ...
    total attention the class has drawn (and is what the bubbles of Figure 4.2.5a are sized by); the
...
#: Where the two market dates come from, printed under Figure 4.2.7c and Tables 4.2.7d/e.
ARI_DATE_SRC = (
...
def flags() -> pd.DataFrame:
    ...
    rows = [
        ...
        # 2026-09-23: Table 4.1.6 was cut on the author's ruling ("basically the same thing as the
        # figure"), so it is no longer a candidate; class_cycles() still writes the CSV. 4.1.7 comes
        # off this list at the same time: its class panel was the part that repeated 4.1.6, and it
        # has been replaced by the class × region matrix, which nothing else in the document shows.
        ("4.1.8 diversity per window, figure and table", "4.1.8",
         ...
         "reported in 4.1.3.3, against a null instead of a sampling band"),
        ("Table 4.1.9a zones", "4.1.9", "20 rows; the figure carries it"),
        # 2026-09-23, author's ruling on 4.2.1: the firm-tier table was cut once the curve started
        # printing the firms and the aircraft of each segment. firm_tiers() still writes the CSV.
        # 2026-09-23, 4.2.3: the firm tiles were replaced by the firm-influence figure; fig_firm_tiles
        # is kept and parked in la_figures.render_all.
        ("4.2.2 the body of filers over time, figure and both tables", "4.2.2",
         "DECIDE (2026-09-23): the section was kept and normalised rather than cut. Panel (i) and "
         "Table 4.2.2a are firm demographics that nothing else shows; panel (ii) and Table 4.2.2b "
         "now compare the class mix of the entering FIRMS with the class mix of the window's "
         "AIRCRAFT, which is the one claim 4.1.2 cannot make. If the section still has to go, the "
         "part to keep is panel (ii) — it could be folded into 4.2.4, whose successions answer the "
        ...
        # The figure now groups the firms in region blocks and Table 4.2.6 gives the region-pair means.
        # 2026-09-23, author's ruling: the TABLE of 4.3.4 is cut, and so are Tables 4.3.3, 5.1 and
        # 5.5g. Only the figure of 4.3.4 is still a candidate, so the row says so.
        ("Figure 4.3.4 country × class", "4.3.4", "the specialisation index (4.3.3) answers it at archetype level; its table was cut on 2026-09-23"),
...
def build_all(ds: Dataset) -> Dict[str, pd.DataFrame]:
    ...
    # written to tables/ and read by Figure 4.1.9b; not placed in a node of its own
    ...
    # written to tables/ and read by Figure 5.5c; not placed in a node of its own
    ...
    # written to tables/ and read by Figure 5.2 (iii) and its takeaway; not placed in a node of its
    # own — the figure carries the reading and the CSV keeps every share checkable beside its n.
    # To print it, add "la_duct_units" to node 5.2 in la_index.NODES.
    ...
    # markdown table (the author's complaint about "table 4.2.4" is about the figure, which is the
    # matrix), while the takeaway of Figure 4.2.4 reads its pair counts out of this frame and the CSV
    # keeps them checkable. To print it, add "la_transitions" to node 4.2.4 in la_index.NODES.
