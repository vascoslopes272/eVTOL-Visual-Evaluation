...
_item("la_top_archetypes", [HL],
      ...
      "aircraft, and what the top 3 and top 5 hold together. The three levels are three "
      "descriptions of the same aircraft, coarse to fine. **A0** is the architecture class alone. "
      "**A1t**, the *design species*, is the class plus how many wings the aircraft has plus "
      "whether at least one of its propulsors tilts — so 'SLC · 1 wing · no tilt' is one species "
      "and 'CVT · 1 wing · tilts' another. **A0c** is the class plus the band of propulsive units "
      "it carries.",
...
_item("sm_class_configs", [HL],
      _u(UA, "{unique_s}", "priority window", "% of the class"),
      "**(i)** the share of the class that sits in its single most common configuration, on the "
      "class's own labels — the configuration itself is named per class in the table beside this "
      "figure. **(ii)** distinct configurations divided by the aircraft of that class-window: 1.0 "
      "means every aircraft is its own configuration, 0.2 that five aircraft share one. A falling "
      "line would be convergence; a flat line is not.",
...
_item("zones", [HL, PS],
      ...
      "**Top panel, and the axes matter.** Left to right: the share of the 2016-23 aircraft the "
      "archetype holds — far right means many recent aircraft. Bottom to top: distinct filers per "
      "aircraft — high up means a different firm behind almost every aircraft, low down means a "
      "few firms holding several each. So the **crowded core is the lower right** (a large "
      "archetype held by relatively few filers) and the **empty rim is the left-hand edge**, where "
      "most archetypes sit with a handful of aircraft. Bubble size is the aircraft count; the two "
      "grey crosshairs are the medians of the two axes. A triangle is an archetype first seen in "
      "2016 or later, an inverted triangle one not seen after 2019, a circle one present "
      "throughout. **Bottom panel:** the same archetypes over the five windows, the count printed "
      "in the cell and the darkest cell being the archetype's own busiest window. A blank cell "
      "means no aircraft of that archetype in that window — it is not an 'absent' category, and "
      "an archetype with fewer than five aircraft in total is not drawn here at all. How many "
      "propulsive units each of these archetypes carries is the band in its own name and is drawn "
      "per class in the propulsive-units figure of this section.",
...
_item("la_driver_verdicts", [HL, CB],
      ...
      "One row per trace — a trace is one labelled feature read against the year, inside each "
      "class. *importance* says how much of the chapter's argument the row carries: **central**, "
      "supporting, or a null, meaning the trace separates nothing. *what moved* gives the first "
      "reportable window, the 2020-23 value, Spearman ρ and p (methods box). Two trace names are "
      "not self-explaining: *fuselage kinematics not fixed* is the share of a class whose body "
      "itself tilts or rotates rather than staying level — it is that share that falls inside "
      "Multirotor — and *stations carrying units* counts the places on the airframe a group of "
      "propulsors sits on.",
...
_item("coverage", [PS, HL],
      ...
      "**(i)** the cumulative share of the corpus covered by the top N firms, cut into four "
      "segments: read up from a rank on the x axis to see how much of the analysis set those firms "
      "hold between them. **(ii) and (iii) bring in an outside opinion of the same firms.** The "
      "AAM Reality Index is a published score, 0 to 10, that one analyst house gives each eVTOL "
      "manufacturer for how close it is to carrying passengers — funding, prototypes flown, "
      "certification progress. It is a judgement about the company, not a measurement of this "
      "corpus, and 18 of the firms that file here carry one. Panel (ii) puts those 18 beside every "
      "other named firm and asks whether being rated goes with holding more aircraft. Panel (iii) "
      "asks the sharper question: among the rated firms, does a higher score go with more aircraft "
      "in this corpus? That is what Spearman ρ measures — a number from −1 to +1 saying whether "
      "one quantity rises as the other does, reading only the ORDER of the values, with p read as "
      "in the methods box. A ρ near zero, as here, means the index position and the patent "
      "position are unrelated.",
...
_item("la_filer_mix", [PS, HL],
      ...
      "*share per primary patent* = that filer type's primary patents divided by all primary "
      "patents in the corpus, so the column sums to 1; *share of the analysis set* is the same "
      "division done on aircraft; *aircraft per patent* is the row's aircraft divided by its own "
      "patents, which is why it is near 1 everywhere. The *distinct filers* cell is a count of "
      "named organisations only: individual inventors are not resolved to named persons, so that "
      "row and the total print what is known instead of a number that would be false precision.",
...
ANSWER: Dict[str, str] = {
    ...
    "2": "**Nothing is converging, and the classes are drifting apart.** On the three conditions "
         ...
         "{tk_dr_slc_units_first}. The count of tilting joints rises inside the classes that "
         "transition and the pooled line is flat, but divided by the propulsive units those joints "
         "have to move it falls — designers are not adding joints, they are adding propulsors and "
         "the joints follow at a declining rate per propulsor, which is the direction the cost "
         "drivers predict. Two traces move against the only driver that predicts them: "
