...
def fig_lead_lag(v: pd.DataFrame, path: Path) -> Path:
    ...
    *is* Tables 2.1.8a/b — one row per group, the bar running from its 25 % year to its 75 %
    ...
    return _save(fig, path, SRC + "; complete priority years only (≤ 2023), so 2024-26 is excluded "
                 ...
                 "corpus; a cell of the matrix with fewer than 5 aircraft is printed “–”; Tables 2.1.8a/b/c. "
                 "NOT normalised against B64 aviation patenting, and it does not need to be: every group is "
                 "divided by its own total, and the stored baseline (2.1.1) has one series for all offices, "
...
def fig_class_cycles(v: pd.DataFrame, path: Path) -> Path:
    """2.1.7 as ONE longitudinal bar, the percentage printed per window (author's ruling,
...
def fig_hill(v: pd.DataFrame, path: Path) -> Path:
    """2.1.9 made self-explaining (author, 2026-09-23: "I don't really know the meaning of D1,
    ...
    return _save(fig, path, SRC + "; unit = unique aircraft; Table 2.1.9; A0 = the architecture class "
                 ...
                 "(Figure 2.1.1): the field grew hard and did not widen. Whether the early windows were already narrower "
                 "than chance is a different question, and 2.1.3.3 answers it against a null.")
...
def fig_filer_weight(v: pd.DataFrame, path: Path) -> Path:
    ...
    Nothing else in the document answers it. Table 2.1.6a counts *distinct filers* per archetype,
    which says how many there are and not how unevenly they divide it, and the one-vote-per-filer
    check of 2.1.2 was cut on 2026-09-22. So this figure carries the answer in two readings: how
    ...
    return _save(fig, path, SRC + "; unit = unique aircraft, so a firm that filed four patents on one "
                 ...
                 "filer, so no two lone inventors are merged; Table 2.1.6b, and per window "
                 ...
                 "Table 2.1.6b counts the same thing a third way: give every filer one vote instead of "
...
def fig_zones(v: pd.DataFrame, path: Path) -> Path:
    ...
    _legend_arch(fig, ARCH_ORDER + ["Other"])
    return _save(fig, path, SRC + "; A0c archetypes (class × propulsive-unit bin) with 5 or more aircraft; Table 2.1.6a.",
...
#: what each PatSeer legal status means, spelled out under Figure 2.1.10 (author, 2026-09-23:
...
# ------------------------------------------------------- 2.2 provenance ----
#: A percentage of a handful of aircraft is not a trend. Every cell of Figure 2.3.2a/b that holds
...
def fig_region_grid(v: pd.DataFrame, path: Path, variables: Optional[List[str]] = None,
    ...
    if ratio is not None and ratio.notna().any():
        ...
        norm += (f"The corpus as a whole went from {got.iloc[0]:.1f} patents per 1 000 B64 patents in "
                 ...
                 f"of every raw rise here is the general one and a part is not (Figure 2.1.1). ")
    return _save(fig, path,
...
def fig_country_class(v: pd.DataFrame, path: Path) -> Path:
    ...
    ax.set_title("Class share per applicant country, eight largest countries (codes as Figure 1.1a)")
...
def fig_coverage(v: pd.DataFrame, path: Path) -> Path:
    ...
    ax.set_ylim(0, 1.06)
    ax.yaxis.set_major_formatter(matplotlib.ticker.PercentFormatter(1.0, decimals=0))
    ax.set_xlabel("named companies ranked by unique aircraft (largest first); they are named in Figure 2.2.3a")
    ...
    return _save(fig, path, SRC + "; Table 2.2.1 and Table 2.2.7a; the segment counts of (i) are "
...
def fig_filers_over_time(v: pd.DataFrame, path: Path) -> Path:
    ...
    _legend_arch(fig, ARCH_ORDER + ["Other"])
    return _save(fig, path, SRC + "; Tables 2.2.2a/b; named firms = companies and institutes.",
                 ...
                 "AIRCRAFT (the same window's class mix, the quantity Figure 2.1.2 draws). Where the left bar runs "
...
def fig_firm_influence(v: pd.DataFrame, path: Path) -> Path:
    ...
    Replaces the firm tiles at 2.2.3 (author, 2026-09-23: "I don't really know what to take out
    ...
    cannot be one firm's portfolio. Together they answer the author's 2.1.6 question — how much of
...
def fig_proximity_region(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    """2.2.6 — the proximity matrix with the firms grouped in region blocks (user ruling 2026-09-23)."""
...
def fig_ari(v: pd.DataFrame, path: Path) -> Path:
    ...
    _legend_arch(fig, ARCH_ORDER + ["Other"])
    return _save(fig, path, ARI_SRC + "; corpus counts from " + SRC + "; Table 2.2.7a.",
...
def fig_ari_history(v: pd.DataFrame, path: Path) -> Path:
    """2.2.7b — how each firm's score moved, and funding against patented aircraft.
    ...
    return _save(fig, path, ARI_SRC + "; Table 2.2.7a. " + str(len(disclosed)) + " of the " + str(len(fc))
...
def fig_ari_clock(v: pd.DataFrame, path: Path) -> Path:
    """2.2.7c — the patent clock against the market clock.
    ...
    return _save(fig, path, ARI_SRC + "; corpus priority years; Tables 2.2.7d and 2.2.7e. "
                 ...
                 "is the years from a firm's first patent here to its first flight, and Table 2.2.7e groups that gap by "
...
def fig_mission(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, EVN_SRC + "; G1 class folded as Table 3.5a (VT vectored thrust, LC lift + cruise, WM wingless, "
                 ...
                 "publicly announced aircraft of the corpus, not a sample of it — Figure 3.5c tests how far they stand "
...
def fig_linked_check(v: pd.DataFrame, path: Path) -> Path:
    """Figure 3.5c: whether the aircraft linked to an evtol.news page stand for the whole set.

    Section 3.5 is a description of the linked aircraft; the author's ruling of 2026-09-23 is that
    ...
    return _save(
        ...
        + f" — {med}. The reason is in how a link is made: an aircraft can only be linked when the "
          ...
          "Section 3.5 therefore describes announced eVTOL programmes, and its class mix and its "
          "geography can be carried back to the whole corpus; its filer mix, its dates and the size "
          "of its aircraft cannot.")


#: Figure 2.1.1 opens here. (i) pools everything earlier into this bar; (ii) starts here as a single year.
FILINGS_FIRST_YEAR = 2005

#: Dated sector events on the timeline of Figure 2.1.1 — the author's ruling of 2026-09-23: the
...
def fig_filings_per_year(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    # Table 2.1.1 keeps every year as its own row; the How-to-read line says so.
    ...
    src = (SRC + f"; Table 2.1.1; PatSeer snapshot {t.attrs['snapshot']}; "
    ...
    if pooled:
        ...
        read += (f" The first bar is the only pooled one: it holds {pooled[0]}-{FILINGS_FIRST_YEAR} together "
                 ...
                 f"hence its tick ≤{FILINGS_FIRST_YEAR}; Table 2.1.1 keeps those years as separate rows.")
    if strip_in:
    ...
    if normalised:
        ...
        read += (f" (ii) asks whether the rise is the sector's own: aviation patenting also grew, so the corpus's "
                 ...
                 f"{r.get('ratio_start')} in the base window (Table 2.1.1). The hatched years are incomplete on "
...
def fig_dominant_design(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; levels defined in Table 2.1.3.1a, conditions in Table 2.1.3.2, numbers in Table 2.1.3.3b; "
...
def fig_dominant_design_q(ds: Dataset, v: pd.DataFrame, path: Path,
    ...
    return _save(fig, path, SRC + "; the Gower distance fixed in Preliminary Analysis 5.3, imported from "
                 "its code; Q is Rao's quadratic entropy (5.5); numbers in Table 2.1.3.3c; 200 permutations.",
...
def fig_class_configs(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; Table 2.1.4 names the most common configuration per class and window; "
...
def fig_dimension_drift(v: pd.DataFrame, path: Path) -> Path:
    ...
    # the four fields of the 2.1.4 configuration, one per panel, then powertrain. The last one
    ...
    fig.suptitle("Dimension drift inside each class: panels (i)–(iv) are the four fields the configuration\n"
                 "of Figure 2.1.4 is made of, read one at a time, then powertrain",
                 x=0.01, y=1.10, ha="left", fontsize=9.5, fontweight="bold")
    return _save(fig, path, SRC + "; Table 2.1.5, where a window with fewer than 5 aircraft of the class keeps its "
                 "row and its count and only its measures are blank.",
                 "A line is an architecture class (the G1 top type), not an archetype; every point counts unique aircraft. "
                 "Panels (i)–(iv) take the configuration of Figure 2.1.4 apart, one field per panel, so the two figures "
                 "read together. Their percentages divide by the class's own aircraft in that window (the `aircraft` "
                 "column of Table 2.1.5, every one of them answered) — never by a count of rotors. Panel (v) is the one "
                 ...
                 "stays in Table 2.1.5, where the two points off 100 % (CVT 0.93 in 2016-19, TR 0.98 in 2020-23) are an "
...
def fig_transitions(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + f"; each named firm's aircraft in priority order, every consecutive pair counted "
                 ...
                 "a class the field is leaving from a class the field never stays in — and read with Figure 2.2.2 it "
...
def fig_ip_strategy(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    """2.2.5a — depth against breadth, then claims and citations per class.
    ...
    _hgrid(ax3, "x")
    return _save(fig, path, SRC + "; PatSeer claim_count, forward_citations, family_size; Tables 2.2.5a/b. "
...
def fig_specialisation(v: pd.DataFrame, path: Path) -> Path:
    """2.3.3 as a coloured dot chart, one row per archetype, one dot per region.
...
def fig_examination(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; PatSeer legal_status_raw at the 2026-06 snapshot; Tables 3.6a/b.",
...
def fig_c1_removal(ds: Dataset, path: Path) -> Path:
    ...
    _hgrid(ax2, "y")
    return _save(fig, path, SRC + "; wizard disapproval reasons and the Similar tags (Table A.1a); class codes as Figure 1.1a.",
...
ATLAS_REUSE: Dict[str, Callable] = {
    # parked 2026-09-22: superseded by Figure 2.1.1 (filings_per_year) and placed in no node, so it was
...
#: User rulings: the rotorcraft must be visible in Figure 2.1.2 (2026-09-22), and the hoverbikes and
...
#: ISO 3166-1 alpha-2 written out. The author read the country axis of Figure 2.3.1 and did not
...
def _post_region(fig, ds: Dataset) -> None:
    """Figure 2.3.1, left panel: spell the country codes out and print each country's share of the
...
#: the three classes whose propulsors move — the split Figure 3.4 turns out to measure
TILTING_CLASSES = ("TR", "CVT", "TW")


def _post_state_by_arch(fig, ds: Dataset) -> None:
    """Figure 3.4: say what the flight state is actually evidence of.
...
def fig_design_heatmaps(ds: Dataset, av: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; archetype_frame (boom bin, any tilting unit); class codes as Figure 1.1a.",
...
def fig_units(ds: Dataset, av: pd.DataFrame, path: Path, base_v: Optional[pd.DataFrame] = None) -> Path:
    ...
    Panel (iii) was added 2026-09-23. The author asked of Figure 2.3.2b whether ducting
    ...
    # sides of it — the convention of Figure 2.3.2a/b: a thin cell is suppressed visibly, never
    ...
    return _save(fig, path, SRC + "; propulsive_units, propulsion states (rules of 2026-09-19); class codes as "
                 "Figure 1.1a; the band shares of (iii) are tables/la_duct_units.csv, with the chi-square "
                 "tests behind them.",
                 why_median +
...
def fig_trace_drift(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    """Figure 2.4.1: every labelled trace of :data:`la_tables.TRACES` marked ``drawn``, one panel
    each, per class and window — the machinery of Figure 2.1.5 extended from four dimensions to
    ...
    fig.suptitle("The labelled traces inside each class, per window: one panel per trace,\n"
                 "ordered by the kind of driver that predicts it (the four of Figure 2.1.5 not redrawn)",
                 x=0.01, y=1.02, ha="left", fontsize=9.5, fontweight="bold")
    a = t.attrs
    return _save(fig, path, SRC + "; tables/la_trace_drift.csv (the frame of Table 2.4.3a), the same class rule and "
                 "window cells as Table 2.1.5.",
                 ...
                 "the trace (the author's table, 2.4): A1 technological, A2 physical, B requirement, C life-cycle cost. "
...
def fig_trace_couplings(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    """Figure 2.4.2: the couplings the fixed physical drivers (A2) predict, as bias-corrected
...
def render_all(ds: Dataset, out_dir: Path, partial_window_start: int = 2024, **_) -> Dict[str, Path]:
    ...
    jobs = {
        # parked 2026-09-22 (author's ruling on 2.1.2): the builders stay, they are simply not drawn.
        ...
        # 2026-09-23, author's ruling: Figure 2.3.2b keeps filer type alone. Powertrain and ducting
        ...
        # row, so a fourth row puts Figure 2.3.2a at roughly 13.5 in of drawn height against the
        ...
        # parked 2026-09-23 (author's ruling on 2.2.3): replaced at 2.2.3 by "firm_influence", which
        ...
        # 2.4 design drivers and their traces (2026-09-23)
