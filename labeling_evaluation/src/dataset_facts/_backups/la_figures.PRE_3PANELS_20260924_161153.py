...
def fig_filings_per_year(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    if normalised:
        ...
        if ratio_col in ti.columns:
            ...
            ax2.text(done.index.max() - 0.3, 0.25, "dotted line: 1 = grows only as fast as the baseline", fontsize=7,
                     color=INK2, ha="right")
            ...
            ax2.set_ylim(0, top * 1.4)               # room for the legend above the lines
            ax2.legend(loc="upper left", fontsize=7, frameon=True,
                       title="each line: eVTOL patents \u00f7 that baseline's patents, same nine offices", title_fontsize=6.5)
            ...
            ax2.text(0.02, 0.50, f"read \u00d7{_m:.1f} in {peak_year}: eVTOL was {_re:.0f} of every\n"
                     f"1 000 aeronautics patents that year, against\n{_rs:.1f} per 1 000 in {lo}-{hi} \u2014 its slice of\n"
                     f"aeronautics patenting is {_m:.1f}\u00d7 what it was",
                     transform=ax2.transAxes, ha="left", va="top", fontsize=6.6, color=INK2,
        ...
        if ratio_col in ti.columns:
            ...
            read += (f" (ii) asks whether the rise is the sector's own: patenting at large also grew, so each line is "
                     f"eVTOL's share of one baseline of the same nine offices and priority year — aeronautics (B64), "
                     f"all CPC section B, all patents — divided by its own {lo}-{hi} mean, so every line starts at 1 and "
                     f"a line at 3 means eVTOL grew three times faster than that baseline. By {peak_year} eVTOL had grown "
                     ...
                     f"per 1 000 B64 patents by {last} against {r.get('ratio_start')} in the base window (Table 1.1a). "
                     f"The hatched years are incomplete on both sides and are not drawn; (ii) does not pool: its first "
                     f"point is {FILINGS_FIRST_YEAR} alone.")
        else:
