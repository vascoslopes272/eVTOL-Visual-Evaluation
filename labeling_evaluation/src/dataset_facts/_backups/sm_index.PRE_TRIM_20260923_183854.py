...
* **No front matter.** There is no "how to read" page, no data-set paragraph and no vocabulary
  ...
  drawing — which used to take page 2 — is the closing appendix since 2026-09-23, on its own
  landscape page, with the pointer to it in the opening paragraph of Q1.
...
NODES: List[Dict] = [
    ...
    dict(id="Q1",
         text="*The data set, in two lines.* {acquired_s} patents were acquired, {representative_s} "
              ...
              "leaves out; the twelve architecture classes every finding is stated in are drawn "
              "in the appendix at the end of this document, which is the one page to keep open "
              "beside the rest.\n\n"
    ...
    dict(id="Q2",
         text="**Lift + Cruise overtakes Tilt Rotor and keeps the lead, and nothing is abandoned "
              ...
              "twelve of the drawing in the closing appendix) the one dimension that moves the same "
    ...
    dict(id="Q6",
         text="**Three countries hold almost two thirds of the corpus — US {tk_us_n}, CN "
              ...
              "result, graded core, carried by Figure 1.2.6 of the full document and not "
    ...
    # ---------------------------------------------------------------- the taxonomy, at the back
    # Author's instruction 2026-09-23: "regarding the two pages of taxonomy, put them in the end,
    # in the appendix". It was the first figure of Q1 and left the title page half empty; it is a
    # reference, not a finding, so it closes the document instead. In ``APPENDIX`` below, so it
    # opens its own page — and it is a LANDSCAPE figure, which ``report`` does not bind to the
    # heading, so the heading and this paragraph sit above it and the drawing has the page to
    # itself. The same drawing is Figure D.1b of the full document, where it is Appendix D too.
    dict(id="taxonomy", title="Appendix — the taxonomy every finding is stated in",
         text="The twelve architecture classes of card G1, which every class name in this "
              "document refers to. It is printed here rather than in front so that the document "
              "opens on a finding; a reader meeting a class name for the first time can turn to "
              "this page and back.",
         figures=["codebook_classes"]),
]
...
TEXTS: Dict[str, str] = {
    ...
    "new.1":
        ...
        "held fixed. The measure is panel (iii) of the propulsion figure in Q4 and "
    ...
    "new.4":
        ...
        "(Figure 1.2.4 of the full document, {tk_trans_pairs} pairs); it has never been cut by "
