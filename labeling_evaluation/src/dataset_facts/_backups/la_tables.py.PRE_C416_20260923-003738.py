...
def filer_mix(v: pd.DataFrame) -> pd.DataFrame:
    ...
    for k in order:
        ...
        if not (p or a):
            continue
        rows.append({"filer": k, "primary patents": p, "share per primary patent": round(p / len(pat), 2),
                     "unique aircraft": a, "share of the analysis set": round(a / len(v), 2),
                     "aircraft per patent": round(a / p, 2) if p else np.nan})
    out = pd.DataFrame(rows)
    n_named = int(v.loc[v["named"], "company_canonical"].nunique())
    out["filer"] = out["filer"].replace({"Named company": f"Named companies ({n_named} named filers incl. institutes)"})
    return out
