...
def fig_filings_per_year(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    if normalised:
        ...
        if ratio_col in ti.columns:
            ...
            # 2026-09-24 (final): three small panels, one per baseline, each drawing the RAW fraction
            # in its own unit — eVTOL patents per 1 000 aeronautics (B64) patents, per 10 000 CPC
            # section-B patents, per 100 000 patents, same nine offices, same priority year. No
            # normalisation: the author asked how the lines are calculated and why 2005-09 — the
            # base window only existed to put three fractions of very different size on one axis.
            # Each panel now shows its own numbers; the 2005-09 mean is a dotted reference line and
            # the peak is labelled with its multiple of that line. Complete years only.
            base = la_baseline.load().reindex(done.index)
            _e = pd.to_numeric(done["patents acquired"], errors="coerce")
            series = [("B64", pd.to_numeric(done["B64 patents (same offices)"], errors="coerce"), 1_000,
                       "aeronautics patents (B64)", "per 1 000")]
            if "cpcB_offices" in base.columns and base["cpcB_offices"].notna().any():
                series.append(("cpcB", pd.to_numeric(base["cpcB_offices"], errors="coerce"), 10_000,
                               "transport patents (section B)", "per 10 000"))
            if "all_offices" in base.columns and base["all_offices"].notna().any():
                series.append(("all", pd.to_numeric(base["all_offices"], errors="coerce"), 100_000,
                               "patents of any kind", "per 100 000"))
            ax2.set_axis_off()
            ax2.text(0, 0, " ", alpha=0)          # keeps ax2 a "drawn" axes for the (ii) mark
            # child (inset) axes of ax2, so fig.axes still holds two graphs and the (i)/(ii) marks
            # of FIGURE_PANELS land; ax2 itself only carries the title of the row
            n_p = len(series)
            gap_w = 0.10
            pw = (1.0 - gap_w * (n_p - 1)) / n_p
            axs = [ax2.inset_axes([i * (pw + gap_w), 0.0, pw, 0.72]) for i in range(n_p)]
            first_incomplete = int(ti.index[~ti["complete"]].min())
            idx_lines, peak_year = {}, None
            for k, ((key, denom, per, lab, unit), axp) in enumerate(zip(series, axs)):
                frac = _e / denom * per
                base_v = float(frac.reindex(range(lo, hi + 1)).mean())
                idx_lines[key] = frac / base_v
                if peak_year is None:
                    peak_year = int(frac.idxmax())
                pk = float(frac.loc[peak_year])
                axp.plot(frac.index, frac, color=INK, lw=1.8, marker="o", ms=3.2)
                axp.axhline(base_v, color=INK2, lw=0.9, ls=":")
                axp.text(ti.index.max() + 0.6, base_v * 0.92, f"{lo}-{hi}\naverage {base_v:.1f}", fontsize=6,
                         color=INK2, va="top", ha="right")
                axp.annotate(f"{peak_year}: {pk:.0f} {unit}\n\u00d7{pk / base_v:.1f} the average",
                             (peak_year, pk), textcoords="offset points", xytext=(-2, 5), ha="right",
                             fontsize=6.5, fontweight="bold", color=INK)
                axp.set_ylim(0, pk * 1.45)
                axp.yaxis.set_major_locator(matplotlib.ticker.MaxNLocator(4, integer=True))
                axp.set_xlim(done.index.min() - 0.8, ti.index.max() + 0.8)
                axp.axvspan(first_incomplete - 0.5, ti.index.max() + 0.5, color=GRID, alpha=0.75, zorder=0,
                            hatch="//", lw=0)
                yrs = [y for y in ti.index if y % 5 == 0]
                axp.set_xticks(yrs, [str(y) for y in yrs], fontsize=6.5, rotation=45)
                axp.tick_params(axis="y", labelsize=6.5)
                axp.set_title(f"{unit}\n{lab}", fontsize=7.2, loc="left")
                _hgrid(axp, "y")
            axs[0].set_ylabel("eVTOL patents", fontsize=7.5)
            axs[-1].text(first_incomplete - 0.3, axs[-1].get_ylim()[1] * 0.96, "still\nfilling", va="top",
                         fontsize=6.5, color=INK2)
            ax2.set_title("Of every 1 000 aeronautics patents (10 000 section-B, 100 000 of any kind)\n"
                          "filed at the same nine offices in a year, how many are eVTOL patents of this corpus",
                          fontsize=9.5)
        else:
        ...
        if ratio_col not in ti.columns:      # the single-axes (index) form; the panels above set their own
            first_incomplete = int(ti.index[~ti["complete"]].min())
            ...
            if ratio_col in ti.columns:
                ax2.set_ylabel(f"eVTOL patents \u00f7 baseline patents,\nas a multiple of the {lo}-{hi} value")
                ax2.set_title("How much faster eVTOL grew than the patenting around it "
                              f"(eVTOL patents \u00f7 a baseline's patents, as a multiple of its {lo}-{hi} value)", fontsize=9.5)
            else:
        ...
        if ratio_col in ti.columns:
            ...
            read += (f" (ii) asks whether the rise is the sector's own: patenting at large also grew, so each panel "
                     f"divides this corpus's eVTOL patents of a year by ALL patents of one kind filed at the same nine "
                     f"offices with that priority year \u2014 aeronautics (CPC B64), transport and operations (all of CPC "
                     f"section B), patents of any kind \u2014 and states the result as a count per 1 000, per 10 000 and "
                     f"per 100 000. Nothing is indexed: the dotted line is the {lo}-{hi} average of the same fraction, "
                     f"there only so the rise can be read as a multiple. By {peak_year} eVTOL had grown "
                     f"{m_b64:.0f}\u00d7 faster than aeronautics{wider}; the rise is not aviation's tide, not transport's, "
                     f"and not the patent system's. In absolute terms the corpus holds {r.get('ratio_end')} eVTOL patents "
                     f"per 1 000 B64 patents by {last} against {r.get('ratio_start')} in {lo}-{hi} (Table 1.1a). "
                     f"The hatched years are incomplete on both sides and are not drawn; the panels do not pool: the "
                     f"first point is {FILINGS_FIRST_YEAR} alone.")
        else:
