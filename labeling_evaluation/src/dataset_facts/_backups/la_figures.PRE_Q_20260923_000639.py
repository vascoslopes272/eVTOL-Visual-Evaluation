...
def fig_class_cycles(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; Table 4.1.6.",
...
def fig_hill(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; Table 4.1.8; A0 = class, A0c = class × propulsor-unit bin.",
...
def fig_coverage(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; Table 4.2.1; ARI firms from " + ARI_SRC + ".",
...
def fig_filings_per_year(ds: Dataset, v: pd.DataFrame, path: Path) -> Path:
    ...
    normalised = ix_e in t.columns and t[ix_e].notna().any() and t[ix_a].notna().any()
    if normalised:
    ...
    if normalised:
        r = la_baseline.readings(t)
        lo, hi = la_baseline.BASE_YEARS
        ax2.plot(t.index, t[ix_e], color=INK, lw=2, marker="o", ms=3.5,
                 label="eVTOL: patents acquired, this corpus")
        ax2.plot(t.index, t[ix_a], color=MUTED, lw=2, ls="--", marker="s", ms=3.5,
                 label="aviation: all B64 patents, same offices")
        ax2.axhline(100, color=INK2, lw=0.8, ls=":")
        first_incomplete = int(t.index[~t["complete"]].min())
        ax2.axvspan(first_incomplete - 0.5, t.index.max() + 0.5, color=GRID, alpha=0.75, zorder=0, hatch="//", lw=0)
        ...
        if last in t.index:
            for col, val in ((ix_e, t.loc[last, ix_e]), (ix_a, t.loc[last, ix_a])):
                ax2.annotate(f"{val:.0f}", (last, val), textcoords="offset points", xytext=(4, 4),
                             fontsize=7, color=INK2)
        ax2.set_xticks(list(t.index), [("≤2005" if y == 2005 else str(y)) for y in t.index],
        ...
        read += (f" (ii) removes the question the raw counts cannot answer: aviation patenting itself grew over the "
                 ...
                 f"publication offices the corpus itself draws on.")
...
def fig_dominant_design(v: pd.DataFrame, path: Path) -> Path:
    ...
    return _save(fig, path, SRC + "; levels defined in Table 4.1.3.1a, conditions in Table 4.1.3.2, numbers in Table 4.1.3.3b; "
                 ...
                 "A0c = class × propulsor bin, A1t = class × wings × tilting. (ii) and (iii) condition 2: the window is more concentrated "
...
def fig_dimension_drift(v: pd.DataFrame, path: Path) -> Path:
    ...
    metrics_ = [("median propulsive units", "median\npropulsive units", None),
                ("share with a ducted unit", "% of aircraft\nwith ducted units", 1),
                ("share with a tilting unit", "% of aircraft\nwith tilting units", 1),
                ("share with booms", "% of aircraft\nwith booms", 1)]
    fig, axes = plt.subplots(1, 4, figsize=(W, 2.9), gridspec_kw={"wspace": 0.42})
    x = np.arange(len(WINDOW_NAMES))
    for ax, (col, title, top) in zip(axes, metrics_):
        for code, sub in t.groupby("code", sort=False):
            sub = sub.set_index("window").reindex(WINDOW_NAMES)
            _class_line(ax, x, sub[col], sub["aircraft"], code, lw=1.8, ms=4)
        ...
        if top:
            ...
            ax.set_ylim(0)
        _hgrid(ax, "y")
    _class_legend(fig, t, ncol=3, y=-0.03)
    fig.subplots_adjust(bottom=0.30)
    # every line must stay under TEXT_W * 0.98 or _wrap_to flattens the whole suptitle and re-wraps it wider
    fig.suptitle("Dimension drift inside each class, per window: the four dimensions of Figure 4.1.4,\n"
                 "read one at a time. Every % is a share of the class's own aircraft, never of rotors",
    ...
    return _save(fig, path, SRC + "; Table 4.1.5, where a window with fewer than 5 aircraft of the class keeps its "
                 ...
                 "A line is an architecture class (the G1 top type), not an archetype; every point counts unique aircraft. "
                 "Each percentage divides by the class's own aircraft in that window (the `aircraft` column of Table 4.1.5, "
                 "all of them answered here) — never by a count of rotors. Substitution shows here before a class label "
                 "changes: a class whose propulsive-unit count climbs, or whose ducted share falls, is changing what it is "
                 "while keeping its name. Panel (iii) is near-flat by construction — Tilt Rotor, Combined vectored thrust "
                 "and Tilt Wing carry tilting units by definition of the class and Lift + Cruise carries none — so only "
                 "Multirotor moves there. " + _selection_line(t))
...
#: User ruling 2026-09-22: the rotorcraft have to be visible in Figure 4.1.2, not hidden in "Other".
#: {figure name: {class code: colour}}. The colour must be one the black-and-white pass knows, so
#: ``CAT[7]`` — the one categorical hue the seven fixed classes do not use — and never ``OTHER``.
ATLAS_EXTRA_CLASSES: Dict[str, Dict[str, str]] = {"atlas_arch_time": {"RC": CAT[7]}}
...
ATLAS_NOTE: Dict[str, tuple] = {
    ...
    "atlas_arch_gt": (
        ...
        "{dropped} the matrix cannot hold are named in the table of section 3.3: one whose patent states no "
        "class, and two the wizard left unlabelled, one of them read as 'Convertiplane' — outside the twelve "
        "classes. None was relabelled to fit.",
...
def _thin_bands(fig, colors: Dict[str, str]) -> None:
    ...
    for ax in fig.axes:
        ...
        for p in bars:
            fc = p.get_facecolor()
            if to_hex(fc[:3]) not in want or p.get_height() >= 0.05:
                continue                         # 5 % and over already carries its own label
            x = p.get_x() + p.get_width() + 0.03
            y = p.get_y() + p.get_height() / 2
            ax.text(x, y, f"{p.get_height():.0%}" if p.get_height() >= 0.01 else f"{p.get_height() * 100:.1f}%",
                    rotation=90, ha="left", va="center", fontsize=6.5, color=INK, clip_on=False, zorder=5,
                    bbox=dict(fc=SURFACE, ec="none", pad=0.6, alpha=0.85))
